...
- API contract docs: docs/srs-api-contracts.md (JSON schemas).
...
Full JSON schemas are in docs/srs-api-contracts.md.
...
- HotelDetail:
  - Components: ImageGallery, Facilities, RoomList (two rate types visible: refundable/non-refundable), Room quantity selectors, real-time Total room information box showing formula: ([Original Price]+[Tax&Fees]+[Commission]) * nights * qty
...
- Produce docs/srs-api-contracts.md (done)
