...
Integration tests verify that multiple services work together correctly in the local deployment environment. These tests validate:

- Service-to-service communication
- API endpoints and contracts
- Database operations (both Neon and static DB)
...
it('saves booking to Neon database', async () => {
...
```typescript
expect(response.status, `Expected 200 but got ${response.status}. Response: ${JSON.stringify(response.data)}`).toBe(200);
```
