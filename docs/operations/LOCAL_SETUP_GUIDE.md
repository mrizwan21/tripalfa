...
This guide walks you through setting up TripAlfa for local development with all services
running as native processes on your machine (no Docker required).

**Deployment Model:** Process-First (Services run directly on host machine)  
**Database:** 4-Database Architecture (Local PostgreSQL) - Core, Ops, Finance, Local  
...
- **PostgreSQL** 14+ (single instance running all 4 databases on port 5432)
...
## ⚠️ Important: Docker-Compose Change

**Previous Workflow:** `docker-compose up` started all backend services
**Current Workflow:** Services run natively on host machine; docker-compose only starts databases

The docker-compose.yml file has been simplified to only run the required databases (PostgreSQL + Redis). Backend services now run as native Node.js processes for better local development experience.

### Quick Start (Databases Only via Docker)

```bash
# Start only databases (PostgreSQL x4 + Redis)
docker-compose -f infrastructure/compose/docker-compose.yml up -d

# Then start services natively
pnpm run services:start  # or follow manual setup below
```
...
### 2.1 PostgreSQL Setup (4-Database Architecture)

TripAlfa uses a 4-database architecture:

- **tripalfa_local** - Static Reference Data (Hotel content, Duffel, LiteAPI)
- **tripalfa_core** - Core Business Data (Users, Bookings, Wallet, KYC, Inventory)
- **tripalfa_ops** - Operational Data (Notifications, Rules, Documents, Disputes)
- **tripalfa_finance** - Financial Data (Invoices, Commission, Suppliers, Loyalty)

#### Option A: Using Docker (if you have Docker installed)

```bash
docker run -d \
  --name tripalfa-postgres \
...
docker exec -i tripalfa-postgres psql -U postgres -c "CREATE DATABASE tripalfa_local;"
docker exec -i tripalfa-postgres psql -U postgres -c "CREATE DATABASE tripalfa_core;"
docker exec -i tripalfa-postgres psql -U postgres -c "CREATE DATABASE tripalfa_ops;"
docker exec -i tripalfa-postgres psql -U postgres -c "CREATE DATABASE tripalfa_finance;"
```
...
sudo -u postgres psql -c "CREATE DATABASE tripalfa_local;"
sudo -u postgres psql -c "CREATE DATABASE tripalfa_core;"
sudo -u postgres psql -c "CREATE DATABASE tripalfa_ops;"
sudo -u postgres psql -c "CREATE DATABASE tripalfa_finance;"
```
...
# Run migrations for all databases (see FOUR_DATABASE_ARCHITECTURE.md for details)
# Core database
prisma migrate deploy --schema=./schema.core.prisma
# Ops database
prisma migrate deploy --schema=./schema.ops.prisma
# Finance database
prisma migrate deploy --schema=./schema.finance.prisma
# Local database
prisma migrate deploy --schema=./schema.local.prisma
```

For detailed migration instructions, see [FOUR_DATABASE_ARCHITECTURE.md](../FOUR_DATABASE_ARCHITECTURE.md).
...
# Should show: tripalfa_local, tripalfa_core, tripalfa_ops, tripalfa_finance
```
...
#### Database URLs (4-Database Architecture)

```dotenv
# 4-Database Architecture - Local PostgreSQL
...
Before running services, generate the Prisma clients for all databases:
...
This generates clients for all 4 databases (core, ops, finance, local).
...
```bash
# Verify local PostgreSQL database connection
...
[Booking | User | Payment | ... Services]
    ↓
[Local PostgreSQL | Static DB | Redis Cache]
```
...
### Local PostgreSQL Database Connection Error
...
- `STATIC_DATABASE_URL` - Local PostgreSQL for reference data (port 5432)
- `CORE_DATABASE_URL` - Local PostgreSQL for core application data
- `OPS_DATABASE_URL` - Local PostgreSQL for operational data
- `FINANCE_DATABASE_URL` - Local PostgreSQL for financial data
- `LOCAL_DATABASE_URL` - Local PostgreSQL for local/hotel data
...
**Database:** Local PostgreSQL (4-database architecture)
