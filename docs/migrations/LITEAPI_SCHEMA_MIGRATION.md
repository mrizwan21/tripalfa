...
│  │   Port: 5432 (external)     │    │     Cloud PostgreSQL        │ │
│  │   Container: 5432           │    │                            │ │
│  ├─────────────────────────────┤    ├─────────────────────────────┤ │
│  │ • Hotels (full data)        │    │ • Users                     │ │
│  │ • Places                    │    │ • Bookings                  │ │
│  │ • Facilities                │    │ • Transactions              │ │
│  │ • Rooms & Amenities         │    │ • Wallet                    │ │
│  │ • Chains & Types            │    │ • Notifications             │ │
...
1. **Static Data Sync**: LiteAPI data is fetched and stored in Local Postgres
...
psql -h localhost -p 5432 -U staticdb_admin -d tripalfa_static
...
async function getHotelData(hotelId: string, language = 'en') {
  ...
  try {
    const apiResponse = await fetch(
      `https://api.liteapi.travel/data/hotel`,
      {
        method: 'POST',
    ...
    `, [hotelId, language, JSON.stringify({ hotelId, language }), 
        apiResponse.status, durationMs, 'success', null, 'hotel_not_found']);
    ...
    await staticDb.query(`
      SELECT hotel.log_api_request($1, $2, $3, $4, $5, $6, $7, $8)
    `, [hotelId, language, JSON.stringify({ hotelId, language }),
        null, durationMs, 'failed', error.message, 'hotel_not_found']);
