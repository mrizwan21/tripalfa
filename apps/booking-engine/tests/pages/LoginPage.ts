...
export class LoginPage extends BasePage {
  async login(email: string, password: string) {
    try {
      ...
      const isTestMode = process.env.NODE_ENV === 'test' || process.env.VITE_TEST_MODE === 'true';
      ...
      await expect(this.getByTestId('login-email')).toBeVisible({ timeout: 10000 });
  ...
  async isAlreadyLoggedIn(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const bodyVisible = await this.page.locator('body').isVisible({ timeout: 3000 });
      ...
      console.log('isAlreadyLoggedIn check failed, assuming not logged in:', error);
      return false;
    }
  }
  ...
  async signup(email: string, password: string, fullName: string) {
    try {
      ...
      await expect(this.getByTestId('signup-email')).toBeVisible({ timeout: 10000 });
      ...
      await this.page.waitForURL(/\/(dashboard|flights|hotels|login|\?|$)/, { timeout: 45000 });
      await this.page.waitForLoadState('domcontentloaded');
      ...
      if (currentUrl.includes('/signup')) {
        throw new Error('Signup failed: Still on signup page after signup attempt');
      }
  ...
  async signupWithRetry(email: string, password: string, fullName: string, maxRetries: number = 2) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries) {
          console.error(`Signup failed after ${maxRetries} attempts for user: ${email}`);
          throw error;
        }
        
        console.log(`Signup attempt ${attempt} failed, retrying in 2 seconds...`);
        await this.page.waitForTimeout(2000); // Wait before retry
  ...
  private async authenticateViaAPI(email: string, password: string) {
    try {
      ...
      const response = await apiContext.post('http://localhost:3004/auth/login', {
        data: {
          email,
          password,
          testMode: true
      ...
      try {
        await this.page.evaluate((token) => {
          if (typeof localStorage !== 'undefined') {
            ...
            localStorage.setItem('user', JSON.stringify({ email: 'test@example.com', id: 'test-user-id' }));
          }
        }, responseData.token);
      } catch (storageError) {
        console.log('localStorage not available in test context, continuing...');
