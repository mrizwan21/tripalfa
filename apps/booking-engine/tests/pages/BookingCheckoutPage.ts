...
export class BookingCheckoutPage extends BasePage {
  ...
  async payWithCard(
    ...
    await this.getByTestId("card-holder-name").fill(cardholderName, {
      force: true,
    });
  ...
  async completeWalletPayment() {
    ...
    await this.page.waitForSelector('[data-testid="wallet-balance-display"]', {
      timeout: 10000,
    });
  ...
  async verifyWalletBalanceDisplayed(): Promise<number> {
    const balanceText = await this.getByTestId(
  ...
  async verifyInsufficientBalanceWarning(): Promise<boolean> {
    const warning = await this.page
      .locator('[data-testid="insufficient-balance"]')
      .isVisible();
    return warning;
  }
  ...
  async getDiscountAmount(): Promise<number> {
    const discountText =
      await this.getByTestId("discount-amount").textContent();
  ...
  async proceedToPaymentFinalization(holdOrderId: string) {
    ...
    await this.page.waitForSelector(
      '[data-testid="payment-finalization-page"]',
      { timeout: 10000 },
    );
  }
  ...
  async completePayment(options: {
    ...
    await this.page.evaluate((paymentData) => {
      window.dispatchEvent(
        new CustomEvent("mockPaymentCompleted", {
          detail: {
            ...paymentData,
            status: "success",
            bookingRef: "BK123456",
  ...
  async schedulePaymentReminder(options: {
    ...
    await this.page.evaluate((reminderData) => {
      window.dispatchEvent(
        new CustomEvent("mockPaymentReminderScheduled", {
          detail: reminderData,
        }),
      );
    }, options);
  ...
  async selectPaymentMethod(
    method: "wallet" | "card" | "paypal" | "bank" | "balance" | "bank_transfer",
  ) {
  ...
  async initiateRefund(options: {
    ...
    await this.page.evaluate((opts) => {
      window.dispatchEvent(
        new CustomEvent("initiateRefund", {
          detail: opts,
        }),
      );
    }, options);
