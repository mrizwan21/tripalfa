...
export class PassengerDetailsPage extends BasePage {
  async fillPassengerDetails(
    ...
    await this.getByTestId(`passenger-first-name${suffix}`).fill(firstName, {
      force: true,
    });
    await this.getByTestId(`passenger-last-name${suffix}`).fill(lastName, {
      force: true,
    });

    if (options?.nationality) {
      await this.setSelectValue(
        `passenger-nationality${suffix}`,
        options.nationality,
      );
    }
    if (options?.gender) {
      ...
      await this.page
        .getByText(options.gender, { exact: true })
        .click({ force: true });
    }
    if (options?.residencyCountry) {
      await this.setSelectValue(
        `passenger-residency${suffix}`,
        options.residencyCountry,
      );
    }
    ...
    if (options?.passportNumber) {
      await this.getByTestId(`passenger-passport${suffix}`).fill(
        options.passportNumber,
        { force: true },
      );
    }
    ...
    if (options?.email) {
      await this.getByTestId(`passenger-email${suffix}`).fill(options.email, {
        force: true,
      });
    }
    if (options?.phone) {
      await this.getByTestId(`passenger-phone${suffix}`).fill(options.phone, {
        force: true,
      });
    }
  }
  ...
  async selectCalendarDate(
    calendarType: "date-of-birth" | "passport-expiry",
    dateString: string,
  ) {
    ...
    await this.page
      .getByTestId(`calendar-day-${dateString}`)
      .click({ force: true });
  }
  ...
  async fillBillingAddress(
    ...
    await this.page.fill(
      'input[placeholder="Building, Street Name, District"]',
      street,
      { force: true },
    );
  ...
  async continue() {
    ...
    const errorMessage = this.page.locator(
      "text=/Please fix errors above to proceed/i",
    );
    ...
    if (isErrorVisible) {
      ...
      const errorTexts = await this.page
        .locator('[class*="text-red-500"]')
        .allTextContents();
      throw new Error(
        `Form validation errors are present: ${errorTexts.join(", ")}`,
      );
    }
    ...
    if (invalidFields > 0) {
      throw new Error(
        `Found ${invalidFields} fields with validation errors (red borders)`,
      );
    }
    ...
    const alertMessage = this.page
      .locator("text=/Failed to process booking/i")
      .or(this.page.locator("text=/Unable to create booking/i"));
    ...
    if (currentUrl.includes("checkout")) {
      ...
      throw new Error(
        "Form submission did not trigger navigation. Form may be invalid or API call failed.",
      );
    } else {
