...
export class HotelHomePage extends BasePage {
  async searchHotel(
    ...
    await this.getByTestId("hotel-checkin-date").fill(checkInDate, {
      force: true,
    });
    await this.getByTestId("hotel-checkout-date").fill(checkOutDate, {
      force: true,
    });
  ...
  async fillCorporateDetails(details: {
    ...
    await this.getByTestId("corporate-company-name").fill(details.companyName, {
      force: true,
    });
    await this.getByTestId("corporate-billing-code").fill(details.billingCode, {
      force: true,
    });
    await this.getByTestId("corporate-cost-center").fill(details.costCenter, {
      force: true,
    });
    await this.getByTestId("corporate-po-number").fill(details.poNumber, {
      force: true,
    });
  ...
  async selectPackageDeal(
    dealType: "flight-hotel" | "hotel-car" | "all-inclusive",
  ) {
  ...
  async configureFlightDetails(details: {
    ...
    await this.getByTestId("package-flight-from").fill(details.from, {
      force: true,
    });
    await this.getByTestId("package-flight-to").fill(details.to, {
      force: true,
    });
    await this.getByTestId("package-flight-departure").fill(
      details.departureDate,
      { force: true },
    );
    await this.getByTestId("package-flight-return").fill(details.returnDate, {
      force: true,
    });
    await this.setSelectValue(
      "package-airline-preference",
      details.airlinePreference,
    );
  }

  async filterByAvailability(
    availabilityType: "last-minute" | "extended-stay" | "standard",
  ) {
