...
export class FlightHomePage extends BasePage {
  ...
  async addMultiCityLeg(from: string, to: string, date: string, adults: number, travelClass: string) {
    await this.getByTestId('add-multi-city-leg').click({ force: true });
    const legIndex = await this.getMultiCityLegCount();
    
    await this.getByTestId(`multi-city-from-${legIndex}`).fill(from, { force: true });
    await this.getByTestId(`multi-city-to-${legIndex}`).fill(to, { force: true });
    await this.getByTestId(`multi-city-date-${legIndex}`).fill(date, { force: true });
    await this.setSelectValue(`multi-city-adults-${legIndex}`, adults.toString());
