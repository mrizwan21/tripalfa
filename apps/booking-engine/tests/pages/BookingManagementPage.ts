...
export class BookingManagementPage extends BasePage {
  ...
  async viewBookingDetails(bookingReference: string) {
    ...
    await this.page.waitForSelector('[data-testid="booking-detail-page"]', {
      timeout: 10000,
    });
  }
  ...
  async verifyBookingVisible(bookingReference: string): Promise<boolean> {
    const booking = this.page.locator(
      `[data-testid="booking-reference"]:has-text("${bookingReference}")`,
    );
  ...
  async bulkCancelSelected() {
    ...
    await this.page.waitForSelector('[data-testid="bulk-cancel-confirm"]', {
      timeout: 10000,
    });
  }
