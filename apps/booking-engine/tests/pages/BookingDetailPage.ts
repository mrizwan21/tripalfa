...
export class BookingDetailPage extends BasePage {
  ...
  async addPassenger(passenger: PassengerInfo) {
    ...
    await this.getByTestId("passenger-firstname").fill(passenger.firstName, {
      force: true,
    });
    await this.getByTestId("passenger-lastname").fill(passenger.lastName, {
      force: true,
    });
    await this.getByTestId("passenger-passport").fill(passenger.passport, {
      force: true,
    });
  }
  ...
  async saveModifications() {
    ...
    await this.page.waitForSelector('[data-testid="modification-success"]', {
      timeout: 10000,
    });
  }
  ...
  async viewStatusHistory() {
    ...
    await this.page.waitForSelector('[data-testid="status-timeline"]', {
      timeout: 10000,
    });
  }
  ...
  async selectNewFlightOption(optionType: string) {
    await this.getByTestId(`flight-option-${optionType}`).click({
      force: true,
    });
    await this.page.waitForSelector('[data-testid="price-difference"]', {
      timeout: 10000,
    });
  }
  ...
  async proceedToPayment() {
    ...
    await this.page.waitForSelector('[data-testid="payment-page"]', {
      timeout: 10000,
    });
  }
  ...
  async selectAlternativeFlight(index: number) {
    await this.getByTestId(`alternative-flight-${index}`).click({
      force: true,
    });
  }
  ...
  async confirmRebooking() {
    ...
    await this.page.waitForSelector('[data-testid="new-booking-reference"]', {
      timeout: 10000,
    });
  }
  ...
  async addNote(noteText: string) {
    ...
    await this.page.waitForSelector('[data-testid="note-added"]', {
      timeout: 10000,
    });
  }
  ...
  async viewSpecialRequests() {
    ...
    await this.page.waitForSelector('[data-testid="special-requests-list"]', {
      timeout: 10000,
    });
  }
  ...
  async updateSpecialRequest(request: string) {
    ...
    await this.getByTestId("special-request-input").fill(request, {
      force: true,
    });
    await this.getByTestId("save-request-btn").click({ force: true });
    await this.page.waitForSelector('[data-testid="request-updated"]', {
      timeout: 10000,
    });
  }
  ...
  async verifyPassengerCount(expectedCount: number) {
    const passengers = await this.page
      .locator('[data-testid^="passenger-row-"]')
      .count();
