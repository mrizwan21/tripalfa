...
test.describe('End-to-End Wallet Booking Flow - Real Duffel API', () => {
  ...
  test('WB-E2E-001: Complete flight booking with wallet - full journey', async ({ page }) => {
    ...
    await passengerDetails.fillBillingAddress('456 Wallet St', 'Los Angeles', '90001', 'US');
    ...
    console.log(`Balance deduction verified: $${initialBalance} - $${bookingAmount} = $${finalBalance}`);
    ...
    const transactionExists = await walletPage.verifyTransactionExists(bookingReference);
    expect(transactionExists).toBe(true);
    
    console.log('Transaction found in wallet history');
  });

  test('WB-E2E-002: Flight booking with wallet - cancellation and refund to wallet', async ({ page }) => {
    const walletPage = new WalletPage(page);
    const flightHome = new FlightHomePage(page);
    const flightList = new FlightListPage(page);
    const flightDetail = new FlightDetailPage(page);
    ...
    await passengerDetails.fillBillingAddress('789 Cancel Ave', 'Chicago', '60601', 'US');
    await passengerDetails.continue();
    ...
    if (await cancelButton.isVisible()) {
      ...
      console.log('Cancellation button not visible - checking for alternative...');
    ...
    await walletPage.viewTransactions();
    const refundTransactionExists = await walletPage.verifyTransactionExists('refund');
    ...
    console.log('End-to-end wallet booking cancellation test completed successfully');
  });

  test('WB-E2E-003: Wallet booking with insufficient balance - top-up and complete', async ({ page }) => {
    const walletPage = new WalletPage(page);
    const flightHome = new FlightHomePage(page);
    const flightList = new FlightListPage(page);
    const flightDetail = new FlightDetailPage(page);
    ...
    if (lowBalance < bookingAmount) {
      ...
      const insufficientWarning = await checkout.verifyInsufficientBalanceWarning();
      console.log(`Insufficient balance warning visible: ${insufficientWarning}`);
      ...
      if (await topUpOption.isVisible()) {
        ...
        // Fill top-up form
        await page.getByTestId('topup-amount').fill(topUpAmount.toString(), { force: true });
        ...
        await expect(page.getByTestId('topup-success')).toBeVisible({ timeout: 10000 });
  ...
  test('WB-E2E-004: Multiple bookings with wallet - verify cumulative balance deduction', async ({ page }) => {
    const walletPage = new WalletPage(page);
    const flightHome = new FlightHomePage(page);
    const flightList = new FlightListPage(page);
    const flightDetail = new FlightDetailPage(page);
  ...
  test('WB-E2E-005: Wallet booking with promo code - verify discount applied', async ({ page }) => {
    const walletPage = new WalletPage(page);
    const flightHome = new FlightHomePage(page);
    const flightList = new FlightListPage(page);
    const flightDetail = new FlightDetailPage(page);
