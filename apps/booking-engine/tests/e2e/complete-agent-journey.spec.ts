...
test.describe('Complete Agent Manual Booking Journey E2E', () => {
  test('Agent creates manual flight booking with notifications', async ({ page }) => {
    ...
    await bookingMgmt.sendCustomNotification({
      ...
      message: 'Your flight booking request has been submitted. We will confirm availability shortly.',
      channels: ['email', 'sms']
    });
    ...
    await expect(page.locator('text=Booking submitted to supplier')).toBeVisible();
    ...
    await expect(page.locator('text=Booking amendment requested')).toBeVisible();

    // Step 12: Supplier approves amendment (simulate)
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('mockAmendmentApproval', {
  ...
  test('Agent sends bulk notifications', async ({ page }) => {
    ...
    await adminNotifications.selectCustomers(['customer1@test.com', 'customer2@test.com']);

    // Step 2: Compose bulk notification
    await adminNotifications.composeBulkNotification({
      title: 'Service Update',
      message: 'We have updated our booking system. Please check your bookings.',
      type: 'service_update',
