...
test.describe("Scheduled Notification Journey E2E", () => {
  test("Booking reminder scheduled and delivered on time", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Scheduled: Booking reminder"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Your flight departs in 3 days"),
    ).toBeVisible();
  ...
  test("Urgent reminder for next-day travel", async ({ page }) => {
    ...
    await page.goto("/notifications");
    await expect(
      page.locator("text=Your flight departs in 1 hour"),
    ).toBeVisible();

    // Step 6: Urgent reminder sent via multiple channels
    await expect(
      page.locator("text=Urgent: Email + SMS + Push sent"),
    ).toBeVisible();
  });
  ...
  test("Scheduled notification cancellation", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Booking reminders scheduled"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Scheduled reminders cancelled"),
    ).toBeVisible();
    ...
    await adminNotifications.goto("/admin/notifications");
    await expect(
      page.locator("text=No pending reminders for cancelled booking"),
    ).toBeVisible();
  });

  test("Scheduled notification rescheduling after amendment", async ({
    page,
  }) => {
    const login = new LoginPage(page);
    const checkout = new BookingCheckoutPage(page);
    ...
    await adminNotifications.goto("/admin/notifications");
    await expect(
      page.locator("text=Reminder rescheduled for 7 days before new date"),
    ).toBeVisible();
  });
});
