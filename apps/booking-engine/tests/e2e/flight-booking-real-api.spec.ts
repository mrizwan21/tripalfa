...
test.describe('Flight Booking Flow - Real API Data', () => {
  ...
  test('FB-002: Complete flight booking with real API data, ancillaries, and seat selection', async ({ page }) => {
  ...
  test('FB-003: Verify booking management dashboard functionality', async ({ page }) => {
    const bookingMgmt = new BookingManagementPage(page);
    ...
    if (bookingCards.length > 0) {
      ...
      await expect(page.getByTestId('ancillary-modification-modal')).toBeVisible();
    }
  });
});