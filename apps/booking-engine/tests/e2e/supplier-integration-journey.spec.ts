...
test.describe("Supplier Integration Journey E2E", () => {
  test("Complete Duffel order lifecycle with webhooks", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockDuffelWebhook", {
          detail: {
            type: "order.created",
  ...
  test("Supplier cancellation triggers refund process", async ({ page }) => {
    ...
    await page.goto("/notifications");
    await expect(
      page.locator("text=Your booking has been cancelled"),
    ).toBeVisible();
    await expect(
      page.locator("text=Refund of $1200 will be processed"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Supplier cancellation: ord_123"),
    ).toBeVisible();
  });
  ...
  test("Webhook signature validation and security", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockValidWebhook", {
          detail: {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockInvalidWebhook", {
          detail: {
    ...
    await expect(
      page.locator("text=Invalid webhook signature rejected"),
    ).toBeVisible();
    ...
    await adminNotifications.goto("/admin/notifications");
    await expect(
      page.locator("text=Security alert: Invalid webhook signature"),
    ).toBeVisible();
  });
  ...
  test("Supplier integration error handling", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockSupplierAPIError", {
    ...
    await adminNotifications.goto("/admin/notifications");
    await expect(
      page.locator("text=Supplier API error: rate_limit_exceeded"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Please check back in a few minutes"),
    ).toBeVisible();
  ...
  test("Multi-supplier integration (Duffel + LiteAPI)", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockSupplierRouting", {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockMultiSupplierConfirmation", {
          detail: {
    ...
    await page.goto("/notifications");
    await expect(
      page.locator("text=Multi-city booking confirmed"),
    ).toBeVisible();
    ...
    await adminNotifications.goto("/admin/notifications");
    await expect(
      page.locator("text=Duffel: ord_duffel_123 confirmed"),
    ).toBeVisible();
    await expect(
      page.locator("text=LiteAPI: ord_liteapi_456 confirmed"),
    ).toBeVisible();
  });
});
