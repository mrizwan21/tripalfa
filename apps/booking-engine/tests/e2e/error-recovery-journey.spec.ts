...
test.describe("Error Recovery Journey E2E", () => {
  test("Validation error recovery flow", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Please correct the following errors"),
    ).toBeVisible();
  ...
  test("Payment processing error recovery", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockPaymentProcessingError", {
  ...
  test("System error recovery with graceful degradation", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockSystemError", {
          detail: {
    ...
    await expect(
      page.locator("text=Please try again in a few minutes"),
    ).toBeVisible();

    // Step 4: System recovers
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockSystemRecovery", {
  ...
  test("Network connectivity error recovery", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockNetworkError", {
