...
test.describe('Complete User Journey E2E', () => {
  test('Flight booking, payment, notifications, supplier webhook, reminders', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('mockSupplierWebhook', { detail: { type: 'itinerary_change' } }));
    });
