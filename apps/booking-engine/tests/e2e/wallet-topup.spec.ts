...
test.describe("Wallet Top-Up Flow", () => {
  test("WT-001: Top up wallet with $5000 USD and verify dashboard update", async ({
    page,
  }) => {
    // ========================================
    ...
    const walletSnapshotCard = page
      .locator('div:has-text("Wallet snapshot")')
      .first();
    await expect(walletSnapshotCard).toBeVisible();

    // Get the balance from dashboard
    const dashboardBalanceText = await walletSnapshotCard
      .locator(".font-medium")
      .first()
      .textContent();
    const dashboardBalance = parseFloat(
      dashboardBalanceText?.replace(/[^0-9.]/g, "") || "0",
    );
    ...
    const dashboardWalletCard = page
      .locator('div:has-text("Wallet snapshot")')
      .first();
    await expect(dashboardWalletCard).toBeVisible();

    const updatedDashboardBalanceText = await dashboardWalletCard
      .locator(".font-medium")
      .first()
      .textContent();
    const updatedDashboardBalance = parseFloat(
      updatedDashboardBalanceText?.replace(/[^0-9.]/g, "") || "0",
    );
    console.log(`Updated dashboard balance: $${updatedDashboardBalance}`);

    // Dashboard should now show balance increased by $5000
    const dashboardDifference = Math.abs(
      updatedDashboardBalance - expectedBalance,
    );
  ...
  test("WT-002: Verify wallet balance persists across navigation", async ({
    page,
  }) => {
    ...
    const walletBalance1 = await page
      .getByTestId("wallet-balance")
      .textContent();
    ...
    await page
      .getByTestId("wallet-balance")
      .waitFor({ state: "visible", timeout: 30000 });
    const walletBalance2 = await page
      .getByTestId("wallet-balance")
      .textContent();
