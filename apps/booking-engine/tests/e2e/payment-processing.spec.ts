...
test.describe("Payment Processing", () => {
  ...
  test("PP-001: Card payment success", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="card-payment-form"]'),
    ).toBeVisible();
  ...
  test("PP-003: Card payment - Insufficient funds", async ({ page }) => {
    ...
    await page
      ...
      .fill("4000000000009995", { force: true });
    ...
    await expect(
      page.locator('[data-testid="wallet-payment-form"]'),
    ).toBeVisible();
  });
  ...
  test("PP-004: Card payment - Invalid card number", async ({ page }) => {
    ...
    await page
      .getByTestId("card-number")
      .fill("1234567890123456", { force: true });
    await page.getByTestId("card-expiry").fill("12/30", { force: true });
    await page.getByTestId("card-cvc").fill("123", { force: true });
    await page
      .getByTestId("card-holder-name")
