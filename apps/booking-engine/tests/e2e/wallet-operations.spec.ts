...
const usersModule = await import("../fixtures/users.json", {
  with: { type: "json" },
});
const flightsModule = await import("../fixtures/flights.json", {
  with: { type: "json" },
});
const paymentsModule = await import("../fixtures/payments.json", {
  with: { type: "json" },
});
...
test.describe("Wallet Operations Flow", () => {
  ...
  test("WO-001: View wallet balance and transaction history", async ({
    page,
  }) => {
    ...
    await expect(page.locator('[data-testid="wallet-currency"]')).toHaveText(
      /USD|EUR|GBP/,
    );
    ...
    if (transactionCount > 0) {
      ...
      await expect(
        page.locator('[data-testid="transaction-type"]'),
      ).toBeVisible();
      await expect(
  ...
  test("WO-002: Top-up wallet with card payment", async ({ page }) => {
    ...
    await page
      .getByTestId("topup-amount-input")
      .fill(topUpAmount.toString(), { force: true });

    // Verify amount validation
    await expect(
      page.locator('[data-testid="topup-amount-display"]'),
    ).toBeVisible();
    ...
    if (await savedCardButton.isVisible()) {
      ...
      await page
        .getByTestId("card-holder-name")
        .fill("Test User", { force: true });
    }
    await page.getByTestId("topup-pay-button").click({ force: true });

    // Verify top-up success
    await expect(page.getByText(/top.*up.*successful|successful/i)).toBeVisible(
      { timeout: 10000 },
    );
  ...
  test("WO-003: Top-up wallet - Invalid amount validation", async ({
    page,
  }) => {
    const walletPage = new WalletPage(page);
    ...
    const isNegativeErrorVisible = await negativeError
      .isVisible()
      .catch(() => false);
  ...
  test("WO-004: Top-up wallet - Payment failure", async ({ page }) => {
    ...
    await page
      .getByTestId("card-number")
      .fill("4000000000000002", { force: true });
  ...
  test("WO-005: Use wallet for booking payment", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="wallet-balance-display"]'),
    ).toBeVisible();
    ...
    if (balanceBeforeBooking >= bookingAmount) {
      ...
      await expect(
        page.locator('[data-testid="confirmation-page"]'),
      ).toBeVisible();
      ...
      const isTransactionVisible = await transactionRef
        .isVisible()
        .catch(() => false);
  ...
  test("WO-006: Wallet transfer (if feature exists)", async ({ page }) => {
    ...
    if (await transferButton.isVisible()) {
      ...
      await page
        .getByTestId("recipient-email")
        .fill(users.premiumUser?.email || "premium@test.com", { force: true });
      ...
      await expect(
        page.getByText(/transfer.*successful|successful/i),
      ...
      const isTransferVisible = await transferText
        .isVisible()
        .catch(() => false);
  ...
  test("WO-008: Filter wallet transactions", async ({ page }) => {
    ...
    const topupTransactions = await page
      .locator('[data-testid="transaction-row"]')
      .all();
    for (const transaction of topupTransactions) {
      const typeText = await transaction
