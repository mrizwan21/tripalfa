...
test.describe("Multi-Channel Notification Journey E2E", () => {
  test("High-priority booking triggers all channels simultaneously", async ({
    page,
  }) => {
  ...
  test("Channel failure does not block other channels", async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockChannelFailure", {
  ...
  test("Emergency notifications bypass channel preferences", async ({
    page,
  }) => {
    const login = new LoginPage(page);
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("mockEmergencyNotification", {
    ...
    await expect(
      page.locator("text=Email notification sent (emergency)"),
    ).toBeVisible();
    await expect(
      page.locator("text=SMS notification sent (emergency)"),
    ).toBeVisible();
    await expect(
      page.locator("text=Push notification sent (emergency)"),
    ).toBeVisible();
  });
});
