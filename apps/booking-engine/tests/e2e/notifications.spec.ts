...
test.describe('Notifications E2E Tests', () => {
  ...
  test.describe('Scenario 1: View Notifications List', () => {
    test('should display notifications page with all elements', async ({ page }) => {
      // Verify page title
      ...
      await expect(page.locator('[role="list"]')).toHaveCount(1, { timeout: 5000 });
    });
  ...
  test.describe('Scenario 2: View Notification Details', () => {
    test('should open details popup when clicking notification', async ({ page }) => {
      const viewButton = page.locator('text=VIEW DETAILS').first();
      await viewButton.click();

      // Popup should be visible
      const popup = page.locator('[role="dialog"], [role="presentation"]').first();
    ...
    test('should close popup with close button', async ({ page }) => {
      ...
      const closeButton = page.locator('[aria-label="Close notification details"]');
    ...
    test('should display passenger name for SSR notifications', async ({ page }) => {
      const viewButton = page.locator('text=VIEW DETAILS').first();
      await viewButton.click();
  ...
  test.describe('Scenario 3: Mark Notification as Read', () => {
    test('should mark notification as read', async ({ page }) => {
      ...
      const initialBadge = page.locator('.rounded-full:has-text(/^\\d+$/)').first();
      ...
      if (await markReadButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const updatedBadge = page.locator('.rounded-full:has-text(/^\\d+$/)').first();
        const updatedCount = parseInt(await updatedBadge.textContent(), 10);
        expect(updatedCount).toBeLessThan(initialCount);
      }
    });

    test('should update visual indicator when marked as read', async ({ page }) => {
      const markReadButton = page.locator('text=MARK AS READ').first();
      if (await markReadButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await expect(
          notification.locator('text=MARK AS READ')
  ...
  test.describe('Scenario 6: Pagination', () => {
    test('should display pagination controls', async ({ page }) => {
      ...
      if (await paginationButton.isVisible({ timeout: 1000 }).catch(() => false)) {
    ...
    test('should change page size', async ({ page }) => {
      ...
      if (await pageSizeSelect.isVisible({ timeout: 1000 }).catch(() => false)) {
