...
test.describe("Wallet Management - Day 5-6 Enhanced Tests", () => {
  ...
  test("WL-001: View wallet balance and transaction history", async ({
    page,
  }) => {
  ...
  test("WL-003: Wallet top-up with declined card handling", async ({
    page,
  }) => {
    const walletPage = new WalletPage(page);
    const topUpPage = new WalletTopUpPage(page);
  ...
  test("WL-008: Wallet refund processing from cancelled booking", async ({
    page,
  }) => {
    const walletPage = new WalletPage(page);
