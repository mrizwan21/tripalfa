...
test.describe("Error Scenarios", () => {
  ...
  test("ES-002: Handle network errors", async ({ page }) => {
    ...
    await expect(
      page.getByText(/network.*error|connection.*failed|error/i),
  ...
  test("ES-003: Handle 500 server errors", async ({ page }) => {
    ...
    await expect(
      page.getByText(/something.*went.*wrong|server.*error|error/i),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("ES-005: Handle invalid search parameters", async ({ page }) => {
    ...
    await expect(page.getByText(/invalid|error|not found/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("ES-006: Handle no search results", async ({ page }) => {
    ...
    await expect(
      page.getByText(/no.*flights.*found|no.*results|no.*available/i),
    ).toBeVisible({ timeout: 10000 });
  ...
  test("ES-007: Handle API 403 Forbidden errors", async ({ page }) => {
    ...
    // Verify error message
    await expect(
      page.getByText(/forbidden|unauthorized|access denied|error/i),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("ES-009: Handle malformed API responses", async ({ page }) => {
    ...
    // Verify error message
    await expect(page.getByText(/error|something.*wrong|failed/i)).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("ES-010: Handle concurrent request failures", async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid*="flight-card"]', { timeout: 10000 })
      .catch(() => {});

    // Attempt to select flight and mock failure on required subsequent API calls
    await page.route("**/api/flights/details", (route) =>
      route.abort("failed"),
    );
  ...
  test("ES-011: Handle API rate limiting (429 errors)", async ({ page }) => {
    ...
    await expect(
      page.getByText(/rate.*limit|too.*many|slow.*down|try.*later|error/i),
    ).toBeVisible({ timeout: 10000 });
  });

  test("ES-012: Handle missing required fields in response", async ({
    page,
  }) => {
    const flightHome = new FlightHomePage(page);
