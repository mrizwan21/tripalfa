...
test.describe('Complete Wallet Management Journey E2E', () => {
  ...
  test('Wallet payment during booking with balance notifications', async ({ page }) => {
    const login = new LoginPage(page);
    const wallet = new WalletPage(page);
  ...
  test('Wallet transaction history and notifications', async ({ page }) => {
    ...
    await page.goto('/notifications');
    await expect(page.locator('text=Transaction history exported')).toBeVisible();
  });
  ...
  test('Wallet auto-recharge and notifications', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('mockLowBalance', {
  ...
  test('Wallet refund processing and notifications', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('mockRefundInitiated', {
  ...
  test('Wallet security and fraud notifications', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('mockSuspiciousActivity', {
