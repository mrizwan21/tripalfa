...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    const isAuthenticated = await page.locator('[data-testid="user-menu"]').isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (!isAuthenticated) {
      ...
      // Fill login form
      await page.getByTestId('login-email').fill(process.env.TEST_USER_EMAIL || 'testuser1@example.com');
      await page.getByTestId('login-password').fill(process.env.TEST_USER_PASSWORD || 'Test@123');
