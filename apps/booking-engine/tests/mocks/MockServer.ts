...
export class MockServer {
  ...
  private async setupEventListeners() {
    ...
    await this.page.exposeFunction('handleMockPaymentCompleted', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockWalletCredit', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockWalletDebit', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockWalletTransfer', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockLowBalanceAlert', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockHoldOrderCreated', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockRefundProcessed', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockBankTransferCompleted', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockBankTransferFailed', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockPaymentReminderScheduled', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockUrgentPaymentReminder', async (eventData: any) => {
    ...
    await this.page.exposeFunction('handleMockTimeAdvanced', async (eventData: any) => {
  ...
  async mockWebhookEndpoint(type: string, options?: { simulateFailure?: boolean }) {
    ...
    await this.page.route('**/wallet/transactions', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          transaction: { ...body, id: `txn_${Math.random().toString(36).substr(2, 9)}` }
        })
      });
    });
  }
  ...
  async clearNotifications() {
    ...
    await this.page.request.post(`${this.baseUrl}/notifications/sent`, {
      data: { action: 'clear' }
    }).catch(() => {}); // Ignore errors
  }
    ...
    await this.page.route('**/bank/transfer/*', async (route) => {
      ...
      if (status === 'success' && isComplete) {
        ...
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ status: 'failed', error: 'insufficient_funds' })
        });
      } else {
        await route.fulfill({
          status: 200,
    ...
    await this.page.route('**/payments/reminder*', async (route) => {
      ...
      if (url.includes('/urgent')) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ sent: true, reminderId: Math.random().toString(36).substr(2, 9) })
        });
      } else {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ scheduled: true, reminderId: Math.random().toString(36).substr(2, 9) })
        });
      }
    });
  }
  ...
  async simulateWebhookEvent(type: string, data: any) {
    await this.page.evaluate(({ webhookType, webhookData }) => {
  ...
  async simulateWalletEvent(type: string, data: any) {
    await this.page.evaluate(({ eventType, eventData }) => {
      window.dispatchEvent(new CustomEvent('mockWalletEvent', {
  ...
  async simulateBankTransferEvent(type: string, data: any) {
    await this.page.evaluate(({ eventType, eventData }) => {
      window.dispatchEvent(new CustomEvent(`mockBankTransfer${eventType}`, {
        detail: eventData
      }));
    }, { eventType: type, eventData: data });
  }
  ...
  async simulatePaymentReminderEvent(type: string, data: any) {
    await this.page.evaluate(({ eventType, eventData }) => {
      window.dispatchEvent(new CustomEvent(`mockPaymentReminder${eventType}`, {
        detail: eventData
      }));
    }, { eventType: type, eventData: data });
  }
  ...
  async getWebhookEvents(): Promise<any[]> {
    try {
      const response = await this.page.request.get(`${this.baseUrl}/webhooks/events`);
  ...
  async getWalletBalance(userId: string): Promise<number> {
    try {
      const response = await this.page.request.get(`${this.baseUrl}/wallet/balance/${userId}`);
