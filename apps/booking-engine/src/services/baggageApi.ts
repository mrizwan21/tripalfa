...
export async function checkBaggageEligibility(
  ...
  try {
    ...
    const result = await api.post<any>("/api/bookings/baggage/eligibility", {
      offerId,
...
export async function getAvailableBaggage(
  ...
  try {
    console.log(
      "[Baggage] Fetching available baggage services for offer:",
      offerId,
    );
...
export async function bookBaggage(
  ...
  try {
    console.log("[Baggage] Booking baggage services:", {
      offerId,
