...
export async function getBookingAirlineCredits(
  ...
  try {
    console.log(
      "[AirlineCredits] Fetching applicable credits for booking:",
      bookingId,
    );

    const result = await api.post<any>("/api/admin/airline-credits/booking", {
      bookingId,
      orderId,
    ...
    return {
      ...
      averageExpiryDays:
        credits.credits.length > 0
          ? Math.floor(totalExpiryDays / credits.credits.length)
          : 0,
    };
  } catch (error) {
 ...
 */
export async function getCreditsForDisplay(customerId: string): Promise<{
  ...
  try {
    ...
    return {
      active: credits.credits.filter(
        (c) => new Date(c.expiryDate) > thirtyDaysFromNow,
      ),
