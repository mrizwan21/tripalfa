...
function offerToExtraSlice(offer: any) {
  ...
  return {
    ...
    duration:      secsToDuration(offer.duration?.departure ?? offer.duration?.total ?? 0),
...
function mapOffer(
  ...
  if (bl.hand_weight) {
    bags.push({ quantity: 1, weight: bl.hand_weight, unit: 'kg', type: 'carry_on' });
  }
  if (bl.hold_weight) {
    bags.push({ quantity: bl.hold_dimensions_sum ?? 1, weight: bl.hold_weight, unit: 'kg', type: 'checked' });
  }
 ...
 */
async function fetchLeg(leg: KiwiLeg, adults: number, currency: string): Promise<any[]> {
  try {
    ...
    if (!res.ok) {
      ...
      console.warn(`[kiwiNomadApi] Leg ${leg.origin}→${leg.destination} HTTP ${res.status}: ${errText}`);
      return [];
    }
    ...
    console.warn(`[kiwiNomadApi] Leg ${leg.origin}→${leg.destination} failed:`, err);
    return [];
  }
}
...
export async function searchKiwiMultiCity(
  ...
  if (legs.length < 2) {
    throw new Error('[kiwiNomadApi] Multi-city search requires at least 2 legs.');
  }

  console.log(`[kiwiNomadApi] Searching ${legs.length} legs via Kiwi Nomad:`, legs);
