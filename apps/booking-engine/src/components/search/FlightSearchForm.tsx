...
function AnimatedIcon({
  ...
  return (
    <Icon
      className={cn(
        'transition-all duration-300 group-hover/icon:scale-110 group-hover/icon:text-[hsl(var(--primary))]',
        className
      )}
      size={size}
      weight={weight}
...
function AirportSuggestion({
  ...
  return (
    <button
      type="button"
      className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-[hsl(var(--primary))]/5 transition-all duration-200 text-left border-b border-gray-100/50 last:border-b-0 group/item"
      onClick={() => onSelect(airport)}
    >
      <div className="w-10 h-10 rounded-xl bg-[hsl(var(--primary))]/10 flex items-center justify-center shrink-0 transition-all duration-200 group-hover/item:bg-[hsl(var(--primary))]/15">
        <Icon className="text-[hsl(var(--primary))]" size={18} weight="duotone" />
      </div>
      ...
      <div className="px-2.5 py-1 rounded-lg bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))] text-xs font-bold shrink-0 font-mono">
        {airport.code}
      </div>
    </button>
  );
...
export function FlightSearchForm({
  ...
  const handleAirportSelect = (airport: AirportResult, field: 'origin' | 'destination') => {
    if (field === 'origin') {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
        ...
        <div ref={originRef} className="lg:col-span-3 relative group">
          ...
          {originAirports.length > 0 && activeField === 'origin' && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-xl rounded-xl shadow-2xl shadow-black/10 ring-1 ring-gray-200/50 z-50 max-h-72 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="lg:col-span-1 flex items-center justify-center pt-6">
          <button
            ...
            className={cn(
              ...
              'hover:shadow-lg hover:border-[hsl(var(--primary))]/30 hover:bg-[hsl(var(--primary))]/5',
          ...
          >
            <ArrowsLeftRight
              className="text-gray-500 group-hover:text-[hsl(var(--primary))] transition-all duration-300"
              size={18}
              weight="duotone"
            />
          </button>
          ...
          <div className="relative">
            <div className="absolute inset-y-0 left-0 w-12 flex items-center justify-center pointer-events-none">
              <CalendarBlank
                size={20}
                weight="duotone"
                className="text-gray-400 group-focus-within:text-[hsl(var(--primary))] transition-all duration-300"
              />
            </div>
            <Input
              type="date"
