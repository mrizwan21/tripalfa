...
interface WeatherWidgetProps {
  ...
  hourly?: Array<{ dt: number; temp: number; icon: string; description: string }>;
  daily?: DailyForecast[];
  timezone?: string;
  ...
  if (error) {
    return (
      <div className="bg-red-50 rounded-2xl p-4 mb-4 border border-red-200 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-red-900">Weather unavailable</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 pt-4 border-t border-blue-200">
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-blue-900">{current.humidity}%</span>
          </div>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-sm font-bold text-blue-900">{Math.round(current.uvi)}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="weather-widget">
      <WeatherCurrentCard
        ...
        error={error}
      />
      {!loading && !error && daily && <WeatherForecastCards daily={daily} units={units} />}
    </div>
  );
}