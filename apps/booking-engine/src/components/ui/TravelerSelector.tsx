...
export function TravelerSelector() {
    ...
    const categories: { key: keyof PassengerCounts; label: string; sub: string }[] = [
    ...
    return (
        <div>
            ...
            <Popover.Root open={open} onOpenChange={setOpen} modal={true}>
                ...
                <Popover.Portal>
                    <Popover.Content className="w-80 bg-white rounded-xl shadow-xl border border-gray-100 p-4 z-[100] animate-in fade-in-0 zoom-in-95" sideOffset={8} align="start" onOpenAutoFocus={(e) => e.preventDefault()}>
                        <h3 className="font-bold text-lg mb-4 text-gray-900 border-b pb-2">Travellers</h3>
                        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                            {categories.map((cat) => (
                                <div key={cat.key} className="flex justify-between items-center bg-white">
                                    <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className="w-8 h-8 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:border-blue-600 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                        >
                                            <Minus size={14} />
                                        </button>
                                        <span className="w-4 text-center font-semibold text-gray-900">{counts[cat.key]}</span>
                                        <button
                                            onClick={() => updateCount(cat.key, 1)}
                                            className="w-8 h-8 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:border-blue-600 hover:text-blue-600 transition-colors"
                                        >
                                            <Plus size={14} />
                                        </button>
                                    </div>
