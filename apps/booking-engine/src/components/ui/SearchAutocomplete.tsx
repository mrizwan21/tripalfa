...
export function SearchAutocomplete({
  ...
  icon,
  dataTestId,
}: SearchAutocompleteProps): React.JSX.Element {
  ...
  const handleSearch = async (val: string) => {
    ...
    if (val.length < 2) {
      console.log(
        `[SearchAutocomplete] Query too short (${val.length} chars), not fetching`,
      );
      setIsOpen(false);
      setResults([]);
      return;
    ...
    try {
      ...
      console.log(
        `[SearchAutocomplete] Opening dropdown with ${suggestions.length} items`,
      );
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="relative h-full flex items-center px-4 gap-2">
        <div className="mr-1 text-muted-foreground flex-shrink-0 gap-4">
          {icon ? (
            icon
          ) : (
            <MapPin className="text-[hsl(var(--secondary))]" size={16} />
          )}
        </div>
        <input
          type="text"
          ...
          className={`w-full h-full bg-transparent border-none text-sm font-bold text-foreground placeholder:text-muted-foreground focus:ring-0 p-0 truncate ${className}`}
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute top-full left-0 w-full min-w-[300px] mt-2 bg-card rounded-xl shadow-2xl border border-border py-2 z-[999] animate-in fade-in slide-in-from-top-1">
          <div className="max-h-64 overflow-y-auto no-scrollbar">
            {results.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="px-4 py-3 hover:bg-muted/50 cursor-pointer flex items-center gap-3 group transition-colors border-b last:border-0 border-border/50"
                  onClick={() => {
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-muted/50 flex items-center justify-center text-muted-foreground group-hover:bg-[hsl(var(--secondary)/0.1)] group-hover:text-[hsl(var(--secondary))] transition-colors gap-2">
                    <IconComp size={16} />
                  </div>
                  ...
                  {item.code && (
                    <span className="text-[10px] font-black uppercase bg-muted/50 text-muted-foreground px-1.5 py-0.5 rounded ml-2">
