 ...
 */
function getDefaultCoordinates(city?: string, country?: string): MapCoordinates {
  // Default to Dubai
...
function HotelLocationCard({
  ...
  return (
    <div className="absolute bottom-6 left-6 right-6 md:left-auto md:right-6 md:w-80 bg-white rounded-2xl shadow-2xl p-6 z-10 border border-gray-100">
      ...
      {onNavigate && (
        <button
          onClick={onNavigate}
          className="mt-4 w-full h-10 bg-[#6366F1] hover:bg-[#5558E3] text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2"
        >
          <Navigation size={14} />
          Get Directions
        </button>
    ...
    return (
      ...
      >
        <div className="text-center p-8">
          ...
          <p className="text-gray-400 text-sm mt-2">Hotel coordinates not provided</p>
        </div>
      </div>
    );
  }
