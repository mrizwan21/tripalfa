...
export const DiscountModal: React.FC<DiscountModalProps> = ({
    ...
    const title = (
        <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Apply Discount</h3>
        </div>
    );
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
                <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Discount Type</label>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            {type === 'fixed' ? <span className="text-[var(--color-text-tertiary)] text-sm">{currency}</span> : <Tag className="h-4 w-4 text-[var(--color-text-tertiary)]" />}
                        </div>
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Reason / Coupon Code</label>
                    <input
                        type="text"
                        required
                        value={reason}
