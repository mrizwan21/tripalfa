...
export const getDocuments = async (
  ...
  try {
    ...
    if (!bookingId) {
      return res
        .status(400)
        .json({ success: false, error: "Booking ID is required" });
    }
 ...
 */
function generateSampleFlightBooking(bookingId: string): {
...
function generateSampleHotelBooking(bookingId: string): {
...
export const downloadDocument = async (
  ...
  try {
    const { bookingId: rawBookingId, documentType: rawDocumentType } =
      req.params;
    const bookingId = Array.isArray(rawBookingId)
      ? rawBookingId[0]
      : rawBookingId;
    ...
    if (!bookingId || !documentType) {
      return res.status(400).json({
        success: false,
        error: "Booking ID and document type are required",
      });
    }
    ...
    switch (documentType) {
      ...
      case "credit_note":
        ...
        const refund: RefundDetails = {
          ...
          originalPaymentAmount: 1250.0,
          cancellationFees: 250.0,
          taxRefund: 500.0,
        };
        documentName = "Refund Note";
        htmlContent = generateRefundNote(
          refund,
          booking as FlightBooking | HotelBooking,
          customer,
        );
        break;
      case "debit_note":
        ...
        const debitNote: DebitNoteDetails = {
          ...
          originalBookingAmount: 1250.0,
          adjustmentAmount: 150.0,
          adjustedAmount: 1400.0,
        };
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${documentName.replace(/\s+/g, "_")}_${bookingId}.html"`,
    );
...
export const issueTicket = async (
  ...
  try {
    ...
    if (!walletId || !amount || !currency) {
      return res.status(400).json({
        success: false,
        error: "walletId, amount, and currency are required",
      });
    }
 ...
 */
export const sendOfflineRequest = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
