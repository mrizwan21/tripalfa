import { Controller, Post, Get, Body, Query, Param, UseMiddleware, Logger, HttpStatus } from '@nestjs/common';
...
export class PricingController {
  ...
  async calculatePrice(@Body() request: any) {
    try {
      ...
      const result = await this.pricingService.calculatePricing({
        ...
        bookingDate: request.bookingDate ? new Date(request.bookingDate) : new Date(),
  ...
  async getPriceBreakdown(@Param('bookingId') bookingId: string) {
    try {
      ...
      const breakdown = await this.pricingService.getPricingBreakdown(bookingId);
  ...
  async getPricingAuditLogs(@Body() request: any) {
    try {
      this.logger.log(`Retrieving audit logs for booking: ${request.bookingId}`);
