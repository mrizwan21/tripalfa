        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <CardTitle className="text-lg">{getStatusText(request.status)}</CardTitle>
            </div>
            <CardDescription>Request submitted {timeElapsed || 'recently'}</CardDescription>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-600 mb-1">Priority</p>
            <Badge className="bg-blue-100 text-blue-800">{request.priority || 'Medium'}</Badge>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-600">{new Date(request.createdAt).toLocaleString()}</p>
            </div>
          </div>

          <div className="flex items-center gap-3 text-sm">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
              [OfflineRequestStatus.PRICING_SUBMITTED, OfflineRequestStatus.PENDING_CUSTOMER_APPROVAL, OfflineRequestStatus.APPROVED, OfflineRequestStatus.PAYMENT_PENDING, OfflineRequestStatus.COMPLETED].includes(request.status)
                ? 'bg-blue-100'
                : 'bg-gray-100'
            }`}>
              {[OfflineRequestStatus.PRICING_SUBMITTED, OfflineRequestStatus.PENDING_CUSTOMER_APPROVAL, OfflineRequestStatus.APPROVED, OfflineRequestStatus.PAYMENT_PENDING, OfflineRequestStatus.COMPLETED].includes(request.status) ? (
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-600">Our team is evaluating your request</p>
            </div>
          </div>

          <div className="flex items-center gap-3 text-sm">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
              [OfflineRequestStatus.APPROVED, OfflineRequestStatus.PAYMENT_PENDING, OfflineRequestStatus.COMPLETED].includes(request.status)
                ? 'bg-green-100'
                : 'bg-gray-100'
            }`}>
              {[OfflineRequestStatus.APPROVED, OfflineRequestStatus.PAYMENT_PENDING, OfflineRequestStatus.COMPLETED].includes(request.status) ? (
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-600">Awaiting your decision on updated pricing</p>
            </div>
          </div>
        </div>
        ...
        {request.requestedChanges && (
          <div className="bg-white rounded-lg p-3 border border-gray-200">
            <p className="text-sm font-medium text-gray-900 mb-2">Changes Requested</p>
        ...
        <div className="bg-white rounded-lg p-3 border border-gray-200">
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            <div>
              ...
              <p className="font-semibold text-gray-900">${priceImpact.original.toFixed(2)}</p>
            </div>
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">${priceImpact.approved.toFixed(2)}</p>
            </div>
          </div>
        ...
        {canAcceptReject && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <h4 className="font-semibold text-green-900 mb-3">Your Decision Required</h4>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-green-700 mt-0.5">
                  Your booking has been updated with the approved changes. Check your email for confirmation and documents.
                </p>
              </div>
            </div>
            {request.requestedChanges?.newTotalPrice && (
              <div className="mt-3 pt-3 border-t border-green-200">
                <a href="#" className="text-sm text-green-600 font-medium hover:text-green-700 flex items-center gap-1">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-0.5">
                  Unfortunately, your requested changes could not be accommodated. Please contact support for more information.
                </p>
              </div>
            </div>
          </div>
