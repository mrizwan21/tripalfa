...
import { MoreHorizontal, Eye, CreditCard, XCircle, Edit, ChevronDown } from 'lucide-react';

export function BookingActions({ status, product, id }: { status: string; product: string; id?: string }) {
  const navigate = useNavigate();
  const [isOpen, setIsOpen] = useState(false);

  const actions = [
    { label: 'View Details', icon: Eye, value: 'View' },
    { label: 'Pay Now', icon: CreditCard, value: 'Pay', disabled: !status.includes('On hold') },
