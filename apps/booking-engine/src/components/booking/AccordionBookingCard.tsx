...
export const AccordionBookingCard: React.FC<AccordionBookingCardProps> = ({ booking }) => {
    return (
        <div className="max-w-md mx-auto bg-[var(--color-bg-tertiary)] min-h-screen sm:min-h-0 sm:bg-[var(--color-bg-primary)]">
            ...
            <div className="bg-[var(--color-bg-primary)] p-5 mb-4 border-b border-[var(--color-border-light)] sm:border sm:rounded-xl">
                ...
                <div className="text-sm text-[var(--color-text-tertiary)]">
                    Ref: <span className="font-mono text-[var(--color-text-secondary)]">{booking.reference || booking.id}</span>
                </div>
            </div>
            ...
            <div className="px-4 pb-10 sm:px-0">
                <Accordion allowMultiple={false}>
                    ...
                    <AccordionItem title="Costing" badge={`${booking.total.currency} ${booking.total.amount.toFixed(0)}`}>
                        <CostingSection booking={booking} />
                    </AccordionItem>

                    <AccordionItem title="Payments" badge={booking.paymentStatus || 'Status'}>
