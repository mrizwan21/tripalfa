...
function StatusPill({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`px-3 py-1 ${config.bg} ${config.text} border ${config.border} rounded-full text-[9px] font-black uppercase tracking-widest whitespace-nowrap mb-auto`}
    >
      {s}
    </span>
  );
  ...
  if (!items || items.length === 0) {
    return (
      <div className="py-20 text-center space-y-4">
        <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto gap-2">
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
      {items.map((b, index) => (
        <div
          ...
          className="group relative bg-white rounded-3xl border border-gray-100 p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 overflow-hidden flex flex-col"
        >
          ...
          <div className="relative z-10 flex flex-col flex-1 h-full gap-4">
            <div className="flex items-start justify-between gap-4 mb-6">
              <div className="space-y-1.5 min-w-0">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-6 h-6 rounded-lg ${b.product === "flight" ? "bg-blue-50 text-blue-500" : "bg-violet-50 text-violet-500"} flex items-center justify-center`}
                  >
                    {b.product === "flight" ? (
            ...
            <div className="pt-6 border-t border-dashed border-gray-100 flex items-center justify-between gap-2">
              <div className="flex items-center gap-3 min-w-0 overflow-hidden">
                <div className="w-8 h-8 shrink-0 rounded-full border-2 border-white shadow-sm overflow-hidden bg-primary-50 flex items-center justify-center text-[10px] font-black text-primary-600 transition-transform group-hover:scale-110 gap-2">
                  {(
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Link
                  ...
                  className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-primary-600 hover:text-white transition-all shadow-sm group/btn active:scale-95 gap-2"
                  title="View Details"
                >
                  <ArrowRight
                    size={14}
