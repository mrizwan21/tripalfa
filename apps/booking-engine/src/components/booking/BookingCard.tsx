...
export function BookingCard({ booking }: { booking: Booking }) {
  return (
    <div className="border rounded p-4 flex justify-between gap-4">
      <div>
        <div className="text-sm text-muted-foreground">{booking.product.toUpperCase()}</div>
      ...
      <div className="text-right">
        ...
        <Link to={`/booking/${booking.id}`} className="inline-block mt-2 text-sm text-blue-600">
          View
        </Link>
      </div>
    </div>
