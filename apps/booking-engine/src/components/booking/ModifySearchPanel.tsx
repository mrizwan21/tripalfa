...
export function ModifySearchPanel() {
    ...
    const [departureDate, setDepartureDate] = useState<Date | null>(
        safeParseDate(searchParams.get('date')) || safeParseDate(searchParams.get('departureDate'))
    );
    const [returnDate, setReturnDate] = useState<Date | null>(
        safeParseDate(searchParams.get('returnDate'))
    );

    const [adults, setAdults] = useState(searchParams.get('adults') || searchParams.get('travelers') || '1');
