...
export const DocumentsSection: React.FC<DocumentsSectionProps> = ({ booking, onTicketIssued }) => {
    const [isIssueModalOpen, setIsIssueModalOpen] = useState(false);
    const [isOfflineRequestOpen, setIsOfflineRequestOpen] = useState(false);

    const isIssued = booking.status === 'Issued' || booking.status === 'Ticketed' || booking.status === 'Vouchered';
            ...
            <Card className="bg-gradient-to-r from-indigo-50 to-white border-indigo-100">
                <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                        <div className={`p-3 rounded-full ${isIssued ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                            {isIssued ? <CheckCircle2 className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
                        </div>
                        <div>
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        {!isIssued ? (
                            <Button
                                ...
                                className="flex-1 md:flex-none bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-200"
                            >
                            ...
                            <Button
                                ...
                                className="flex-1 md:flex-none border-gray-300 hover:border-gray-400 text-gray-700"
                            >
            ...
            <Card>
                ...
                <CardContent className="space-y-1">
                    {documents.map((doc) => (
                        <div
                            key={doc.id}
                            className={`flex items-center justify-between p-4 rounded-lg border transition-all ${doc.available
                                ? 'border-gray-100 hover:border-gray-200 hover:bg-gray-50'
                                : 'border-transparent opacity-50 bg-gray-50'
                                }`}
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 flex items-center gap-2">
                                        <span className="bg-gray-100 px-1.5 py-0.5 rounded text-[10px] font-medium">{doc.type}</span>
                                        {doc.available ? `Generated on ${new Date(doc.date).toLocaleDateString()}` : 'Not generated yet'}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                {doc.available ? (
                                    <>
                                        ...
                                        <Button size="sm" variant="outline" className="h-8 gap-2 text-xs border-gray-200">
                                            <Download className="w-3.5 h-3.5" />
                                            Download
                                        </Button>
                                    </>
                                ) : (
                                    <span className="text-xs text-gray-400 italic px-3">Pending Issuance</span>
                                )}
                            </div>
                        </div>
                    ))}
