...
export class BookingManagementController {
    ...
    try {
      ...
      if (bookingData.customerInfo.type === "individual") {
        ...
        customer = await prisma.company.findUnique({
          where: {
            name: bookingData.customerInfo.companyRegistrationNumber,
          } as any,
        });
      }
      ...
      const booking = await prisma.booking.create({
        data: {
          ...
          status: bookingData.bookingOptions?.hold
            ? BookingStatus.HOLD
          ...
          assignedAgent: bookingData.bookingOptions?.hold
            ? null
      ...
      // Record metrics
      (metricsStore as any).recordBookingCreated(
        booking.serviceType,
        booking.status,
      );
      ...
      res.status(201).json({
        ...
        data: {
          booking,
          message: bookingData.bookingOptions?.hold
  ...
  async cancelOrder(req: Request, res: Response): Promise<Response | void> {
    ...
    try {
      ...
      const { duffelOrderCancellationService } =
        await import("../services/duffelOrderCancellationService");
      ...
      const cancellationResult =
        await duffelOrderCancellationService.cancelOrder({
          orderId: duffelOrderId,
  ...
  async getCancellationStatus(
    ...
    try {
      ...
      const status =
        await duffelOrderCancellationService.getCancellationStatus(
          duffelOrderId,
        );
  ...
  async getAvailableAirlineCredits(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const creditsWithWarnings = credits.map((credit: any) => ({
        ...credit,
        expiresIn: Math.ceil(
          (credit.expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
        ),
        expiringSoon:
          credit.expiresAt.getTime() - now.getTime() < 7 * 24 * 60 * 60 * 1000, // Less than 7 days
      }));
      ...
      res.status(200).json({
        ...
        data: {
          ...
          summary: {
            ...
            expiringInNext7Days: creditsWithWarnings.filter(
              (c: any) => c.expiresIn <= 7,
            ).length,
          },
        },
  ...
  async searchSuppliers(req: Request, res: Response): Promise<Response | void> {
    ...
    try {
      ...
      if (
        ...
        Array.isArray(searchParams.serviceTypes) &&
  ...
  async searchInventory(req: Request, res: Response): Promise<Response | void> {
    ...
    try {
      ...
      const limit = Math.max(
        1,
        Math.min(100, parseInt(String(searchParams.limit || 10), 10)),
      );
      const offset = (page - 1) * limit;
      ...
      if (
        searchParams.minAvailable !== undefined &&
        searchParams.minAvailable !== ""
      ) {
      ...
      if (searchParams.serviceTypes) {
        const serviceTypesArray = Array.isArray(searchParams.serviceTypes)
          ? searchParams.serviceTypes
          : [searchParams.serviceTypes];
  ...
  async createInventory(req: Request, res: Response): Promise<Response | void> {
    ...
    try {
      ...
      // Cache the inventory
      await cacheService.set(cacheKeys.inventory(inventory.id), inventory, 600);
  ...
  async checkAvailability(
    ...
    try {
      ...
      const canReserve =
        inventory.available >= quantity && inventory.status === "active";
  ...
  async getAuditLog(req: Request, res: Response): Promise<Response | void> {
    ...
    try {
      ...
      if (
        ...
        String(auditParams.resourceType).length > 0
      ) {
  ...
  async getPaymentOptions(
    ...
    try {
      ...
      const bookingPaymentService = (
        await import("../services/bookingPaymentService")
      ).default;
      const paymentOptions =
        await bookingPaymentService.getAvailablePaymentOptions(
          customerId,
  ...
  async processCombinedPayment(
    ...
    try {
      ...
      const paymentBreakdown =
        await bookingPaymentService.processCombinedPayment({
   ...
   */
  async getBookingPaymentDetails(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const paymentDetails =
        await bookingPaymentService.getBookingPaymentDetails(bookingId);
   ...
   */
  async createBookingWithCombinedPayment(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown error";
      logger.error("Error creating booking with combined payment", {
        error: errorMsg,
      });
      return res.status(400).json({
        success: false,
   ...
   */
  async checkOrderChangeEligibility(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const { changeable, changeableSlices } =
        await duffelClient.isOrderChangeable(orderId);
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown error";
      logger.error("Error checking order change eligibility", {
        error: errorMsg,
      });
      return res.status(400).json({
        success: false,
  ...
  async checkBaggageEligibility(
    ...
    try {
      ...
      const availableBaggages =
        await duffelClient.getAvailableBaggages(orderId);
      ...
      return res.status(200).json({
        ...
        message: eligible
          ? "Baggage services available for this order"
  ...
  async bookBaggageServices(
    ...
    try {
      ...
      const result = await duffelClient.addServicesToOrder(
        orderId,
        baggages,
        payment,
      );
      ...
      const bookedBaggages = bookedServices.filter(
    ...
    try {
      ...
      if (!orderId || !slices || !slices.remove || !slices.add) {
        return res.status(400).json({
          success: false,
          error:
            "orderId, slices.remove array, and slices.add array are required",
        });
      }
      ...
      for (const addSlice of slices.add) {
        if (
          !addSlice.origin ||
          !addSlice.destination ||
          !addSlice.departure_date ||
          !addSlice.cabin_class
        ) {
          return res.status(400).json({
            success: false,
            error:
              "Each slice in add array must have origin, destination, departure_date, and cabin_class",
          });
        }
      }

      const { duffelClient } = await import("../integrations/duffelApiClient");
      const changeRequest = await duffelClient.createOrderChangeRequest(
        orderId,
        slices,
      );
  ...
  async getOrderChangeOffers(
    ...
    try {
      ...
      const { duffelClient } = await import("../integrations/duffelApiClient");
      const changeRequest =
        await duffelClient.getOrderChangeRequest(orderChangeRequestId);
   ...
   */
  async createPendingOrderChange(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const { duffelClient } = await import("../integrations/duffelApiClient");
      const pendingChange =
        await duffelClient.createPendingOrderChange(orderChangeOfferId);
      ...
      return res.status(201).json({
        ...
        message:
          "Pending order change created successfully. Review pricing before confirming.",
      });
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Unknown error";
   ...
   */
  async getPendingOrderChangeStatus(
    req: Request,
    res: Response,
  ): Promise<Response | void> {
    const typedReq = req as TypedRequest;
    try {
      ...
      const { duffelClient } = await import("../integrations/duffelApiClient");
      const orderChange =
        await duffelClient.getPendingOrderChange(orderChangeId);
      ...
      return res.status(200).json({
        ...
        message: orderChange.confirmed_at
          ? "Order change confirmed"
          : "Order change is pending",
      });
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Unknown error";
      logger.error("Error retrieving pending order change status", {
        error: errorMsg,
      });
      return res.status(400).json({
        success: false,
  ...
  async confirmOrderChange(
    ...
    try {
      ...
      const { duffelClient } = await import("../integrations/duffelApiClient");
      const confirmedChange = await duffelClient.confirmOrderChange(
        orderChangeId,
        payment,
      );
...
export const boundBookingManagementController = {
  createBooking: bookingManagementController.createBooking.bind(
    bookingManagementController,
  ),
  searchBookings: bookingManagementController.searchBookings.bind(
    bookingManagementController,
  ),
  searchCustomers: bookingManagementController.searchCustomers.bind(
    bookingManagementController,
  ),
  createCustomer: bookingManagementController.createCustomer.bind(
    bookingManagementController,
  ),
  searchSuppliers: bookingManagementController.searchSuppliers.bind(
    bookingManagementController,
  ),
  createSupplier: bookingManagementController.createSupplier.bind(
    bookingManagementController,
  ),
  getBookingDetails: bookingManagementController.getBookingDetails.bind(
    bookingManagementController,
  ),
  updateBooking: bookingManagementController.updateBooking.bind(
    bookingManagementController,
  ),
  cancelBooking: bookingManagementController.cancelBooking.bind(
    bookingManagementController,
  ),
  confirmBooking: bookingManagementController.confirmBooking.bind(
    bookingManagementController,
  ),
  assignBooking: bookingManagementController.assignBooking.bind(
    bookingManagementController,
  ),
  holdInventory: bookingManagementController.holdInventory.bind(
    bookingManagementController,
  ),
  releaseInventory: bookingManagementController.releaseInventory.bind(
    bookingManagementController,
  ),
  deleteInventory: bookingManagementController.deleteInventory.bind(
    bookingManagementController,
  ),
  issueTicket: bookingManagementController.issueTicket.bind(
    bookingManagementController,
  ),
  updateWorkflowStatus: bookingManagementController.updateWorkflowStatus.bind(
    bookingManagementController,
  ),
  updatePriority: bookingManagementController.updatePriority.bind(
    bookingManagementController,
  ),
  getInventory: bookingManagementController.getInventory.bind(
    bookingManagementController,
  ),
  updateInventory: bookingManagementController.updateInventory.bind(
    bookingManagementController,
  ),
  searchInventory: bookingManagementController.searchInventory.bind(
    bookingManagementController,
  ),
  createInventory: bookingManagementController.createInventory.bind(
    bookingManagementController,
  ),
  checkAvailability: bookingManagementController.checkAvailability.bind(
    bookingManagementController,
  ),
  getPricingRules: bookingManagementController.getPricingRules.bind(
    bookingManagementController,
  ),
  createPricingRule: bookingManagementController.createPricingRule.bind(
    bookingManagementController,
  ),
  updatePricingRule: bookingManagementController.updatePricingRule.bind(
    bookingManagementController,
  ),
  deletePricingRule: bookingManagementController.deletePricingRule.bind(
    bookingManagementController,
  ),
  getCommissionRules: bookingManagementController.getCommissionRules.bind(
    bookingManagementController,
  ),
  createCommissionRule: bookingManagementController.createCommissionRule.bind(
    bookingManagementController,
  ),
  updateCommissionRule: bookingManagementController.updateCommissionRule.bind(
    bookingManagementController,
  ),
  deleteCommissionRule: bookingManagementController.deleteCommissionRule.bind(
    bookingManagementController,
  ),
  getBookingReport: bookingManagementController.getBookingReport.bind(
    bookingManagementController,
  ),
  getCommissionReport: bookingManagementController.getCommissionReport.bind(
    bookingManagementController,
  ),
  getInventoryReport: bookingManagementController.getInventoryReport.bind(
    bookingManagementController,
  ),
  getAuditLog: bookingManagementController.getAuditLog.bind(
    bookingManagementController,
  ),
  getBookingHistory: bookingManagementController.getBookingHistory.bind(
    bookingManagementController,
  ),
  generateReport: bookingManagementController.generateReport.bind(
    bookingManagementController,
  ),
  getDashboardStats: bookingManagementController.getDashboardStats.bind(
    bookingManagementController,
  ),
  bulkUpdateBookings: bookingManagementController.bulkUpdateBookings.bind(
    bookingManagementController,
  ),
  getComplianceReport: bookingManagementController.getComplianceReport.bind(
    bookingManagementController,
  ),
  cancelOrder: bookingManagementController.cancelOrder.bind(
    bookingManagementController,
  ),
  getCancellationStatus: bookingManagementController.getCancellationStatus.bind(
    bookingManagementController,
  ),
  getAvailableAirlineCredits:
    bookingManagementController.getAvailableAirlineCredits.bind(
      bookingManagementController,
    ),
  getPaymentOptions: bookingManagementController.getPaymentOptions.bind(
    bookingManagementController,
  ),
  processCombinedPayment:
    bookingManagementController.processCombinedPayment.bind(
      bookingManagementController,
    ),
  getBookingPaymentDetails:
    bookingManagementController.getBookingPaymentDetails.bind(
      bookingManagementController,
    ),
  refundCombinedPayment: bookingManagementController.refundCombinedPayment.bind(
    bookingManagementController,
  ),
  createBookingWithCombinedPayment:
    bookingManagementController.createBookingWithCombinedPayment.bind(
      bookingManagementController,
    ),
  applyCreditsToBooking: bookingManagementController.applyCreditsToBooking.bind(
    bookingManagementController,
  ),
  checkOrderChangeEligibility:
    bookingManagementController.checkOrderChangeEligibility.bind(
      bookingManagementController,
    ),
  createOrderChangeRequest:
    bookingManagementController.createOrderChangeRequest.bind(
      bookingManagementController,
    ),
  getOrderChangeOffers: bookingManagementController.getOrderChangeOffers.bind(
    bookingManagementController,
  ),
  createPendingOrderChange:
    bookingManagementController.createPendingOrderChange.bind(
      bookingManagementController,
    ),
  getPendingOrderChangeStatus:
    bookingManagementController.getPendingOrderChangeStatus.bind(
      bookingManagementController,
    ),
  confirmOrderChange: bookingManagementController.confirmOrderChange.bind(
    bookingManagementController,
  ),
  checkBaggageEligibility:
    bookingManagementController.checkBaggageEligibility.bind(
      bookingManagementController,
    ),
  getAvailableBaggages: bookingManagementController.getAvailableBaggages.bind(
    bookingManagementController,
  ),
  bookBaggageServices: bookingManagementController.bookBaggageServices.bind(
    bookingManagementController,
  ),
  getOrderBaggages: bookingManagementController.getOrderBaggages.bind(
    bookingManagementController,
  ),
};
