...
const GATEWAY_URL =
  import.meta.env.VITE_API_GATEWAY_URL || "http://localhost:3001/api";

// Warn in development if using fallback
if (!import.meta.env.VITE_API_GATEWAY_URL) {
  console.warn(
    "[HoldOrdersService] Using fallback URL. Set VITE_API_GATEWAY_URL for production.",
  );
}
...
class HoldOrdersService {
  ...
  async checkHoldEligibility(
    ...
    if (type === "hotel") {
      ...
      return {
        ...
        priceGuaranteeExpiresAt: new Date(
          Date.now() + 24 * 60 * 60 * 1000,
        ).toISOString(), // 24h guarantee
        paymentRequiredBy: new Date(
          Date.now() + 12 * 60 * 60 * 1000,
        ).toISOString(), // Pay in 12h
    ...
    try {
      ...
      throw new Error(
        `Failed to check hold eligibility: ${axiosError.message}`,
      );
    }
  }
  ...
  async createHoldOrder(
    data: HoldOrderData & { type?: "flight" | "hotel" },
  ): Promise<HoldOrder> {
    ...
    try {
      ...
      const eligibility = await this.checkHoldEligibility(
        data.offerId,
        "flight",
      );
      ...
      if (!eligibility.eligible) {
        throw new Error(
          `Flight offer ${data.offerId} is not eligible for hold orders. Instant payment is required or it is non-refundable.`,
        );
      }
      ...
      const isRefundable =
        duffelOrder.conditions?.refund_before_departure?.allowed === true;
      if (!isRefundable) {
        ...
        throw new Error(
          "This flight fare is non-refundable and cannot be held.",
        );
      }
  ...
  async checkScheduleChange(
    ...
    try {
      ...
      return {
        changed,
        details: changed
          ? "Flight schedule has changed since order creation"
          : "Schedule unchanged",
      };
    } catch (error) {
      const axiosError = error as AxiosError;
  ...
  async payForHoldOrder(paymentData: PaymentData): Promise<{
    ...
    try {
      ...
      const orderType =
        ...
        (paymentData.orderId.startsWith("LA-") ? "liteapi" : "duffel");
      ...
      if (orderType === "duffel") {
        ...
        return {
          ...
          message:
            "Flight payment successfully processed and documents generated",
        };
      }
      ...
      if (orderType === "liteapi") {
        ...
        const mockBooking: any = {
          ...
          pricing: {
            customerPrice: paymentData.amount,
            currency: paymentData.currency,
          },
        };
      ...
      if (axiosError.response?.status === 409) {
        ...
        if (errorData.errors?.[0]?.code === "schedule_changed") {
          throw new Error(
            "SCHEDULE_CHANGED: Flight schedule has changed. Please review before paying.",
          );
        }
        if (errorData.errors?.[0]?.code === "price_changed") {
          throw new Error(
            "PRICE_CHANGED: Price has changed. Please verify before paying.",
          );
        }
      }
  ...
  async getAvailableServices(orderId: string): Promise<any[]> {
    try {
      return await this.callDuffelAdapter<any[]>("availableServices", {
        orderId,
      });
    } catch (error) {
      const axiosError = error as AxiosError;
      throw new Error(
        `Failed to retrieve available services: ${axiosError.message}`,
      );
    }
  }
   ...
   */
  async addServiceToHoldOrder(
    orderId: string,
    serviceId: string,
  ): Promise<any> {
    try {
      return await this.callDuffelAdapter<any>("addService", {
        orderId,
        serviceId,
      });
    } catch (error) {
  ...
  generateHoldOrderSummary(holdOrder: HoldOrder): string {
    const paymentDeadline = holdOrder.paymentRequiredBy
      .toISOString()
      .split("T")[0];
   ...
   */
  private async sendHoldNotificationEmail(
    holdOrder: HoldOrder,
    data: HoldOrderData,
  ): Promise<void> {
    try {
      ...
      await sendEmail({
        ...
        htmlContent: `
          <div style="font-family: Arial, sans-serif; color: ${COLORS.slate[700]}; padding: 20px; border: 1px solid ${COLORS.slate[200]}; border-radius: 10px;">
            <h2 style="color: ${COLORS.primary};">TripAlfa Reservation</h2>
            <p>Hello,</p>
            ...
            <p style="background: rgba(245, 158, 11, 0.12); padding: 15px; border-radius: 8px; border-left: 4px solid ${COLORS.warning};">
