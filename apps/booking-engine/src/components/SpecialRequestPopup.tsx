...
export const SpecialRequestPopup = ({
  isOpen,
  onClose,
  onConfirm,
}: SpecialRequestPopupProps) => {
  const [selectedPassenger, setSelectedPassenger] = useState(1);

  const ssrOptions = [
    {
      id: "WCHR",
      name: "Wheelchair (WCHR)",
      description:
        "For passengers who can climb stairs but need assistance for long distances",
      icon: <Heart size={20} />,
    },
    {
      id: "BSCT",
      name: "Bascinet (BSCT)",
      description:
        "Infant cot request (subject to availability for infants < 2 years)",
      icon: <Info size={20} />,
    },
    {
      id: "EXST",
      name: "Extra Legroom (RQ)",
      description: "Request for preferred seating (Pending Airline Approval)",
      icon: <Plus size={20} />,
    },
    {
      id: "BLND",
      name: "Blind Passenger (BLND)",
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {ssrOptions.map((opt) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isSelected ? "bg-primary text-primary-foreground shadow-lg" : "bg-muted/50 text-muted-foreground"}`}
                  >
                    {opt.icon}
                  </div>
                  ...
                  <div className="text-right">
                    {isSelected && (
                      <div className="flex flex-col items-center gap-1">
                        <div className="w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center gap-2">
                          <Check size={14} />
                        </div>
          ...
          <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100/50 flex items-start gap-4">
            ...
            <p className="text-[10px] font-bold text-blue-600/80 uppercase tracking-wide leading-relaxed">
              Note: Most special requests are marked as **RQ (On Request)**. Our
              travel experts will coordinate with the airline and update the
              status in your booking notification queue once confirmed.
            </p>
          </div>
        </div>

        <div className="p-8 bg-foreground flex items-center justify-between gap-2">
          <div>
            <p className="text-[10px] font-black text-background/40 uppercase tracking-widest">
              Selected Requests
            </p>
            <p className="text-xl font-black text-background">
              {Object.values(requests).flat().length} Services
            </p>
          </div>
          <Button
            onClick={() => onConfirm(requests as any)}
            className="bg-primary hover:bg-accent text-primary-foreground px-12 h-14 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all hover:-translate-y-1"
          >
            Send Requests
          </Button>
        </div>
