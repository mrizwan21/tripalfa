...
const getEventIcon = (type: HistoryEvent['type']) => {
    switch (type) {
        ...
        case 'modification': return <User className="w-4 h-4" />;
...
const getEventColor = (type: HistoryEvent['type']) => {
    switch (type) {
        ...
        case 'modification': return 'bg-blue-100 text-blue-600 border-blue-200';
        ...
        case 'login': return 'bg-yellow-100 text-yellow-600 border-yellow-200';
        case 'system':
        default: return 'bg-gray-100 text-gray-600 border-gray-200';
    }
};

export const BookingHistoryTimeline: React.FC<BookingHistoryTimelineProps> = ({ events, className }) => {
    if (!events.length) {
        return <div className="text-center py-8 text-[var(--color-text-tertiary)]">No history events found.</div>;
    }

    return (
        <div className={cn("space-y-8 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-[var(--color-border-light)] hover:before:bg-[var(--color-border-hover)] before:transition-colors", className)}>
                ...
                <div key={event.id || index} className="relative pl-12 group">
                    ...
                    <div className="bg-white p-4 rounded-lg border border-[var(--color-border-light)] shadow-sm hover:shadow-md transition-all">
                        <div className="flex justify-between items-start mb-1">
                            <h4 className="text-sm font-semibold text-[var(--color-text-primary)]">{event.action}</h4>
                        ...
                        {event.user && (
                            <div className="flex items-center gap-2 text-xs text-[var(--color-text-tertiary)] border-t border-[var(--color-border-light)] pt-2 mt-2">
                                ...
                                <span>Action by: <span className="font-medium text-[var(--color-text-primary)]">{event.user}</span></span>
                            </div>
                        )}
                    </div>
                </div>
