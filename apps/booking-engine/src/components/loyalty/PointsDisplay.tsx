  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (animatingPoints < currentPoints) {
        animatingPoints = Math.min(animatingPoints + pointsPerFrame, currentPoints);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (animatingEarning < pointsToEarn) {
        animatingEarning = Math.min(animatingEarning + earningPerFrame, pointsToEarn);
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              {showLabel && <p className={`${labelSizeClasses[size]} text-gray-600 font-medium`}>
                Current Balance
              </p>}
              <p className={`${sizeClasses[size]} font-black text-blue-600 mt-1`}>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`${sizeClasses[size]} font-black text-green-600 mt-1`}>
                  +{displayedEarning.toLocaleString()}
                </p>
              </div>
              <div className="h-10 w-10 rounded-full bg-green-500 text-white flex items-center justify-center font-bold text-sm">
                +
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-xl border border-amber-200">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className={`${sizeClasses[size]} font-black text-amber-600 mt-1`}>
