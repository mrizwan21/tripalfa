...
export class SeatMapsService {
  ...
  async getSeatMapsForBooking(
    ...
    try {
      logger.info(
        `[SeatMaps] Booking flow - Fetching seat maps for offer: ${offerId}`,
      );
      ...
      logger.info(
        `[SeatMaps] Success - Found ${transformedSeatMaps.length} segment(s)`,
      );
  ...
  async getSeatMapsForOrder(
    ...
    try {
      logger.info(
        `[SeatMaps] Post-booking flow - Fetching seat maps for order: ${orderId}`,
      );
      ...
      const seatMapData = await this.duffelClient.getSeatMapForOffer(
        orderData.offer_id,
      );
      ...
      logger.info(
        `[SeatMaps] Success - Found ${currentSeats.length} passenger(s) with seats`,
      );
      ...
      return {
        ...
        error: {
          code: "INTERNAL_ERROR",
          message:
            "An unexpected error occurred while fetching order seat maps",
          details: {
            hint: "Please try again later",
          },
        },
  ...
  private getAircraftConfig(aircraftType: string): AircraftConfig {
    ...
    return configs[aircraftType.toLowerCase()] || configs["airbus-320"];
  }
}
