...
import { X, ChevronLeft, ChevronRight, Utensils, Plane, Check } from 'lucide-react';
...
export const MealSelectionPopup = ({ 
    ...
    const [selectedMeals, setSelectedMeals] = useState<SelectedMeal[]>(existingSelections);

    // Use provided meals or defaults based on carrier type
    const mealOptions: MealOption[] = availableMeals && availableMeals.length > 0 
        ? availableMeals 
        : isLCC ? DEFAULT_LCC_MEALS : DEFAULT_FSC_MEALS;
    ...
    const currentMeal = selectedMeals.find(
        m => m.passengerId === currentPassenger?.id && m.segmentId === currentSegment?.id
    );
    ...
    const handleRemoveMeal = () => {
        if (!currentPassenger || !currentSegment) return;
        setSelectedMeals(prev => prev.filter(
            m => !(m.passengerId === currentPassenger.id && m.segmentId === currentSegment.id)
        ));
    };
    ...
    const getMealTypeStyle = (type: MealOption['type']) => {
        switch (type) {
            case 'vegetarian': return { bg: 'bg-green-100', text: 'text-green-600', label: 'Veg' };
            case 'vegan': return { bg: 'bg-emerald-100', text: 'text-emerald-600', label: 'Vegan' };
            case 'non-vegetarian': return { bg: 'bg-red-100', text: 'text-red-600', label: 'Non-Veg' };
            case 'special': return { bg: 'bg-purple-100', text: 'text-purple-600', label: 'Special' };
        ...
        return (
            <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-12 flex flex-col items-center gap-8 animate-in zoom-in-95 duration-300">
                    ...
                    <h2 className="text-xl font-black text-gray-900 text-center leading-relaxed">Confirm Meal Selections</h2>
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-5xl rounded-[3rem] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-500 border border-gray-100 flex flex-col max-h-[95vh]" data-testid="meal-selection-modal">
                ...
                <div className="p-8 text-center relative border-b border-gray-50">
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                        {isLCC ? 'Pre-order meals for your journey' : 'Select your preferred meal options'}
                    </p>
                ...
                <div className="flex-1 overflow-y-auto p-12 space-y-12">
                            ...
                            {mealOptions.map((meal) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-wide mb-1">{meal.name}</h3>
                                        {meal.description && (
                                            <p className="text-[9px] font-medium text-gray-400 mb-3 line-clamp-2">{meal.description}</p>
                                        )}
                                        
                                        <div className="flex items-center justify-between">
                                            {typeStyle.label && (
                                                <span className={`px-2 py-1 rounded-full text-[8px] font-black uppercase ${typeStyle.bg} ${typeStyle.text}`}>
                                                    {typeStyle.label}
                                                </span>
                                            )}
                    ...
                    {selectedMeals.length > 0 && (
                        <div className="bg-gray-50/50 p-6 rounded-[2rem] border border-gray-100">
                            <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-widest mb-4">All Meal Selections</h3>
