...
import { X, ChevronLeft, ChevronRight, User, Check, Users, Eye, Plane } from 'lucide-react';
...
export const SpecialServicesPopup = ({ 
    ...
    const [selectedServices, setSelectedServices] = useState<SelectedSpecialService[]>(existingSelections);
    const [notes, setNotes] = useState<Record<string, string>>({});

    // Use provided services or defaults
    const ssrOptions: SpecialServiceOption[] = availableServices && availableServices.length > 0 
        ? availableServices 
        : DEFAULT_SSR_OPTIONS;
    ...
    const handleServiceToggle = (service: SpecialServiceOption) => {
        ...
        if (isServiceSelected(service.id)) {
            ...
            setSelectedServices(prev => prev.filter(
                s => !(s.passengerId === currentPassenger.id && 
    ...
    const handleNotesChange = (serviceId: string, value: string) => {
        ...
        setSelectedServices(prev => prev.map(s => {
            if (s.passengerId === currentPassenger.id && 
                s.segmentId === currentSegment.id && 
        ...
        return (
            <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-12 flex flex-col items-center gap-8 animate-in zoom-in-95 duration-300">
                    ...
                    <h2 className="text-xl font-black text-gray-900 text-center leading-relaxed">Confirm Special Services</h2>
                    ...
                    <div className="w-full space-y-3 max-h-48 overflow-y-auto">
                        {selectedServices.map((service, i) => (
                            <div key={i} className="bg-gray-50 rounded-xl px-4 py-3">
                                ...
                                {service.notes && (
                                    <p className="text-[8px] text-gray-500 mt-2 italic">"{service.notes}"</p>
                                )}
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-5xl rounded-[3rem] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-500 border border-gray-100 flex flex-col max-h-[95vh]" data-testid="special-services-modal">

                {/* Header */}
                <div className="p-8 text-center relative border-b border-gray-50">
                    <h2 className="text-2xl font-black text-gray-900">Special Services</h2>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Request special assistance for your journey</p>
                ...
                <div className="flex-1 overflow-y-auto p-12 space-y-12">
                    ...
                    <div className="bg-gray-100/50 p-10 rounded-[3rem] space-y-8">
                        ...
                        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-center">
                            <p className="text-[10px] text-blue-700 font-medium">
                                Special services are subject to availability and airline confirmation. 
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-wide">{service.name}</h3>
                    ...
                    {selectedServices.length > 0 && (
                        <div className="bg-gray-50/50 p-6 rounded-[2rem] border border-gray-100">
                            <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-widest mb-4">All Service Requests</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {selectedServices.map((service, i) => (
                                    <div key={i} className="bg-white rounded-xl p-3 border border-gray-100">
                                        ...
                                        {service.notes && (
                                            <p className="text-[8px] text-gray-500 mt-1 italic truncate">"{service.notes}"</p>
                                        )}
                                    </div>
                                ))}
                            </div>
