...
const TRANSLATION_API_CONFIG = {
  ...
  MICROSOFT_TRANSLATOR_API: 'https://api.cognitive.microsofttranslator.com/translate',
 ...
 */
async function translateTextBatch(texts: Record<string, string>, targetLang: string): Promise<Record<string, string>> {
  // Option 1: Use LibreTranslate (free)
  try {
    const libreTranslateResponse = await fetch(TRANSLATION_API_CONFIG.LIBRE_TRANSLATE_API, {
      method: 'POST',
      headers: {
    ...
    if (libreTranslateResponse.ok) {
      ...
      const translatedTexts = Object.keys(texts).reduce((acc, key, index) => {
  ...
  try {
    const googleApiKey = (globalThis as any).process?.env?.VITE_GOOGLE_TRANSLATE_API_KEY || '';
    if (googleApiKey) {
      const response = await fetch(`${TRANSLATION_API_CONFIG.GOOGLE_TRANSLATE_API}?key=${googleApiKey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
function mockArabicTranslation(text: string): string {
  const arabicMap: Record<string, string> = {
    ...
    '© 2024 Travel Kingdom. All rights reserved.': '© 2024 مملكة السفر. جميع الحقوق محفوظة.',
    'About Us': 'من نحن',
    'Contact': 'اتصل بنا',
...
function mockFrenchTranslation(text: string): string {
  const frenchMap: Record<string, string> = {
    ...
    '© 2024 Travel Kingdom. All rights reserved.': '© 2024 Royaume du Voyage. Tous droits réservés.',
    'About Us': 'À Propos',
    'Contact': 'Contact',
...
function mockSpanishTranslation(text: string): string {
  const spanishMap: Record<string, string> = {
    ...
    '© 2024 Travel Kingdom. All rights reserved.': '© 2024 Reino de Viajes. Todos los derechos reservados.',
...
export function useTranslation() {
  const [translations, setTranslations] = React.useState<Record<string, string>>(DEFAULT_TRANSLATIONS);
  const [isLoading, setIsLoading] = React.useState(false);
  const [currentLang, setCurrentLangState] = React.useState(getCurrentLanguage());
