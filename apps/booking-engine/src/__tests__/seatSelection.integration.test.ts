...
export const generateMockAircraftConfig = (
...
export const generateMockBookingFlowResponse = (
  aircraftType: "narrow-body" | "wide-body" = "narrow-body",
) => {
  return {
    success: true,
...
export const generateMockPostBookingResponse = (
  aircraftType: "narrow-body" | "wide-body" = "narrow-body",
) => {
  return {
    success: true,
...
describe("SeatSelection Component - Integration Tests", () => {
  describe("Booking Flow Mode", () => {
    ...
    it("should mark unavailable seats correctly", () => {
      ...
      const firstRowSeatA =
        mockSeatMap.cabin.rows[0].issuable_seat_designators[0];
      expect(firstRowSeatA.available).toBe(false);
      expect(firstRowSeatA.restrictions?.code).toBe("OCCUPIED");

      const secondRowSeatB =
        mockSeatMap.cabin.rows[1].issuable_seat_designators[1];
      expect(secondRowSeatB.available).toBe(false);

      const firstRowSeatB =
        mockSeatMap.cabin.rows[0].issuable_seat_designators[1];
  ...
  describe("Post-Booking Mode", () => {
    ...
    it("should mark currently booked seats as unavailable", () => {
      ...
      const seatRow5 = seat_maps[0].cabin.rows.find(
        (r) => r.designator === "5",
      );
  ...
  describe("Seat Selection State Management", () => {
    ...
    it("should prevent double-booking of same seat", () => {
      ...
      const isDuplicateSeat = selectedSeats.some(
        (s) => s.designator === "1A" && s.passengerId !== "pass_002",
      );
    ...
    it("should allow seat changes for same passenger", () => {
      ...
      const index = selectedSeats.findIndex(
        (s) => s.passengerId === "pass_001" && s.segmentId === "seg_001",
      );
      if (index >= 0) {
        selectedSeats[index] = {
          designator: "1B",
  ...
  describe("API Contract Validation", () => {
    ...
    it("should match response schema types", () => {
      ...
      expect(typeof response.data.aircraft_config.fuselage_width).toBe(
        "string",
      );
  ...
  describe("Performance Tests", () => {
    ...
    it("should handle multiple segment seat maps", () => {
      ...
      const totalSeats = response.data.seat_maps.reduce((sum, map) => {
        return (
          sum +
          map.cabin.rows.reduce((rowSum, row) => {
  ...
  try {
    ...
    for (const map of seat_maps) {
      ...
      for (const row of map.cabin.rows) {
        if (!row.designator || !Array.isArray(row.issuable_seat_designators))
          return false;
      }
    }

    if (!aircraft_config || !aircraft_config.type) return false;
    if (!["narrow-body", "wide-body"].includes(aircraft_config.fuselage_width))
      return false;
...
export const validatePostBookingResponse = (response: any): boolean => {
  ...
  for (const seat of current_seats) {
    if (!seat.segment_id || !seat.passenger_id || !seat.current_seat)
      return false;
