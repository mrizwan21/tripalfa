  ...
  describe("Type Guard Functions", () => {
    ...
    it("should accurately count notifications by status", () => {
      const statuses: (
        ...
        | "INFO"
        | "CANCELLED"
      )[] = ["PENDING", "CONFIRMED", "REJECTED", "INFO", "CANCELLED"];

      statuses.forEach((status) => {
        const count = getNotificationCountByStatus(
          MOCK_NOTIFICATION_LIST,
          status,
        );
        const expectedCount = MOCK_NOTIFICATION_LIST.filter(
          (n) => n.status === status,
        ).length;
    ...
    it("should correctly search notifications", () => {
      // Search by title
      const titleSearch = searchNotifications(
        MOCK_NOTIFICATION_LIST,
        "Booking",
      );
    ...
    it("should maintain data consistency across all notifications", () => {
      MOCK_NOTIFICATION_LIST.forEach((notif) => {
        // IDs must be unique
        const idCount = MOCK_NOTIFICATION_LIST.filter(
          (n) => n.id === notif.id,
        ).length;
    ...
    it("should handle edge cases gracefully", () => {
      ...
      expect(filterNotificationsByStatus(emptyArray, "CONFIRMED")).toHaveLength(
        0,
      );
