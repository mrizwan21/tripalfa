...
describe('NotificationDetailsPopup Component', () => {
  ...
  describe('Notification Content Display', () => {
    ...
    it('should display notification description', () => {
      ...
      expect(screen.getByText(MOCK_NOTIFICATIONS[0].description)).toBeInTheDocument();
    });
    ...
    it('should display passenger name when available', () => {
      ...
      if (ssrNotification) {
        ...
        expect(screen.getByText(ssrNotification.passengerName!)).toBeInTheDocument();
      }
    });
    ...
    it('should display segment information when available', () => {
      ...
      if (notificationWithSegment) {
        ...
        expect(screen.getByText(notificationWithSegment.segment!)).toBeInTheDocument();
      }
    });

    it('should display price and currency when available', () => {
      const notificationWithPrice = MOCK_NOTIFICATIONS.find((n) => n.price && n.price > 0);
      if (notificationWithPrice) {
        ...
        expect(screen.getByText(notificationWithPrice.currency!)).toBeInTheDocument();
      }
    });
    ...
    it('should display remarks for rejected notifications', () => {
      ...
      if (rejectedNotification) {
        ...
        expect(screen.getByText(rejectedNotification.remarks!)).toBeInTheDocument();
      }
    });
    ...
    it('should display status badge', () => {
      ...
      expect(screen.getByText(new RegExp(MOCK_NOTIFICATIONS[0].status))).toBeInTheDocument();
    });
  });
  ...
  describe('Status-specific Display', () => {
    ...
    it('should display REJECTED notification with remarks', () => {
      ...
      if (rejectedNotification) {
        ...
        expect(screen.getByText(rejectedNotification.status)).toBeInTheDocument();
      }
    });

    it('should display INFO notification with correct styling', () => {
      const infoNotification = MOCK_NOTIFICATIONS.find((n) => n.status === 'INFO');
      if (infoNotification) {
        const { container } = render(
          <NotificationDetailsPopup
            isOpen={true}
  ...
  describe('Notification Types Display', () => {
    it('should display SSR notification type correctly', () => {
      const ssrNotification = MOCK_NOTIFICATIONS.find((n) => n.type === 'INFO' && n.passengerName);
      if (ssrNotification) {
        render(
          <NotificationDetailsPopup
            isOpen={true}
    ...
    it('should display CONFIRMATION notification type correctly', () => {
      const confirmationNotification = MOCK_NOTIFICATIONS.find((n) => n.type === 'SUCCESS' && n.status === 'CONFIRMED');
      if (confirmationNotification) {
        ...
        expect(screen.getByText(confirmationNotification.title)).toBeInTheDocument();
      }
    });

    it('should display ITINERARY_CHANGE notification type correctly', () => {
      const itineraryNotification = MOCK_NOTIFICATIONS.find((n) => n.type === 'INFO' && n.segment);
      if (itineraryNotification) {
        ...
        expect(screen.getByText(itineraryNotification.title)).toBeInTheDocument();
      }
    });

    it('should display AMENDMENT notification type correctly', () => {
      const amendmentNotification = MOCK_NOTIFICATIONS.find((n) => n.type === 'WARNING' && n.status === 'REJECTED');
      if (amendmentNotification) {
        ...
        expect(screen.getByText(amendmentNotification.title)).toBeInTheDocument();
      }
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle notification with all optional fields', () => {
      ...
      if (fullNotification.passengerName) {
        expect(screen.getByText(fullNotification.passengerName)).toBeInTheDocument();
      }
    });
