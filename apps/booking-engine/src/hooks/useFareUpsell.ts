...
import type { FareOption, FareBenefit, FareFlexibility } from '../components/FareUpsellPopup';
...
function buildBenefits(flight: Flight, tier: 'light' | 'standard' | 'flex' | 'api'): FareBenefit[] {
  ...
  if (tier === 'api') {
    if (checked.length === 0) {
      benefits.push({ label: '1st Checked Bag', included: false, price: 'Fee applies' });
    } else {
  ...
  const hasFreeSeat =
    tier === 'flex' ||
    (tier === 'api' && (flight.refundable || /business|first/i.test(flight.cabin ?? '')));
...
function buildFlexibility(refundable: boolean, tier: 'light' | 'standard' | 'flex' | 'api'): FareFlexibility[] {
  ...
  if (tier === 'standard') {
    return [
      ...
      { label: 'Date Change (fee applies)', included: false, price: 'Fee applies' },
    ];
  }
 ...
 */
export function useFareUpsell(flight: Flight | null | undefined): {
  fares: FareOption[];
  ...
  return useMemo(() => {
    ...
    if (flight.upsells && flight.upsells.length > 0) {
      const allFares: Flight[] = [flight, ...flight.upsells];
