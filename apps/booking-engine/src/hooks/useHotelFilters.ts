...
import { HotelFilters } from '../components/hotel/HotelSearchFilters';
 ...
 */
function parseArrayParam(value: string | null, parser: (v: string) => any = Number): any[] {
  if (!value) return [];
...
export function useHotelFilters(): UseHotelFiltersResult {
  ...
  const parsedFilters = useMemo((): HotelFilters => {
    ...
    // Sort by
    const sortBy = searchParams.get(FILTER_PARAMS.sortBy) as HotelFilters['sortBy'];
    if (sortBy && ['top_picks', 'price_asc', 'price_desc', 'rating'].includes(sortBy)) {
    ...
    // Board type
    const boardType = searchParams.get(FILTER_PARAMS.boardType) as HotelFilters['boardType'];
  ...
  const updateUrl = useCallback((newFilters: HotelFilters) => {
  ...
  const setFilters = useCallback((newFilters: HotelFilters) => {
  ...
  const filterQueryParams = useMemo(() => {
    ...
    if (filters.hotelTypeIds.length > 0) params.hotelTypeIds = filters.hotelTypeIds;
    if (filters.boardType) params.boardType = filters.boardType;
    if (filters.minRating > 0) params.minRating = filters.minRating;
    if (filters.advancedAccessibilityOnly) params.advancedAccessibilityOnly = true;
