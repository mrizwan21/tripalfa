  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 gap-2">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-card w-full max-w-5xl rounded-[1rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-border">
...
export default function HotelBookingCard() {
  ...
  const [amendmentPrefill, setAmendmentPrefill] =
    useState<NotificationItem | null>(null);
        ...
        <div className="bg-[hsl(var(--secondary))] px-10 py-4 flex items-center justify-between border-y border-yellow-600/10 shadow-sm relative z-10 transition-all gap-2">
          <div className="flex items-center divide-x divide-border/50 gap-10 h-5">
            ...
            <p className="pl-10 text-[11px] font-black text-foreground uppercase tracking-widest">
              Supplier Ref:{" "}
              <span className="text-[hsl(var(--primary))]">
                SUP-{booking.id.slice(0, 6)}
              </span>
            </p>
            <p className="pl-10 text-[11px] font-black text-foreground uppercase tracking-widest">
              Invoice:{" "}
              <span className="text-[hsl(var(--primary))]">
                INV-{booking.id.slice(0, 6)}
              </span>
            </p>
          </div>
          <div className="flex items-center gap-6">
            ...
            <div className="relative group ml-4">
              ...
              <div className="absolute top-full right-0 mt-2 w-48 bg-card rounded-2xl shadow-2xl border border-border invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all origin-top-right transform scale-95 group-hover:scale-100 z-50 p-2 pointer-events-none group-hover:pointer-events-auto">
                {["Refund", "Amendment", "Cancel", "Special Request"].map(
                  (opt) => (
                    <Button
                      variant="outline"
                      size="default"
        ...
        <div className="p-10">
          ...
          {activeTab === "Costing" && (
            <div className="space-y-10 animate-in slide-in-from-bottom-10 duration-700">
              {/* Detailed Info Card */}
              <div className="bg-card rounded-[2rem] border border-border shadow-sm p-8 flex items-center justify-between group gap-2">
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[hsl(var(--muted))] border-b border-border">
                    <tr className="text-[11px] font-black text-[hsl(var(--primary))] uppercase tracking-widest divide-x divide-border">
                      ...
                      <th className="px-6 py-6 text-center">
                        Base ({booking.total?.currency})
                      </th>
                      <th className="px-6 py-6 text-center">
                        Taxes ({booking.total?.currency})
                      </th>
                      <th className="px-6 py-6 text-center">
                        Net ({booking.total?.currency})
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border text-[11px] font-bold text-muted-foreground">
                    <tr className="bg-card hover:bg-muted transition-colors divide-x divide-border">
                      <td className="px-6 py-10 font-black text-foreground">
                        {hotel.name}
                      </td>
                      <td className="px-6 py-10 text-muted-foreground italic">
                        Hotel Booking
                      </td>
          ...
          {activeTab === "Notification Manager" && (
            <div className="space-y-8 animate-in slide-in-from-right-10 duration-500">
              ...
              {/* Timeline / Log View */}
              <div className="bg-card rounded-[2.5rem] border border-border shadow-sm overflow-hidden p-8">
                  ...
                  {[
                    {
                      ...
                      description:
                        "Your hotel booking at Splendid Shubham has been confirmed.",
                    ...
                    <div key={i} className="relative pl-16 group">
                      {/* Timeline Dot */}
                      <div className="absolute left-0 top-1 w-14 h-14 rounded-2xl border-4 border-background shadow-lg flex items-center justify-center z-10 bg-card">
                        {item.icon}
                      </div>

                      {/* Content */}
                      <div className="bg-muted hover:bg-card p-6 rounded-2xl border border-border transition-all hover:shadow-md group-hover:scale-[1.01]">
