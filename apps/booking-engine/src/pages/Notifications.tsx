...
export default function Notifications() {
  ...
  async function load() {
    ...
    try {
      ...
      const mappedNotifications = Array.isArray(res) ? res.map(mapApiNotificationToItem) : [];
  ...
  useEffect(() => {
    pollingIntervalRef.current = setInterval(() => { load(); }, POLLING_INTERVAL);
    ...
    <div className="max-w-4xl mx-auto space-y-4 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Personalized alerts about your trips and account.</p>
        </div>
      ...
      {!loading && items.length > 0 && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 space-y-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search notifications..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" />
          </div>
          <div className="flex flex-wrap gap-2">
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as NotificationType | 'ALL')} className="px-3 py-2 border border-slate-200 rounded-lg text-sm">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as NotificationStatus | 'ALL')} className="px-3 py-2 border border-slate-200 rounded-lg text-sm">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Button onClick={load} className="mt-3">Try Again</Button>
        </div>
        ...
        <>
          <div className="space-y-3">
            {filteredAndPaginatedItems.map((n) => (
              <div key={n.id} className={cn("group relative bg-white rounded-lg border transition-all", n.read ? "border-slate-100 opacity-75" : "border-slate-200 shadow-sm")}>
                <div className="p-4 flex items-start gap-3">
                  <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center", n.type === 'SUCCESS' ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-600")}>
                    {n.type === 'SUCCESS' ? <CheckCircle2 size={18} /> : <Bell size={18} />}
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <h4 className={cn("font-medium", n.read ? "text-slate-600" : "text-slate-900")}>{n.title}</h4>
                      <span className="text-xs text-slate-400">{format(new Date(n.when), 'MMM d, h:mm a')}</span>
                    </div>
                    <p className={cn("mt-0.5 text-sm", n.read ? "text-slate-400" : "text-slate-600")}>{n.message}</p>
                    <div className="mt-2 flex items-center gap-3">
                      {!n.read && <button onClick={() => markRead(n.id)} className="text-xs font-medium text-purple-600">Mark as read</button>}
                      <button onClick={() => handleViewDetails(n)} className="text-xs font-medium text-slate-400">View Details</button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {selectedNotification && <NotificationDetailsPopup isOpen={isPopupOpen} onClose={handleClosePopup} notification={selectedNotification} />}
