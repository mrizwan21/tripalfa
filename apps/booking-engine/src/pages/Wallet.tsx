    ...
    return (
      <TripLogerLayout>
        <div className="min-h-screen bg-[hsl(var(--background))] flex items-center justify-center px-4 gap-2">
          <div className="bg-card rounded-xl border border-border shadow-sm p-8 text-center w-full max-w-xl">
            <h1 className="text-2xl font-semibold text-foreground mb-2">Wallet Disabled</h1>
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="flex justify-between items-start mb-8 gap-4">
              <div>
                ...
                <p className="text-muted-foreground text-sm">Manage your funds & transactions</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">Total Balance (Est. USD)</p>
                <h2 className="text-3xl font-semibold" data-testid="wallet-balance">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {accounts.map(account => (
              ...
              >
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-1">{account.currency} Wallet</p>
                ...
                <tbody className="divide-y divide-border">
                  {txs.map(tx => (
                    <tr key={tx.id} className="hover:bg-muted/50 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <span className="px-3 py-1 bg-blue-50 text-blue-600 border border-blue-100 rounded-full text-[9px] font-black uppercase tracking-widest">
                          {tx.status}
                        </span>
                      </td>
                      <td
                        className={`px-8 py-6 text-right font-black text-sm ${
                          tx.type === 'topup' ? 'text-blue-600' : 'text-foreground'
                        }`}
                      >
                        {tx.type === 'topup' ? '+' : '-'}
