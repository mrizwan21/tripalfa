...
function baggageLabel(svc: DuffelAncillary): string {
  ...
  const typeLabel =
    meta.type === "carry_on_bag" ? "Carry-on Bag" : "Checked Bag";
  const weightStr = meta.maximum_weight_kg
    ? ` – up to ${meta.maximum_weight_kg}kg`
    : "";
  const dimStr =
...
export default function FlightAddons() {
  ...
  const [selectedBaggage, setSelectedBaggage] = useState<
    Map<string, SelectedBaggageService>
  >(new Map());
  ...
  const baggageServices: DuffelAncillary[] = useMemo(
    () =>
      (flight.ancillaries || []).filter(
        (a: DuffelAncillary) => a.type === "baggage",
      ),
    [flight],
  );
  const hasSeatServices: boolean = useMemo(
    () =>
      (flight.ancillaries || []).some(
        (a: DuffelAncillary) => a.type === "seat",
      ),
    [flight],
  );
  ...
  const handleContinue = () => {
    navigate("/passenger-details", {
      state: {
        ...
        selectedServices: runtimeConfig.features.ancillariesEnabled
          ? selectedServiceIds
          : [],
        // Meal + Special Request SSR codes (free preferences)
        mealSSRs: runtimeConfig.features.ancillariesEnabled
          ? Array.from(selectedMeals)
          : [],
        specialSSRs: runtimeConfig.features.ancillariesEnabled
          ? Array.from(selectedSSRs)
          : [],
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div className="space-y-4">
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                  Services sourced in real-time from{" "}
                  {flight.airline || "the airline"}
                </p>
              </div>
              {/* Step breadcrumb */}
        ...
        <div className="container mx-auto px-4 max-w-7xl mt-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 space-y-10">
              ...
              <div className="bg-card rounded-[2.5rem] border border-border shadow-xl shadow-gray-200/40 overflow-hidden">
                <div className="p-10 border-b border-border/50 flex items-center gap-8">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-blue-50 flex items-center justify-center text-blue-600 shadow-inner gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mt-1">
                      Prices set by {flight.airline || "the airline"} – fetched
                      live from Duffel
                    </p>
                  </div>
                </div>
                ...
                {baggageServices.length === 0 ? (
                  <div className="p-10 flex items-center gap-4 text-muted-foreground">
                    ...
                    <p className="text-sm font-semibold">
                      No additional baggage services are available for this
                      fare. Check your included allowance below.
                    </p>
                  </div>
                ) : (
                  <div className="p-10 space-y-4">
                    {baggageServices.map((svc) => {
                      ...
                      return (
                        <div
                          key={svc.id}
                          className={`flex items-center justify-between p-6 rounded-[1.5rem] border transition-all duration-200 ${qty > 0 ? "border-[hsl(var(--primary))] bg-purple-50/30 ring-2 ring-[hsl(var(--primary)/0.1)]" : "border-border hover:border-border/80"}`}
                        >
                          <div className="flex-1 gap-4">
                            ...
                            <p className="text-[11px] font-bold text-muted-foreground mt-1">
                              {formatMoney(svc.price, svc.currency)} each · max{" "}
                              {max} per person
                            </p>
                          </div>
              ...
              <div className="bg-card rounded-[2.5rem] border border-border shadow-xl shadow-gray-200/40 overflow-hidden">
                <div className="p-10 border-b border-border/50 flex items-center gap-8">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-purple-50 flex items-center justify-center text-purple-600 shadow-inner gap-2">
              ...
              <div className="bg-card rounded-[2.5rem] border border-border shadow-xl shadow-gray-200/40 overflow-hidden">
                ...
                >
                  <div className="w-16 h-16 rounded-[1.5rem] bg-orange-50 flex items-center justify-center text-orange-600 shadow-inner flex-shrink-0 gap-2">
                    ...
                    {IATA_MEAL_SSRS.map((meal) => {
                      ...
                      return (
                        <Button
                          ...
                          className={`flex items-center justify-between p-6 rounded-[1.5rem] border transition-all duration-200 text-left ${
                            active
              ...
              <div className="bg-card rounded-[2.5rem] border border-border shadow-xl shadow-gray-200/40 overflow-hidden">
                <Button
                  ...
                  className="w-full p-10 border-b border-border/50 flex items-center gap-8 text-left hover:bg-muted/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <div className="w-16 h-16 rounded-[1.5rem] bg-teal-50 flex items-center justify-center text-teal-600 shadow-inner flex-shrink-0 gap-2">
                    ...
                    {IATA_SPECIAL_SSRS.map((ssr) => {
                      ...
                      return (
                        <Button
                          ...
                          className={`w-full flex items-center justify-between p-6 rounded-[1.5rem] border transition-all duration-200 text-left ${
                            active
            ...
            <div className="lg:col-span-4">
              <div className="bg-[hsl(var(--primary))] rounded-[2.5rem] p-10 text-[hsl(var(--primary-foreground))] shadow-2xl sticky top-32">
                ...
                <div className="space-y-8">
                  ...
                  <Button
                    ...
                    className="w-full h-16 bg-[hsl(var(--secondary))] hover:bg-[hsl(var(--secondary)/0.9)] text-[hsl(var(--secondary-foreground))] rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-purple-900/40 transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3"
                  >
