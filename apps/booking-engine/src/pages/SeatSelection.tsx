...
export default function SeatSelection() {
  ...
  const passengers = statePassengers.length > 0 
    ? statePassengers 
  ...
  const activePassenger = passengers[activePassengerIndex] || passengers[0] || { id: 'passenger_1', label: 'Passenger 1' };
    ...
    if (element.type === 'seat') {
        ...
        <button
          ...
          className={`w-10 h-10 flex items-center justify-center rounded font-bold text-xs transition-all duration-200 shrink-0 ${
            isSelected
              ...
              : isAvailable
              ? 'bg-white border-2 border-gray-300 text-gray-900 hover:border-blue-400 hover:bg-blue-50 cursor-pointer'
      ...
      return (
        <div
          ...
          className="w-10 h-10 flex items-center justify-center text-xs bg-gray-100 border border-dashed border-gray-300 rounded shrink-0"
          title="Bassinet"
        >
          👶
        </div>
      ...
      return (
        <div
          ...
          className="px-2 h-10 flex items-center justify-center text-sm opacity-60 bg-gray-200 border border-gray-300 rounded flex-1 min-w-0"
  ...
  const renderCabin = (cabin: any, segmentId: string, aircraft?: AircraftConfig) => {
    ...
    return (
      <div key={`cabin-${cabin.cabin_class}`} className="mb-12">
        ...
        <div className="inline-block border border-gray-300 rounded-lg p-4 bg-gray-50 overflow-x-auto">
          {cabin.rows.map((row: any, rowIndex: number) => (
            <div key={`row-${rowIndex}`} className="flex gap-3 mb-3 items-stretch">
              ...
              <span className="w-8 text-center text-xs font-bold text-gray-600 shrink-0 flex items-center justify-center bg-gray-200 rounded h-10">
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-4 pt-6 border-t">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-white border-2 border-gray-300 rounded flex items-center justify-center text-xs font-bold">
              1A
            </div>
            <span className="text-xs text-gray-600">Available</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-blue-600 border-2 border-blue-700 rounded flex items-center justify-center text-xs font-bold text-white">
              1B
            </div>
          ...
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gray-100 border border-dashed border-gray-300 rounded flex items-center justify-center text-xs">
              👶
            </div>
            <span className="text-xs text-gray-600">Bassinet</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="px-2 h-8 bg-gray-200 border border-gray-300 rounded flex items-center justify-center text-xs">
              🚽
            </div>
  ...
  const renderDynamicSeatGrid = (cabinElement: any, aircraft?: AircraftConfig): React.ReactNode => {
    ...
    return (
      <div className="inline-block border border-gray-300 rounded-lg p-4 bg-gray-50">
                  ...
                  {row.sections[sectionIdx]?.elements.map((element: any, elementIndex: number) => {
                    ...
                    return element.type === 'seat' ? (
                      ...
                      <div
                        ...
                        className="w-10 h-10 flex items-center justify-center text-xs text-gray-500 opacity-50"
                        title={element.type}
                      >
      ...
      <TripLogerLayout>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-lg text-gray-600">Loading seat maps...</p>
          </div>
        </div>
      </TripLogerLayout>
    );
      ...
      <TripLogerLayout>
        <div className="max-w-4xl mx-auto p-6 mt-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <span className="text-red-800">No flight offer found. Please select a flight first.</span>
          </div>
        </div>
      </TripLogerLayout>
    );
      ...
      <TripLogerLayout>
        <div className="max-w-4xl mx-auto p-6 mt-8">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <h3 className="text-lg font-semibold text-blue-900 mb-2">Seat Selection Not Available</h3>
            <p className="text-blue-700 mb-4">
              This flight doesn't support seat selection. You can proceed with your booking without selecting specific seats.
            </p>
            <button
      ...
      <div className="max-w-6xl mx-auto p-6 mt-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Choose preferred seats for each segment</p>
          </div>
          {totalSeatCost > 0 && (
            <div className="text-right">
              <p className="text-sm text-gray-600">Seat charges</p>
              <p className="text-3xl font-bold text-blue-600">{formatCurrency(totalSeatCost, 'USD')}</p>
            </div>
          )}
        </div>
        ...
        {passengers.length > 0 && (
          <div className="bg-white rounded-lg border p-6 mb-6">
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                      activePassengerIndex === index ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-600'
                    }`}>
                      {index + 1}
                    </div>
                    <div className="text-left">
                      <p className="font-medium">{passenger.label || passenger.name || `Passenger ${index + 1}`}</p>
            ...
            <p className="mt-4 text-sm text-gray-500">
              Selecting seats for: <span className="font-semibold text-gray-900">{activePassenger?.label || `Passenger ${activePassengerIndex + 1}`}</span>
            </p>
          </div>
        )}
        ...
        {currentSeatMap ? (
                      ...
                      {row.sections.map((section, sectionIndex) => (
                        <div key={`section-${sectionIndex}`} className="flex gap-2">
                          {section.elements.map((element, elementIndex) => {
                            ...
                            const isAvailable =
                              element.type === 'seat' && element.available_services && element.available_services.length > 0;
          ...
          <div className="bg-gray-50 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-600">No seat maps available for this segment</p>
          </div>
        )}
      </div>
    </TripLogerLayout>
