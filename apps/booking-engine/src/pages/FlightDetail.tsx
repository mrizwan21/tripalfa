...
export default function FlightDetail() {
  ...
  const mapDuffelOfferToFlight = (offer: any) => {
    ...
    if (changeCond.allowed === true) {
      ...
      fareRules.push(
        "Date change policy: contact support at least 24h before departure.",
      );
    }
    ...
    const allSlices = slices.map((slice: any, sliceIdx: number) => {
      ...
      return {
        ...
        departureDate:
          slice?.departure_date || segs[0]?.departing_at?.split("T")[0],
        segments: segs.map((seg: any, idx: number) => {
          ...
          const layover = nextSeg
            ? (() => {
                const diff =
                  new Date(
                    nextSeg.departing_at || nextSeg.departure_time,
                  ).getTime() - new Date(arrAt).getTime();
    ...
    return {
      ...
      destination:
        lastSegOfLastSlice?.destination?.iata_code ||
        lastSegOfLastSlice?.destination_iata ||
        "N/A",
      airline:
      ...
      cabin:
        offer.passengers?.[0]?.fare_brand_name ||
        offer.passengers?.[0]?.cabin_class ||
        "Economy",
    ...
    return (
      <TripLogerLayout>
        <div className="flex flex-col items-center justify-center min-h-screen pb-32 gap-4">
          <Loader2
            size={48}
            className="text-[hsl(var(--primary))] animate-spin mb-4"
          />
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                  {slice.departureDate} •{" "}
                                  {slice.segments?.length} flight(s) • Total:{" "}
                                  {slice.totalDuration}
                                </p>
                              </div>
                  ...
                  <div className="space-y-8">
                    <div className="flex items-start gap-6 p-8 bg-muted rounded-3xl border border-border">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-muted-foreground leading-relaxed">
                          Your fare include {flight.includedBags?.[0]?.quantity}{" "}
                          bags ({flight.includedBags?.[0]?.weight}
                          {flight.includedBags?.[0]?.unit} each). Maximum
                    ...
                    <div className="flex items-start gap-6 p-8 border border-border rounded-3xl">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-muted-foreground leading-relaxed">
                          1 piece of 7kg (55 x 35 x 25 cm) plus one small
