...
class CurrencyService {
   ...
   */
  async updateCurrencySettings(request: UpdateCurrencyRequest): Promise<Currency> {
    const response = await this.api.patch<Currency>(`/currencies/${request.id}`, {
  ...
  async setBaseCurrency(id: string): Promise<Currency> {
    const response = await this.api.patch<Currency>(`/currencies/${id}/set-base`);
    return response.data;
  }
  ...
  async convertCurrency(
    ...
    try {
      const response = await this.api.post<CurrencyConversionResult>('/currencies/convert', {
        amount,
        fromCurrency,
        toCurrency,
      });
   ...
   */
  async refreshExchangeRates(): Promise<{ success: boolean; updatedCount: number }> {
    try {
      const response = await this.api.post<{ success: boolean; updatedCount: number }>('/currencies/refresh-rates');
      return response.data;
    } catch (error) {
   ...
   */
  getRoundingModes(): { value: RoundingMode; label: string; description: string }[] {
    return [
      { value: 'HALF_UP', label: 'Half Up (Standard)', description: 'Rounds 0.5 up (2.5 → 3)' },
      { value: 'HALF_DOWN', label: 'Half Down', description: 'Rounds 0.5 down (2.5 → 2)' },
      { value: 'BANKERS', label: "Banker's Rounding", description: 'Rounds to nearest even (2.5 → 2, 3.5 → 4)' },
      { value: 'HALF_ODD', label: 'Half Odd', description: 'Rounds to nearest odd number' },
      { value: 'DOWN', label: 'Round Down', description: 'Always rounds towards zero' },
      { value: 'UP', label: 'Round Up', description: 'Always rounds away from zero' },
   ...
   */
  calculateEffectiveRate(baseRate: number, bufferPercentage: number | null): number {
   ...
   */
  roundAmount(amount: number, roundingMode: RoundingMode, decimalPlaces: number): number {
  ...
  private calculateConversion(
    ...
    const effectiveRate = this.calculateEffectiveRate(baseRate, bufferPercentage);
