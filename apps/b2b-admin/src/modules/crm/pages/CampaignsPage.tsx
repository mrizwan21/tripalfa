...
import { Plus, Search, BarChart3, Users, Mail, MousePointerClick, TrendingUp, MoreHorizontal, Play, Pause, Eye, Megaphone } from '@tripalfa/ui-components/icons';
...
export default function CampaignsPage() {
  ...
  const getStatusForBadge = (status: string): 'primary' | 'info' | 'warning' | 'success' | 'default' => {
    const statusMap = {
  ...
  const CampaignCard = ({ campaign }: { campaign: Campaign }) => {
    ...
    const conversionRate = campaign.sentCount > 0 ? (campaign.convertedCount / campaign.sentCount) * 100 : 0;
        ...
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <StatusBadge status={getStatusForBadge(campaign.status)} label={campaign.status.charAt(0).toUpperCase() + campaign.status.slice(1)} size="sm" />
