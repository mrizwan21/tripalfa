...
import { Search, Plus, Mail, Phone, MapPin, Filter, MoreHorizontal, Database } from '@tripalfa/ui-components/icons';
...
export default function ContactsPage() {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="p-4">
          {isLoading ? (
                ...
                <tbody>
                  {sorted.map(contact => (
                    ...
                    >
                      ...
                      <td className="record-table-cell">
                        <StatusBadge status={contact.tier as any} label={contact.tier?.charAt(0).toUpperCase() + contact.tier?.slice(1) || 'Unknown'} size="sm" />
                      </td>
                      <td className="record-table-cell">
                      ...
                      <td className="record-table-cell">
                        {contact.openTicketsCount > 0 ? (
                          <StatusBadge status="failed" label={contact.openTicketsCount.toString()} size="sm" />
                        ) : (
            ...
            <div className="empty-state">
              ...
              <p className="empty-state-description">Start by adding your first contact to track customer interactions</p>
            </div>
          )}
        </div>
      </div>
      ...
      {selectedContact && (
        <ContactDetailModal contact={selectedContact} isOpen={!!selectedContact} onClose={() => setSelectedContact(null)} />
      )}
    </div>
  );
}
