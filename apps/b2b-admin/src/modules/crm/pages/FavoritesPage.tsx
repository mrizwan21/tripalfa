...
export default function FavoritesPage() {
  ...
  const { data: folders, isLoading: foldersLoading, refetch: refetchFolders } = useQuery<FavoriteFolder[]>({
  ...
  const { data: favoriteItems, isLoading: itemsLoading, refetch: refetchItems } = useQuery<FavoriteItem[]>({
  ...
  const getPriorityForBadge = (priority?: string): 'primary' | 'info' | 'warning' | 'success' | 'default' => {
    const priorityMap = {
    ...
    <div className="space-y-5">
      <div className="action-row">
        <div>
          ...
          <p className="text-caption mt-1">Organize and quick-access important contacts and opportunities</p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-orange-800 mt-1">{kycApplicants.length} applicants favorited for urgent review</p>
              </div>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">
          {selectedFolder ? `Favorites in: ${folders?.find((f) => f.id === selectedFolder)?.name}` : 'All Favorites'}
        </h2>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 flex-wrap mt-3">
                      {item.priority && (
                        <StatusBadge status={getPriorityForBadge(item.priority)} label={item.priority} size="sm" />
                      )}
                      {item.tags.map((tag) => (
      ...
      <Dialog open={isNewItemOpen} onOpenChange={setIsNewItemOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => addFavoriteMutation.mutate({ ...newItemData, folderId: selectedFolder || undefined })}>
              Add
            </Button>
          </DialogFooter>
        </DialogContent>
