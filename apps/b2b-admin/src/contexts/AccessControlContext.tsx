...
const AccessControlContext =
  createContext<AccessControlContextValue>(defaultValue);
...
function derivePermissions(
  ...
  const fromExplicit = Array.isArray(explicitPermissions)
    ? explicitPermissions
    : [];
...
export function AccessControlProvider({ children }: PropsWithChildren) {
  ...
  const [permissions, setPermissions] = useState<string[]>(
    initialPermissions.length
      ? initialPermissions
      : derivePermissions(initialUser, initialRole),
  );
  ...
  const setSession: AccessControlContextValue["setSession"] = ({
    ...
    const resolvedPermissions = derivePermissions(
      resolvedUser,
      resolvedRole,
      nextPermissions,
    );
