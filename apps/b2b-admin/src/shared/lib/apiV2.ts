...
export interface QueueActionPayload {
  action: 'submit' | 'price' | 'invoice' | 'confirm_payment' | 'book_supplier' | 'confirm' | 'complete' | 'cancel'
  reason?: string
}
 ...
 */
export async function updateWorkflowState(id: string, update: WorkflowStateUpdate) {
...
export async function calculatePricing(id: string, override?: PricingOverride) {
  const response = await api.post(`/v2/admin/bookings/${id}/pricing`, { override })
  return response.data
}
...
export async function generateInvoice(id: string, dueDate?: string) {
  const response = await api.post(`/v2/admin/bookings/${id}/invoice`, { dueDate })
  return response.data
}
 ...
 */
export async function processWalletPayment(id: string, paymentMethod: string = 'wallet') {
  const response = await api.post(`/v2/admin/bookings/${id}/pay-wallet`, { paymentMethod })
  return response.data
}
 ...
 */
export async function performQueueAction(id: string, action: QueueActionPayload) {
  const response = await api.post(`/v2/admin/bookings/${id}/queue-action`, action)
  return response.data
}
...
export async function listBookingQueues(filters: BookingV2Filters = {}) {
  ...
  const response = await api.get(`/v2/admin/bookings/queues?${params.toString()}`)
  return response.data
}
