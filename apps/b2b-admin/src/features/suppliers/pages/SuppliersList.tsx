...
export default function SuppliersPage({
    ...
    try {
      ...
      setSuppliers(
        payload.map((supplier) => {
          ...
          const type =
            category.toLowerCase().includes("flight") ||
            category.toLowerCase().includes("gds")
              ? "flight"
              : "hotel";
          return {
            id: supplier.id,
  ...
  const toggleSupplier = async (id: string) => {
    ...
    setSuppliers(
      suppliers.map((s) => (s.id === id ? { ...s, status: nextStatus } : s)),
    );
    try {
      ...
      setSuppliers(
        suppliers.map((s) =>
          s.id === id ? { ...s, status: target.status } : s,
        ),
      );
    }
  };
  ...
  const handleCreateSupplier = async (
    ...
    const slug = values.supplierName
      ...
      .replace(/(^-|-$)/g, "");
    const code =
      values.supplierIdCode?.trim() || slug.toUpperCase().replace(/-/g, "_");
    const category =
      values.supplierType === "GDS" || values.supplierType === "Direct"
        ? "FLIGHT"
        : "HOTEL";
    try {
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="sm:max-w-4xl">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={profileForm.handleSubmit(handleCreateSupplier)}
                disabled={isSaving}
              >
