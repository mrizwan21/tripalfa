...
export const productionConfigSchema = z.object({
  ...
  apiVersion: z.string(),
  authenticationType: z.enum(['api-key', 'oauth2', 'jwt', 'bearer', 'basic']),
  authenticationCredentials: z.record(z.string(), z.unknown()).refine(
...
export const gatewayFormSchema = z.object({
  environments: z.array(environmentSpecificConfigSchema).min(1, 'At least one environment required'),
