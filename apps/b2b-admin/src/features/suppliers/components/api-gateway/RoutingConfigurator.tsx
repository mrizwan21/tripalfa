...
import { GeographyRouting, ChannelRouting, ChannelType } from '@/services/api-manager/types-gateway'
...
export interface RoutingConfiguratorProps {
   ...
   */
  availableEndpoints?: Array<{ id: string; name: string; method: string; url: string }>
  /**
...
export const RoutingConfigurator: React.FC<RoutingConfiguratorProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'geography' | 'channel'>('geography')
  const [editingGeographyId, setEditingGeographyId] = useState<string | null>(null)
  const [editingChannelId, setEditingChannelId] = useState<string | null>(null)

  const currentGeographies = (form.formData.geographyRoutings || geographyRoutings).map(g => ({
    ...g,
    id: (g as any).id || `geo-${Date.now()}-${Math.random()}`,
  })) as GeographyRouting[]
  const currentChannels = (form.formData.channelRoutings || channelRoutings).map(c => ({
    ...c,
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <h3 className="font-semibold text-gray-900 mb-1">Smart Routing Configuration</h3>
        <p className="text-sm text-gray-600">
          Direct API requests to different endpoints based on geography or user channel.
          Set primary and fallback endpoints for redundancy.
        </p>
      </div>
...
const GeographyRoutingSection: React.FC<GeographyRoutingSectionProps> = ({
  ...
  const [newGeography, setNewGeography] = useState<Partial<GeographyRouting> | null>(
    null
  )
  ...
  const handleSave = useCallback(
    (geography: Partial<GeographyRouting>) => {
      ...
      const updated = editingId
        ? geographies.map((g) => (g.geography === editingId ? geographyWithId : g))
    ...
    <div className="space-y-4">
      ...
      {geographies.length === 0 && !newGeography && editingId === null && (
        <div className="p-6 text-center border-2 border-dashed border-gray-300 rounded-lg">
          <p className="text-gray-600 mb-3">No geography routing configured</p>
    ...
    <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                geographyType: e.target.value as 'global' | 'regional' | 'country',
              })
            }
            disabled={disabled}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <div className="flex gap-2 justify-end pt-4 border-t">
        ...
        <button
          ...
          disabled={disabled || !formData.geography || !formData.preferredEndpoint}
          className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50"
        >
  ...
  return (
    <div className="p-4 rounded-lg border border-gray-200 bg-white">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h4 className="font-medium text-gray-900">{geography.geography}</h4>
      ...
      {!geography.isActive && (
        <div className="mt-2 text-sm text-amber-600">⚠️ This routing is disabled</div>
      )}
    </div>
  )
}
...
const ChannelRoutingSection: React.FC<ChannelRoutingSectionProps> = ({
  ...
  const [newChannel, setNewChannel] = useState<Partial<ChannelRouting> | null>(null)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2">
        {CHANNEL_OPTIONS.map((option) => {
          const channelConfig = channels.find((c) => c.channel === option.value)
      ...
      <div className="space-y-2">
        {channels.map((channel) => (
          <ChannelRoutingCard
            key={channel.channel}
            channel={channel}
            channelLabel={CHANNEL_OPTIONS.find((o) => o.value === channel.channel)?.label}
            availableEndpoints={availableEndpoints}
      ...
      {channels.length === 0 && editingId === null && (
        <div className="p-6 text-center border-2 border-dashed border-gray-300 rounded-lg">
          <p className="text-gray-600 mb-3">No channel routing configured</p>
...
const ChannelRoutingForm: React.FC<ChannelRoutingFormProps> = ({
  ...
  const channelLabel =
    allChannels.find((c) => c.value === formData.channel)?.label || formData.channel

  return (
    <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-4">
      <h4 className="font-medium text-gray-900">Configure {channelLabel} Routing</h4>
...
const ChannelRoutingCard: React.FC<ChannelRoutingCardProps> = ({
  ...
  return (
    <div className="p-4 rounded-lg border border-gray-200 bg-white">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h4 className="font-medium text-gray-900">{channelLabel} Routing</h4>
          <p className="text-sm text-gray-600 mt-1">
            Priority: {channel.priority}/10 • Fallbacks: {channel.fallbackEndpointIds?.length || 0}
          </p>
        </div>
        <div className="flex gap-2">
          <button
