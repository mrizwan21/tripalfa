...
export const EnvironmentSelector: React.FC<EnvironmentSelectorProps> = ({
  ...
  const getEnvironmentStatus = (env: Environment): 'configured' | 'missing' | 'invalid' => {
    const envConfig = form.formData.environments?.find((e) => e.environment === env)

    if (!envConfig) return 'missing'

    // Check if required fields are filled
    const hasBaseUrl = envConfig.baseUrl && envConfig.baseUrl.trim() !== ''
    const hasCredentials = envConfig.authenticationCredentials && Object.keys(envConfig.authenticationCredentials).length > 0
        ...
        {environments.map((env) => {
          ...
          return (
            <button
              ...
              `}
              title={disabled ? 'This environment cannot be modified' : undefined}
            >
            ...
            {environments.map((env) => (
              ...
              >
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-medium text-gray-700">Security & Performance</h4>
                  <ul className="text-sm space-y-1">
                    <li className="flex items-center gap-2">
                      ...
                      <span>
                        SSL/TLS {env.config.requiresSSL ? 'Required' : 'Optional'}
                      </span>
                    </li>
                    <li className="flex items-center gap-2">
                    ...
                    <li className="flex items-center gap-2">
                      ...
                      <span>Monitoring {env.config.requiresMonitoring ? 'Required' : 'Optional'}</span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="mt-4 space-y-2">
                  <h4 className="text-sm font-medium text-gray-700">Performance Limits</h4>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="bg-white p-2 rounded">
                      ...
                      <p className="font-semibold text-gray-900">
                        {env.config.rateLimit ? `${env.config.rateLimit} req/s` : 'Unlimited'}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span
                    ...
                    `}
                  >
                    {status === 'configured' ? '✓ Ready' : status === 'invalid' ? '✗ Incomplete' : '○ Empty'}
                  </span>
                </div>
...
const EnvironmentConfigurationStatus: React.FC<EnvironmentConfigurationStatusProps> = ({
  environment,
  status,
}) => {
...
export const EnvironmentSelectorTabs: React.FC<EnvironmentSelectorProps> = (props) => {
...
export const EnvironmentSelectorInline: React.FC<Omit<EnvironmentSelectorProps, 'showStatus'>> = (props) => {
  return <EnvironmentSelector {...props} showStatus={true} showDetails={false} />
}
