...
import type { GatewayConfig, EnvironmentConfig, SupplierAPIGatewayFormData } from '@/services/api-manager/types-gateway'
...
export type GatewayFormSection = 'environment' | 'products' | 'authentication' | 'endpoints' | 'routing' | 'summary'
...
export const GatewayForm: React.FC<GatewayFormProps> = ({
  ...
  const [expandedSections, setExpandedSections] = useState<SectionCompletionStatus>({
    environment: true,
    products: true,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const errorObj = error instanceof Error ? error : new Error('Failed to save configuration')
      ...
      {!readOnly && (
        <div className="flex gap-3 justify-end pt-6 border-t">
          ...
          <button
            ...
            disabled={isSubmitting || !form.isDirty || completedSteps < totalSteps}
            className={`
...
const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({
  ...
  return (
    <div className={`border rounded-lg overflow-hidden ${hasError ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}>
      <button
        onClick={onToggle}
        className={`
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <SummaryCard
          ...
          items={[
            {
              label: 'Selected',
              value: config.selectedEnvironment?.charAt(0).toUpperCase() + (config.selectedEnvironment?.slice(1) || ''),
            },
            {
              label: 'Timeout',
        ...
        <SummaryCard
          ...
          items={[
            {
              label: 'Type',
              value: config.selectedAuthType?.charAt(0).toUpperCase() + (config.selectedAuthType?.slice(1) || ''),
            },
            {
              label: 'Status',
              value: 'Configured',
        ...
        <SummaryCard
          ...
          items={[
            ...
            {
              label: 'Status',
              value: config.endpoints && config.endpoints.length > 0 ? 'Ready' : 'Pending',
            },
          ]}
        />
        ...
        <SummaryCard
          ...
          items={[
            ...
            {
              label: 'Status',
              value: config.geographyRoutings && config.geographyRoutings.length > 0 ? 'Active' : 'None',
            },
          ]}
        />
        ...
        <SummaryCard
          ...
          items={[
            ...
            {
              label: 'Status',
              value: config.channelRoutings && config.channelRoutings.length > 0 ? 'Active' : 'None',
            },
          ]}
        />
      </div>
      ...
      <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <div className="space-y-2 text-sm">
          ...
          <DetailRow label="Environment" value={config.selectedEnvironment || 'N/A'} />
          <DetailRow
      ...
      {/* Ready to Save */}
      <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
        <p className="text-green-800 font-medium">✓ Configuration is complete</p>
        <p className="text-green-700 text-sm mt-1">
          All required sections have been filled. Click "Save Configuration" to proceed.
        </p>
      </div>
    </div>
  )
    ...
    <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
      ...
      <div className="space-y-2">
        {items.map((item, index) => (
          <DetailRow key={index} label={item.label} value={item.value.toString()} />
        ))}
      </div>
    </div>
  )
