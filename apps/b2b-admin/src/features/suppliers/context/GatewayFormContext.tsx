...
type EnvironmentSpecificGateway = {
...
export interface GatewayFormContextType extends GatewayFormState {
  ...
  updateEnvironmentConfig: (
    env: Environment,
    config: EnvironmentSpecificGateway,
  ) => void;
...
const GatewayFormContext = createContext<GatewayFormContextType | undefined>(
  undefined,
);
...
export const GatewayFormProvider: React.FC<GatewayFormProviderProps> = ({
  ...
  const removeEnvironment = useCallback((environment: Environment) => {
    setFormData((prev) => {
      ...
      return {
        ...
        activeEnvironment:
          filtered.length > 0 ? filtered[0].environment : "development",
      };
    });
    setIsDirty(true);
  }, []);
  ...
  const validateEnvironment = useCallback(
    (environment: Environment): boolean => {
      const env = formData.environments?.find(
        (e) => e.environment === environment,
      );
      if (!env) return false;
  ...
  const resetForm = useCallback(
    (initialData?: Partial<SupplierAPIGatewayFormData>) => {
      setFormData(
        initialData || {
          environments: [],
          activeEnvironment: "development",
  ...
  return (
    <GatewayFormContext.Provider value={state}>
      {children}
...
export const useGatewayFormState = () => {
  const { formData, isDirty, isSaving, errors, saveError, successMessage } =
    useGatewayForm();
