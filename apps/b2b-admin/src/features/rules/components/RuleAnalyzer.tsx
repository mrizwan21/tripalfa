...
import type { RuleConflictAnalysis, RuleImpactAnalysis, Rule } from '@/features/rules/types-rule-engine'
...
export const RuleAnalyzer: React.FC<RuleAnalyzerProps> = ({
  ...
  const selectedRule = useMemo(() => {
    return rules.find((r) => r.name === selectedRuleId || r.name?.includes(selectedRuleId || ''))
  ...
  const handleRunAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const recommendations = generateRecommendations(selectedRule, impactAnalysis, conflicts)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Rule Analyzer</h2>
          <p className="text-gray-600 mt-1">Impact analysis and conflict detection</p>
        </div>
        <button
      ...
      {!selectedRule && rules.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-900">
            Select a rule from the Rule Builder to analyze its impact and conflicts
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {activeTab === AnalysisTab.Performance && (
            <PerformanceAnalysisView
              executionTime={analysisResults.impactAnalysis.estimatedExecutionTime}
              affectedCount={analysisResults.impactAnalysis.affectedEntities.reduce((sum: number, e: any) => sum + e.count, 0)}
            />
          )}

          {activeTab === AnalysisTab.Recommendations && (
            <RecommendationsView recommendations={analysisResults.recommendations} />
          )}
        </div>
      )}
    </div>
...
function generateRecommendations(
  ...
  if (impact.affectedEntities > 1000) {
    recommendations.push('High entity count: Consider implementing rate limiting')
  }

  if (impact.estimatedExecutionTime > 500) {
    recommendations.push('Execution time is high: Optimize conditions or actions')
  }
  ...
  if (impact.dependedByRules.length > 3) {
    recommendations.push('This rule has many dependents: Be careful when modifying')
  }
...
const ImpactAnalysisView: React.FC<ImpactAnalysisViewProps> = ({ analysis }) => {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Affected Entities */}
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h3 className="font-semibold text-gray-900 mb-3">Affected Entities</h3>
          <div className="space-y-2">
            <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
              <span className="text-sm font-medium text-gray-700">Total Count</span>
              <span className="text-sm font-bold text-gray-900">{analysis.affectedEntities?.toLocaleString() || 0}</span>
            </div>
            {analysis.impactedEntityTypes?.map((type: string, idx: number) => (
              <div key={idx} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                <span className="text-sm font-medium text-gray-700 capitalize">{type}</span>
                <span className="text-sm font-bold text-gray-900">Impacted</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Execution Time */}
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h3 className="font-semibold text-gray-900 mb-3">Execution Performance</h3>
          <div className="space-y-3">
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm text-gray-600">Estimated Time</span>
                <span className="text-sm font-bold text-gray-900">{analysis.estimatedExecutionTime}ms</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {analysis.dependsOnRules && analysis.dependsOnRules.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="font-semibold text-gray-900 mb-3">Depends On</h3>
            <div className="space-y-1">
              {analysis.dependsOnRules.map((dep: string, idx: number) => (
                <div key={idx} className="text-sm text-gray-600 py-1 border-b last:border-b-0">
                  {dep}
                </div>
              ))}
            </div>
        ...
        {analysis.dependedByRules && analysis.dependedByRules.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="font-semibold text-gray-900 mb-3">Depended By</h3>
            <div className="space-y-1">
...
const ConflictAnalysisView: React.FC<ConflictAnalysisViewProps> = ({ conflicts }) => {
  if (conflicts.length === 0) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
        <CheckCircle className="mx-auto text-green-600 mb-3" size={32} />
        <p className="text-sm text-green-900 font-medium">No conflicts detected</p>
        <p className="text-xs text-green-700 mt-1">This rule is safe to activate</p>
      </div>
    )
  }
...
const ConflictCard: React.FC<{ conflict: any }> = ({ conflict }) => {
  ...
  return (
    <div className={`border-2 rounded-lg p-4 ${severityColors[conflict.severity]}`}>
...
const PerformanceAnalysisView: React.FC<PerformanceAnalysisViewProps> = ({
  ...
  const throughput = affectedCount > 0 ? (affectedCount / (executionTime / 1000)).toFixed(0) : 0
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <MetricCard
        ...
        status={executionTime > 1000 ? 'warning' : executionTime > 500 ? 'caution' : 'good'}
      />
      <MetricCard
...
const MetricCard: React.FC<{
  ...
  return (
    <div className={`border rounded-lg p-4 ${statusColors[status]}`}>
      <p className={`text-xs font-medium mb-2 ${textColors[status]}`}>{label}</p>
...
const RecommendationsView: React.FC<RecommendationsViewProps> = ({ recommendations }) => {
  return (
    <div className="space-y-3">
