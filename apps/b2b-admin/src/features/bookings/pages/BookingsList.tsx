  ...
  const applyLocalFilters = useCallback(
    (records: Booking[]) => {
      return records.filter((b) => {
        if (
          filters.reference &&
          !b.reference.toLowerCase().includes(filters.reference.toLowerCase())
        )
          return false;
        if (
          filters.email &&
          !b.userEmail.toLowerCase().includes(filters.email.toLowerCase())
        )
          return false;
        ...
        if (filters.fromDate) {
          ...
          if (Number.isFinite(from.getTime()) && new Date(b.createdAt) < from)
            return false;
        }

        if (filters.toDate) {
          const to = new Date(filters.toDate);
          if (Number.isFinite(to.getTime()) && new Date(b.createdAt) > to)
            return false;
        }

        if (filters.minAmount && b.amount < Number(filters.minAmount))
          return false;
        if (filters.maxAmount && b.amount > Number(filters.maxAmount))
          return false;
    ...
    async (currentFilters: typeof filters, signal?: AbortSignal) => {
      try {
        ...
        if (currentFilters.reference)
          params.reference = currentFilters.reference;
        ...
        if (currentFilters.minAmount)
          params.minAmount = currentFilters.minAmount;
        if (currentFilters.maxAmount)
          params.maxAmount = currentFilters.maxAmount;
        ...
        const raw =
          ...
          (res.data?.bookings as AdminBooking[]) ||
          [];
        const normalized: Booking[] = raw.map((b) => {
          ...
          const kind: Booking["kind"] =
            typeRaw.includes("flight") || typeRaw.includes("air")
              ? "flight"
    ...
    <div className="container mx-auto py-8 relative">
        ...
        <div className="relative">
          ...
          <Input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              setFilters((prev) => ({ ...prev, quick: e.target.value }))
            }
            className="pl-12 h-12 bg-cyan-500/5 border-cyan-500/20 text-foreground placeholder:text-muted-foreground focus:border-cyan-500/40 focus:ring-cyan-500/20 rounded-xl"
          />
          {filters.quick && (
            <Button
              variant="ghost"
              ...
              className="absolute right-4 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-cyan-400"
            >
              <X className="h-4 w-4" />
            </Button>
          )}
      ...
      >
        ...
        <Button
          ...
          className="flex items-center gap-2 text-sm text-muted-foreground hover:text-cyan-400 transition-colors mb-4"
        >
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              <div className="rounded-xl border border-cyan-500/10 bg-gradient-to-br from-card/80 to-background/90 p-6">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  <div className="space-y-2">
                    <Label
                      htmlFor="reference"
                      className="text-muted-foreground text-xs uppercase tracking-wider font-medium"
                    >
                      Reference
                    </Label>
                    <Input
                      ...
                      className="bg-cyan-500/5 border-cyan-500/20 text-foreground placeholder:text-muted-foreground focus:border-cyan-500/40"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger className="bg-cyan-500/5 border-cyan-500/20 text-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="bg-cyan-500/5 border-cyan-500/20 text-foreground focus:border-cyan-500/40"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={filters.pnr}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setFilters((prev) => ({ ...prev, pnr: e.target.value }))
                      }
                      className="bg-cyan-500/5 border-cyan-500/20 text-foreground placeholder:text-muted-foreground focus:border-cyan-500/40"
                    />
                  </div>
                ...
                <div className="mt-6 flex items-center gap-3">
                  <Button
                    ...
                    onClick={handleReset}
                    disabled={loading}
                    className="border-cyan-500/20 text-cyan-400 hover:/5 hover:border-cyan-500/40 bg-transparent"
                  >
      ...
      <motion.div
        ...
        className="rounded-xl border border-cyan-500/10 bg-gradient-to-br from-card/80 to-background/90 overflow-hidden"
      >
        {loading ? (
