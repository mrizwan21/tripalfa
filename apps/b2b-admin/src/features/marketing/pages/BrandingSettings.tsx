...
import { Save, Upload, RefreshCw, Eye, Check, Copy, Palette, Type, Image } from 'lucide-react';
import api from '@/shared/lib/api';

// Color picker presets for quick selection
const COLOR_PRESETS = [
  // Blues
  { name: 'Ocean Blue', primary: '212 89% 53%', secondary: '210 40% 96%', accent: '25 95% 53%' },
  { name: 'Royal Purple', primary: '262 83% 58%', secondary: '270 50% 96%', accent: '320 85% 55%' },
  { name: 'Forest Green', primary: '142 76% 36%', secondary: '145 35% 92%', accent: '25 95% 53%' },
  { name: 'Sunset Orange', primary: '25 95% 53%', secondary: '30 100% 96%', accent: '350 85% 55%' },
  { name: 'Rose Pink', primary: '340 85% 55%', secondary: '350 100% 96%', accent: '262 83% 58%' },
  { name: 'Teal', primary: '173 80% 40%', secondary: '180 50% 94%', accent: '25 95% 53%' },
  { name: 'Slate', primary: '215 25% 27%', secondary: '210 20% 96%', accent: '25 95% 53%' },
  { name: 'Indigo', primary: '243 75% 59%', secondary: '245 60% 96%', accent: '320 75% 55%' },
];

// Schema for branding form
const brandingSchema = z.object({
  companyName: z.string().min(2, 'Company name is required'),
  primaryColor: z.string().regex(/^\d+\s+\d+%\s+\d+%$/, 'Use HSL format: hue saturation% lightness%'),
  primaryForeground: z.string().regex(/^\d+\s+\d+%\s+\d+%$/, 'Use HSL format'),
  secondaryColor: z.string().regex(/^\d+\s+\d+%\s+\d+%$/, 'Use HSL format'),
  secondaryForeground: z.string().regex(/^\d+\s+\d+%\s+\d+%$/, 'Use HSL format'),
  ...
  faviconUrl: z.string().url('Valid URL is required').optional().or(z.literal('')),
});
...
export default function BrandingSettings() {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--brand-primary-foreground', form.watch('primaryForeground'));
    root.style.setProperty('--brand-secondary', form.watch('secondaryColor'));
    root.style.setProperty('--brand-secondary-foreground', form.watch('secondaryForeground'));
    root.style.setProperty('--brand-accent', form.watch('accentColor'));
    root.style.setProperty('--brand-accent-foreground', form.watch('accentForeground'));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Marketing & Branding</h1>
                ...
                <form onSubmit={form.handleSubmit(handleSave)} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        onClick={() => copyToClipboard(form.watch('primaryColor'), 'primary')}
                      >
                        {copiedColor === 'primary' ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <div
                        className="h-10 w-10 rounded-lg border-2 border-slate-200"
                        style={{ backgroundColor: `hsl(${form.watch('secondaryColor')})` }}
                      />
                      <Input
                        id="secondaryColor"
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        onClick={() => copyToClipboard(form.watch('secondaryColor'), 'secondary')}
                      >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        onClick={() => copyToClipboard(form.watch('accentColor'), 'accent')}
                      >
                        {copiedColor === 'accent' ? (
                          <Check className="h-4 w-4 text-green-500" />
                        ) : (
        ...
        <TabsContent value="preview">
          <Card>
            ...
            <CardContent>
              ...
              >
                {/* Header Preview */}
                <div className="mb-8 flex items-center justify-between rounded-lg bg-slate-50 p-4">
                  <div className="flex items-center gap-4">
                    {form.watch('logoUrl') ? (
                      ...
                      <div 
                        className="flex h-8 items-center justify-center rounded font-bold text-white"
                        style={{ backgroundColor: 'hsl(var(--preview-primary))', padding: '0 1rem' }}
                      >
                ...
                <div className="mb-8 flex flex-wrap justify-center gap-4">
                  <Button
                    style={{
                      ...
                      color: 'hsl(var(--preview-primary-fg))',
                    }}
                  >
                    Primary Button
                  </Button>
                  <Button
                    ...
                    style={{
                      ...
                      color: 'hsl(var(--preview-secondary-fg))',
                    }}
                  >
                    Secondary Button
                  </Button>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {[1, 2, 3].map((i) => (
                    <div
                      ...
                      style={{ borderColor: 'hsl(var(--preview-primary) / 0.2)' }}
                    >
                      <div 
                        className="mb-2 h-2 w-12 rounded"
                        style={{ backgroundColor: 'hsl(var(--preview-primary))' }}
                      />
                      <h3 className="mb-1 font-semibold">Feature {i}</h3>
                ...
                <div className="mt-8 flex justify-center gap-2">
                  <Badge style={{ backgroundColor: 'hsl(var(--preview-primary))' }}>
                    New
                  </Badge>
                  <Badge style={{ backgroundColor: 'hsl(var(--preview-accent))' }}>
                    Popular
                  </Badge>
                </div>
              </div>
