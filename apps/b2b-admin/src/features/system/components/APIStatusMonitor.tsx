...
export const APIStatusMonitor: React.FC<APIStatusMonitorProps> = ({
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            default:
                return 'bg-slate-500/10 border-slate-200 dark:border-slate-900 text-slate-700 dark:text-slate-400';
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <button className="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-200 dark:border-green-800 rounded-xl p-4 hover:shadow-lg transition-all">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Healthy</span>
                        <CheckCircle2 className="w-5 h-5 text-green-500" />
                    </div>
                    <p className="text-3xl font-bold text-green-700 dark:text-green-400">{healthyCount}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">Endpoints operating normally</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 hover:shadow-lg transition-all">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Warning</span>
                        <AlertCircle className="w-5 h-5 text-yellow-500" />
                    </div>
                    <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-400">{warningCount}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">Degraded performance</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-red-500/10 to-red-600/5 border border-red-200 dark:border-red-800 rounded-xl p-4 hover:shadow-lg transition-all">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Error</span>
                        <AlertCircle className="w-5 h-5 text-red-500" />
                    </div>
                    <p className="text-3xl font-bold text-red-700 dark:text-red-400">{errorCount}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">Endpoints down</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-200 dark:border-blue-800 rounded-xl p-4 hover:shadow-lg transition-all">
                    ...
                    <p className="text-3xl font-bold text-blue-700 dark:text-blue-400">{endpoints.length}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">Monitored endpoints</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-slate-700 dark:text-slate-300">Auto-refresh enabled (every 30s)</span>
                </label>
            </div>
            ...
            <div className="space-y-3">
                {endpoints.map((endpoint) => (
                    <button
                        ...
                        className={`w-full group text-left bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:border-slate-300 dark:hover:border-slate-600 ${selectedEndpoint === endpoint ? 'ring-2 ring-blue-500' : ''
                            }`}
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <code className="text-sm text-slate-600 dark:text-slate-400 truncate">
                                        {endpoint.path}
                                    </code>
                                    <span
                            ...
                            <div className="h-32 flex items-end gap-1 bg-gradient-to-t from-blue-500/10 to-transparent rounded-lg p-4">
                                {Array.from({ length: 12 }).map((_, i) => {
                                    ...
                                    return (
                                        <div
                                            key={i}
                                            className="flex-1 bg-gradient-to-t from-blue-500 to-blue-400 rounded-t opacity-70 hover:opacity-100 transition-all"
                                            style={{ height: `${height}%` }}
                                        />
                                    );
                                })}
                            ...
                            <div className="h-32 flex items-end gap-1 bg-gradient-to-t from-red-500/10 to-transparent rounded-lg p-4">
                                {Array.from({ length: 12 }).map((_, i) => {
                                    ...
                                    return (
                                        <div
                                            key={i}
                                            className="flex-1 bg-gradient-to-t from-red-500 to-red-400 rounded-t opacity-70 hover:opacity-100 transition-all"
                                            style={{ height: `${height}%` }}
                                        />
                                    );
                                })}
