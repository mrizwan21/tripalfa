...
describe('RulePermissionUtils', () => {
  ...
  describe('Basic Permission Checks', () => {
    test('should check if user has a specific permission', () => {
      expect(hasPermission(testUserPermissions, RULE_PERMISSIONS.MARKUP_VIEW)).toBe(true);
      expect(hasPermission(testUserPermissions, RULE_PERMISSIONS.MARKUP_DELETE)).toBe(false);
    });

    test('should check if user has any of the required permissions', () => {
      expect(hasAnyPermission(testUserPermissions, [
    ...
    test('should check if user has all required permissions', () => {
      expect(hasAllPermissions(testUserPermissions, [
        RULE_PERMISSIONS.MARKUP_VIEW,
        RULE_PERMISSIONS.COMMISSION_VIEW
      ])).toBe(true);
    ...
    test('should check permissions with detailed result', () => {
      const result1 = checkPermissions(testUserPermissions, [
        RULE_PERMISSIONS.MARKUP_VIEW,
        RULE_PERMISSIONS.MARKUP_DELETE
      ], true);

      expect(result1.hasPermission).toBe(false);
      expect(result1.missingPermissions).toContain(RULE_PERMISSIONS.MARKUP_DELETE);

      const result2 = checkPermissions(testUserPermissions, [
        RULE_PERMISSIONS.MARKUP_VIEW,
        RULE_PERMISSIONS.MARKUP_DELETE
      ], false);
  ...
  describe('Category and Operation Checks', () => {
    ...
    test('should check if user can manage a specific category', () => {
      ...
      expect(canManageCategory(testUserPermissions, 'airline_deal')).toBe(false);
    });
  });
  ...
  describe('Permission Validation', () => {
    ...
    test('should check rule operation permissions', () => {
      ...
      const result2 = checkRuleOperation(testUserPermissions, 'markup', 'delete');
  ...
  describe('Utility Functions', () => {
    ...
    test('should get permissions grouped by category', () => {
      ...
      expect(permissionsByCategory.markup).toContain(RULE_PERMISSIONS.MARKUP_VIEW);
      expect(permissionsByCategory.commission).toContain(RULE_PERMISSIONS.COMMISSION_VIEW);
      expect(permissionsByCategory.coupon).toContain(RULE_PERMISSIONS.COUPON_VIEW);
      expect(permissionsByCategory.airline_deal).toContain(RULE_PERMISSIONS.AIRLINE_DEAL_VIEW);
    });

    test('should get permissions grouped by operation', () => {
      const permissionsByOperation = getPermissionsByOperation();
      
      expect(permissionsByOperation.view).toContain(RULE_PERMISSIONS.MARKUP_VIEW);
      expect(permissionsByOperation.create).toContain(RULE_PERMISSIONS.MARKUP_CREATE);
      expect(permissionsByOperation.edit).toContain(RULE_PERMISSIONS.COUPON_EDIT);
      expect(permissionsByOperation.manage).toContain(RULE_PERMISSIONS.RULE_AUDIT);
    });
  });
});
