...
const BOOKING_SERVICE_URL = process.env.BOOKING_SERVICE_URL || 'http://localhost:3001/bookings';
...
router.get('/user/:userId', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      else if (invResp.data && invResp.data.html) html = invResp.data.html;
    } catch (err) {
    ...
    try {
      ...
      console.error('Puppeteer not available or failed to generate PDF:', err && err.message ? err.message : err);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { totalAmount, currency, dueDate } = req.body;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const invoice = invoices.find(i => i.id === parseInt(id as string));
...
// Add queue action for a booking
router.post(
  '/:id/queue',
  async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { queueType, processedBy } = req.body;
      ...
      mockAuditLogs.push({
        id: mockAuditLogs.length + 1,
        action: `queue-action:${queueType}`,
        performedBy: processedBy || 'system',
        ts: new Date().toISOString(),
        details: { queueId: newQueue.id },
      });

      res.status(201).json(newQueue);
    } catch (error) {
      console.error('Error adding queue action:', error);
      res.status(500).json({ error: 'Failed to add queue action' });
    }
  }
);
