        ...
        <div className="flex gap-2">
          {(['week', 'month', 'quarter'] as const).map(range => (
            <button
              ...
              className={
                dateRange === range ? 'filter-chip-active capitalize' : 'filter-chip capitalize'
              }
            >
              {range}
            </button>
      ...
      <div className="data-grid">
        <StatCard
          ...
          change={
            metrics
              ? Math.round(
                  ((metrics.contacts.newThisMonth - metrics.contacts.newLastMonth) /
                    Math.max(metrics.contacts.newLastMonth, 1)) *
                    100
                )
              : 0
          }
          changeType={
            metrics && metrics.contacts.newThisMonth > metrics.contacts.newLastMonth
              ? 'positive'
              : 'negative'
          }
        />
