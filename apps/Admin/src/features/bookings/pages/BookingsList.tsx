    ...
    (records: Booking[]) => {
      return records.filter(b => {
        if (
          ...
          return false;
        if (filters.email && !b.userEmail.toLowerCase().includes(filters.email.toLowerCase()))
          return false;
        ...
        if (
          filters.pnr &&
          !(b.pnr || b.reference).toLowerCase().includes(filters.pnr.toLowerCase())
        )
          return false;
        if (filters.phone && !(b.phone ?? '').toLowerCase().includes(filters.phone.toLowerCase()))
          return false;
        if (
          filters.supplier &&
        ...
        if (
          filters.channel &&
          !(b.channel ?? '').toLowerCase().includes(filters.channel.toLowerCase())
        )
          return false;
  ...
  const fetchBookings = useCallback(
    async (currentFilters: typeof filters, signal?: AbortSignal) => {
      try {
        ...
        const raw =
          (res.data?.data as AdminBooking[]) || (res.data?.bookings as AdminBooking[]) || [];
        const normalized: Booking[] = raw.map(b => {
          const typeRaw = String(
            b.type ?? b.productType ?? b.bookingType ?? b.category ?? b.kind ?? ''
          ).toLowerCase();
          ...
          return {
            id: String(
              b.id ?? b.bookingRef ?? b.reference ?? `tmp-${Math.random().toString(36).slice(2)}`
            ),
