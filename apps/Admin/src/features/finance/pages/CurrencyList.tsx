...
import { RefreshCw, Settings2, Star, MoreHorizontal, Pencil } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const columns: ColumnDef<CurrencyRecord>[] = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isBase && <Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400" />}
            </div>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="font-mono font-semibold text-indigo-600">{effectiveRate.toFixed(6)}</span>
      );
    },
  },
  {
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="bg-card rounded-lg border border-border p-6 mb-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Configure buffer percentages for ROE fluctuations, decimal precision, and rounding modes
          for each currency. The effective rate is calculated as:{' '}
          <code className="bg-muted px-1 py-0.5 rounded">Base Rate × (1 + Buffer %)</code>
        </p>
      </div>
