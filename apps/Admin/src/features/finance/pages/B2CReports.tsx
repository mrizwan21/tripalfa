...
type BookingTrend = Record<string, any>;
...
export const b2cColumns: ColumnDef<B2CCustomerReport>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className="font-mono font-semibold">${spent.toLocaleString()}</span>;
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="font-mono font-semibold text-indigo-600">${clv.toLocaleString()}</span>
      );
    },
  },
  {
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {customerAnalytics?.totalCustomers || totalCustomers}
            </div>
            <p className="text-xs text-muted-foreground flex items-center mt-1 gap-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-indigo-600">
              ${(customerAnalytics?.averageCustomerValue || avgCustomerValue).toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground flex items-center mt-1 gap-2">
              <TrendingUp className="h-3 w-3 text-green-500 mr-1" />
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {customerAnalytics?.returningCustomers || returningCustomers}
            </div>
            <p className="text-xs text-muted-foreground flex items-center mt-1 gap-2">
              <TrendingDown className="h-3 w-3 text-red-500 mr-1" />
              {(
                ((customerAnalytics?.returningCustomers || returningCustomers) / totalCustomers) *
                100
              ).toFixed(1)}
              % retention rate
            </p>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
                ...
                <div key={trend.date} className="flex-1 flex flex-col items-center gap-1 group">
                  ...
                  >
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-foreground text-background text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                      $
                      {trend.revenue.toLocaleString(undefined, {
          ...
          <div className="flex justify-between mt-4 text-xs text-muted-foreground gap-4">
            ...
            <div className="flex items-center justify-center h-full gap-2">
              <div className="text-center">
                <div className="text-4xl font-bold text-indigo-600">
                  {customerAnalytics?.bookingFrequency ||
                    (totalBookings / totalCustomers).toFixed(1)}
                </div>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable columns={b2cColumns} data={data} searchKey="customerName" />
        </CardContent>
      </Card>
    </div>
  );
