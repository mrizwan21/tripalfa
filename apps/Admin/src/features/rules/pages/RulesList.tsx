...
import type { ActionType, ConditionOperator, CreateRuleRequest, Rule } from '../types-rule-engine';
...
export default function RulesPage() {
  ...
  const [formCategory, setFormCategory] = useState<RulesManagerRule['type']>('markup');
  ...
  const mapRule = useCallback((rule: Rule): RulesManagerRule => {
    ...
    const normalizedStatus: RulesManagerRule['status'] =
      statusValue === 'active' ? 'active' : statusValue === 'draft' ? 'testing' : 'inactive';

    const normalizedType: RulesManagerRule['type'] =
      typeValue === 'commission' || typeValue === 'discount' || typeValue === 'pricing'
        ? typeValue
        : 'markup';
    ...
    const executionCount = Number((rule as { executionCount?: number }).executionCount || 0);
  ...
  const openDialog = useCallback(
    (mode: 'create' | 'edit' | 'duplicate', rule?: RulesManagerRule) => {
      ...
      if (mode === 'create') {
        ...
        setFormDescription(rule.description === 'No description available' ? '' : rule.description);
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Select
                value={formCategory}
                onValueChange={value => setFormCategory(value as RulesManagerRule['type'])}
                disabled={isBusy}
              >
              ...
              <p className="text-xs text-muted-foreground">{categoryPresets[formCategory].help}</p>
              <p className="text-xs text-muted-foreground">
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                {formDescription.trim().length}/{descriptionMaxLength} characters
              </p>
            </div>
          </div>
