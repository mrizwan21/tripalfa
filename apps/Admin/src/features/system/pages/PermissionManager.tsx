...
function getDefaultRolePermissions(): Record<RoleName, string[]> {
  const modulePermissions = routeConfig
    ...
    .filter((permission, index, arr) => permission && arr.indexOf(permission) === index);

  return {
    ADMIN: modulePermissions,
    B2B: modulePermissions.filter(permission => !permission.includes(':manage')),
    VIEWER: modulePermissions.filter(permission => permission.includes(':view')),
  };
}
...
export default function PermissionManager() {
  ...
  const modules = useMemo<ModulePermission[]>(() => {
    return routeConfig
      .filter(route => route.path !== '/' && route.permissions && route.permissions.length > 0)
      .map(route => ({
        module: route.label,
        path: route.path,
  ...
  const hasPermission = (permission: string) => rolePermissions[selectedRole].includes(permission);
  ...
  return (
    <div className="container mx-auto py-8 space-y-6">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Permission Manager</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Assign module-level view/edit access by role. Changes apply across all B2B admin
            modules.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="space-y-0 gap-2">
          ...
          <CardDescription>Choose which access level to configure.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="space-y-0 gap-2">
          ...
          <CardDescription>
            Toggle permissions for each module. Manage/edit access is controlled by `:manage`
            permissions.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 p-6">
          {modules.map(module => {
            const moduleEnabled = module.permissions.every(permission => hasPermission(permission));

            return (
              <div key={module.path} className="rounded-lg border p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <p className="font-semibold text-foreground">{module.module}</p>
                    <p className="text-xs text-muted-foreground">{module.path}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary">{module.permissions.length} permissions</Badge>
                    <Switch
                      checked={moduleEnabled}
                      onCheckedChange={value => setModulePermissions(module.permissions, value)}
                    />
                  </div>
                </div>
                  ...
                  {module.permissions.map(permission => (
                    ...
                    >
                      ...
                      <Switch
                        ...
                        onCheckedChange={value => setPermission(permission, value)}
                      />
                    </div>
                  ))}
                </div>
