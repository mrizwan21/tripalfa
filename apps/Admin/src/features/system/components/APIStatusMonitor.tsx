  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">API Status Monitor</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-200 dark:border-green-800 rounded-xl p-4 hover:shadow-lg transition-all">
          <div className="flex items-center justify-between mb-2 gap-2">
            <span className="text-sm font-semibold text-muted-foreground">Healthy</span>
            <CheckCircle2 className="w-5 h-5 text-green-500" />
          </div>
          <p className="text-3xl font-bold text-green-700 dark:text-green-400">{healthyCount}</p>
          <p className="text-xs text-muted-foreground mt-2">Endpoints operating normally</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 hover:shadow-lg transition-all">
          <div className="flex items-center justify-between mb-2 gap-2">
            <span className="text-sm font-semibold text-muted-foreground">Warning</span>
            <AlertCircle className="w-5 h-5 text-yellow-500" />
          </div>
          <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-400">{warningCount}</p>
          <p className="text-xs text-muted-foreground mt-2">Degraded performance</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-200 dark:border-blue-800 rounded-xl p-4 hover:shadow-lg transition-all">
          ...
          <p className="text-xs text-muted-foreground mt-2">Monitored endpoints</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        ...
        <Label className="flex items-center gap-2 cursor-pointer text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          ...
          />
          <span className="text-sm text-muted-foreground">Auto-refresh enabled (every 30s)</span>
        </Label>
      </div>
        ...
        {endpoints.map(endpoint => (
          ...
          >
            <div className="flex items-start gap-4">
              {/* Status */}
              <div className="flex-shrink-0 pt-1 gap-4">{getStatusIcon(endpoint.status)}</div>

              {/* Info */}
              <div className="flex-1 min-w-0 gap-4">
                <div className="flex items-center gap-2 mb-2">
                  <code className="text-sm font-semibold text-foreground">{endpoint.method}</code>
                  <code className="text-sm text-muted-foreground truncate">{endpoint.path}</code>
                  <span
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">Uptime</p>
                    <p className="text-sm font-medium text-foreground mt-1">
                      {endpoint.uptime.toFixed(2)}%
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">24h Req</p>
                    <p className="text-sm font-medium text-foreground mt-1">
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button variant="outline" size="default" className="p-2 hover:bg-muted rounded-lg">
      ...
      {selectedEndpoint && (
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Error Rate Chart */}
            <div>
              <h4 className="text-sm font-semibold text-muted-foreground mb-4">Error Rate Trend</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="p-4 rounded-lg bg-muted">
              <p className="text-xs font-semibold text-muted-foreground uppercase">Min Response</p>
            ...
            <div className="p-4 rounded-lg bg-muted">
              <p className="text-xs font-semibold text-muted-foreground uppercase">Max Response</p>
              <p className="text-2xl font-bold text-foreground mt-2">
