...
import { addSupplierItem } from '../utils/listUtils';
...
export default function SuppliersPage({
  ...
  const loadSuppliers = async () => {
    ...
    try {
      ...
      const payload: any[] = Array.isArray(res.data)
        ? res.data
      ...
      setSuppliers(
        payload.map(supplier => {
          ...
          const type =
            category.toLowerCase().includes('flight') || category.toLowerCase().includes('gds')
              ? 'flight'
              : 'hotel';
          return {
            id: supplier.id,
            ...
            description:
              supplier.vendor?.name || supplier.code || supplier.description || 'Supplier profile',
          };
        })
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadSupplierResources = async () => {
      try {
        ...
        const productsPayload: any[] = Array.isArray(productsRes.data)
          ? productsRes.data
        ...
        const rulesPayload: any[] = Array.isArray(rulesRes.data)
          ? rulesRes.data
          : (rulesRes.data?.data ?? []);
        setRules(
          rulesPayload.map(item => ({
        ...
        const financePayload: any[] = Array.isArray(financeRes.data)
          ? financeRes.data
          : (financeRes.data?.data ?? []);
        setPayments(
          financePayload.map(item => ({
        ...
        const documentsPayload: any[] = Array.isArray(documentsRes.data)
          ? documentsRes.data
        ...
        const credentialsPayload: any[] = Array.isArray(credentialsRes.data)
          ? credentialsRes.data
          : (credentialsRes.data?.data ?? []);
        setApiCredentials(
          credentialsPayload.map(item => ({
  ...
  const handleCreateSupplier = async (values: z.infer<typeof profileSchema>) => {
    const slug = values.supplierName
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
    const code = values.supplierIdCode?.trim() || slug.toUpperCase().replace(/-/g, '_');
    const category =
      values.supplierType === 'GDS' || values.supplierType === 'Direct' ? 'FLIGHT' : 'HOTEL';
    try {
  ...
  const handleAddProduct = productForm.handleSubmit(async values => {
  ...
  const handleAddRule = ruleForm.handleSubmit(async values => {
    await addSupplierItem(
      selectedSupplierId,
      'rules',
      values,
  ...
  const handleAddDocument = documentForm.handleSubmit(async values => {
    await addSupplierItem(
      selectedSupplierId,
      'documents',
      values,
  ...
  const handleAddApiCredential = apiForm.handleSubmit(async values => {
    await addSupplierItem(
      selectedSupplierId,
      'credentials',
      values,
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="sm:max-w-4xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Capture supplier onboarding details and configuration.
              </DialogDescription>
            </DialogHeader>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                  ...
                  <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
                    <div className="space-y-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <div className="flex items-center justify-between rounded-md border border-dashed p-3 gap-2">
                              <div>
                                <p className="text-sm font-medium">Active supplier</p>
                                <p className="text-xs text-muted-foreground">
                                  Enable to allow bookings and inventory sync.
                                </p>
                              </div>
                              <FormControl>
                                <Switch
                    ...
                    <div className="space-y-4">
                      <div className="rounded-lg border border-dashed p-4 text-center space-y-3">
                        ...
                        <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-md border border-dashed text-muted-foreground gap-4">
                        ...
                        />
                        <p className="text-xs text-muted-foreground">
                          Max size 800 x 800. JPG, PNG, GIF.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="rounded-lg border">
                  ...
                  {products.length === 0 ? (
                    ...
                    products.map(row => (
                      <div
                        key={row.id}
                        className="grid grid-cols-5 gap-2 border-b px-4 py-3 text-sm last:border-b-0"
                      >
                        <span>{row.name}</span>
                ...
                <div className="rounded-lg border">
                  ...
                  {rules.length === 0 ? (
                    ...
                    rules.map(row => (
                      <div
                        key={row.id}
                        className="grid grid-cols-6 gap-2 border-b px-4 py-3 text-sm last:border-b-0"
                      >
                        <span>{row.name}</span>
                ...
                <div className="rounded-lg border">
                  ...
                  {payments.length === 0 ? (
                    ...
                    payments.map(row => (
                      <div
                        key={row.id}
                        className="grid grid-cols-7 gap-2 border-b px-4 py-3 text-sm last:border-b-0"
                      >
              ...
              <TabsContent value="document" className="space-y-6">
                <div className="grid gap-6 lg:grid-cols-2">
                  <Form {...documentForm}>
                    <div className="rounded-lg border p-4 space-y-4">
                      <div className="rounded-md bg-muted/40 px-3 py-2 text-sm font-semibold">
                        Add Documents
                      </div>
                      <FormField
                      ...
                      <div className="flex flex-col items-center justify-center rounded-md border border-dashed p-6 text-center text-muted-foreground gap-4">
                        ...
                        documents.map(doc => (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                Issue {doc.issueDate || '-'} • Expiry {doc.expiryDate || '-'}
                              </p>
                            </div>
                            <Button size="sm" variant="ghost">
                              Remove
                  ...
                  <div className="rounded-lg border p-4 space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-[2fr_auto]">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input
                                placeholder="http://api.hotelbeds.com/hotel-api/1.0/status"
                                {...field}
                              />
                            </FormControl>
                            <FormMessage />
                      ...
                      <div className="flex items-end gap-4">
                        <Button className="h-9" onClick={handleAddApiCredential}>
                          Add
                        </Button>
                      </div>
                    </div>
                      ...
                      {apiCredentials.length === 0 ? (
                        ...
                        apiCredentials.map(cred => (
                          ...
                          >
                            ...
                            <Button size="sm" className="text-white hover:">
                              Remove
                            </Button>
                          </div>
                        ))
            ...
            <DialogFooter>
              ...
              <Button onClick={profileForm.handleSubmit(handleCreateSupplier)} disabled={isSaving}>
