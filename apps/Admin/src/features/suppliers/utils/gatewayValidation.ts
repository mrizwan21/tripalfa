...
const endpointSchema = z.object({
  ...
  timeout: z.number().min(1000, 'Minimum 1 second').max(60000, 'Maximum 60 seconds'),
...
const geographyRoutingSchema = z.object({
...
export const validateEnvironmentConfig = (data: unknown, environment: Environment) => {
...
const validateAuthentication = (data: unknown) => {
  return authenticationSchema.safeParse(data);
};

const validateGeographyRouting = (data: unknown) => {
  return geographyRoutingSchema.safeParse(data);
};

const validateChannelRouting = (data: unknown) => {
