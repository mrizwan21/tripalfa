...
const SystemMonitoring: React.FC = () => {
  ...
  const fetchServiceHealth = async () => {
    ...
    try {
      const serviceStatuses: ServiceStatus[] = await Promise.all(
        SERVICES.map(async (service) => {
          try {
            ...
            const requestRate = parseFloat(
              rateData.data?.result?.[0]?.value?.[1] || "0",
            );
            ...
            const errorRate = parseFloat(
              errorData.data?.result?.[0]?.value?.[1] || "0",
            );
      ...
      setError(
        "Failed to fetch metrics from Prometheus. Ensure Prometheus is running.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <CardContent>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <Box>
                <Typography variant="h4">
                  {services
                    .reduce((acc, s) => acc + s.requestRate, 0)
                    .toFixed(1)}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Requests/sec
        ...
        <Card>
          <CardContent>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box
                sx={{
                  color: services.some((s) => s.circuitBreakerState === "open")
                    ? "error.main"
                    : "success.main",
                }}
              >
              ...
              <Box>
                <Typography variant="h4">
                  {
                    services.filter((s) => s.circuitBreakerState === "open")
                      .length
                  }
                </Typography>
                <Typography variant="body2" color="text.secondary">
          ...
          <CardContent>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box
                sx={{
                  color:
                    services.reduce((acc, s) => acc + s.errorRate, 0) /
                      (services.length || 1) >
                    5
                      ? "error.main"
                      : "primary.main",
          ...
          >
            ...
            <IconButton
              onClick={() =>
                window.open(
                  GRAFANA_DASHBOARDS[
                    selectedDashboard as keyof typeof GRAFANA_DASHBOARDS
                  ],
                  "_blank",
                )
              }
              ...
              <CardContent>
                <Box sx={{ textAlign: "center" }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    ...
                    {service.circuitBreakerState === "half-open" &&
                      "Testing recovery"}
                  </Typography>
                </Box>
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      await fetch(
                        `/api/admin/circuit-breaker/${service.name}/reset`,
                        { method: "POST" },
                      );
                      fetchServiceHealth();
                    }}
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      await fetch(
                        `/api/admin/circuit-breaker/${service.name}/trip`,
                        { method: "POST" },
                      );
                      fetchServiceHealth();
                    }}
