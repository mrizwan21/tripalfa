...
class NotificationService {
   ...
   */
  async sendNotification(request: CreateNotificationRequest): Promise<SendNotificationResponse> {
    try {
      const response = await apiManager.post<any>(`${this.apiBaseUrl}/notifications/send`, request);
   ...
   * Uses centralized APIManager with caching support
   */
  async listNotifications(limit: number = 50, offset: number = 0): Promise<Notification[]> {
    try {
      const response = await apiManager.get<Notification[]>(`${this.apiBaseUrl}/notifications`, {
        params: { limit, offset },
      });

      if (!response.success) {
        throw new Error(response.error?.message || 'Failed to list notifications');
      }
