...
cd /Users/mohamedrizwan/Desktop/TripAlfa\ -\ Node/database/migrations
...
cd /Users/mohamedrizwan/Desktop/TripAlfa\ -\ Node/database/prisma

# Apply migration (if using Prisma Migrate)
npx prisma migrate deploy --schema schema.finance.prisma

# Note: This requires the migration to be in the migrations folder
# tracked by Prisma. For manual SQL, use Method 1.
```
...
async function runMigration() {
  ...
  try {
    const sql = fs.readFileSync(
      path.join(__dirname, '../migrations/20260331000001_add_crm_enhancements.sql'),
      'utf8'
    );
...
- `services/crm-service/src/database.ts` uses `getFinanceDb()`
...
```bash
cd database/prisma
npx prisma generate --schema schema.finance.prisma
```
...
- `docs/CRM_TABLES_ENHANCEMENT_SUMMARY.md`
