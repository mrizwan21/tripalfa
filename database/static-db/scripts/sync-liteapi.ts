...
const HOTELS_DETAIL_LIMIT = Number(
  process.env.LITEAPI_HOTEL_DETAIL_LIMIT ?? 50,
...
interface LiteBedType {
  quantity: number;
  bedType: string;
  bedSize: string;
  Id?: number;
}
interface LiteRoomAmenity {
  amenitiesId: number;
  name: string;
  sort: number;
}
interface LiteRoomPhoto {
  url: string;
  urlHd?: string;
...
interface LiteAccessibility {
  [key: string]: unknown;
}
interface LiteSentimentCategory {
  name: string;
  rating: number;
  description: string;
}
interface LiteSentimentAnalysis {
  pros?: string[];
  cons?: string[];
  categories?: LiteSentimentCategory[];
...
interface LiteAPIDetailResponse<T> {
  data: T;
}
...
async function updateProgress(
  ...
  await query(
    ...
    [
      cc,
      status,
      details?.hotels_count ?? 0,
      details?.details_count ?? 0,
      details?.started_at ?? null,
    ],
  );
}
...
async function upsertCountries(client: AxiosInstance): Promise<string[]> {
  ...
  // Check cache first
  let countries: LiteCountry[] | null =
    cache.get<LiteCountry[]>("/data/countries");
...
async function upsertCurrencies(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteCurrency>>(
    client,
    "/data/currencies",
  );
  let count = 0;
...
async function upsertLanguages(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteLanguage>>(
    client,
    "/data/languages",
  );
  let count = 0;
...
async function upsertChains(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteChain>>(
    client,
    "/data/chains",
  );
  let count = 0;
...
async function upsertHotelTypes(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteHotelType>>(
    client,
    "/data/hotelTypes",
  );
  let count = 0;
...
async function upsertFacilities(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteFacility>>(
    client,
    "/data/facilities",
  );
  let count = 0;
  for (const f of resp.data) {
    ...
    await query(
      ...
      [
        id,
        name,
        JSON.stringify(Object.keys(translations).length ? translations : null),
      ],
    );
    count++;
  }
...
async function upsertIataCodes(client: AxiosInstance): Promise<void> {
  ...
  const resp = await get<LiteAPIListResponse<LiteIataCode>>(
    client,
    "/data/iataCodes",
  );
  let count = 0;
  for (const a of resp.data) {
    await query(
      ...
      [
        ...
        a.latitude ?? null,
        a.longitude ?? null,
        a.countryCode?.toUpperCase() ?? null,
      ],
    );
    count++;
  }
...
async function upsertCities(
  ...
  await Promise.all(
    countryCodes.map((cc) =>
      limit(async () => {
        try {
          // Check cache first
          let cityData: LiteCity[] | null = cache.get<LiteCity[]>(
            "/data/cities",
            { countryCode: cc },
          );
          ...
          if (!cityData) {
            const resp = await withRetry(
              () =>
                get<LiteAPIListResponse<LiteCity>>(client, "/data/cities", {
                  countryCode: cc,
                }),
              `Fetch cities for ${cc}`,
              { maxAttempts: 2 },
          ...
          log.warn(
            `Could not fetch cities for ${cc}: ${(err as Error).message}`,
          );
        }
      }),
    ),
  ...
  log.success(
    `Cities: upserted ${totalCities} across ${countryCodes.length} countries`,
  );
}
...
async function insertHotelRecord(h: LiteHotelListItem): Promise<void> {
  // Sanitize all string fields to prevent UTF8 encoding errors
  const sanitizedName = sanitizeString(h.name) || "Unknown";
  const sanitizedDesc = sanitizeString(h.hotelDescription);
  const sanitizedCity = sanitizeString(h.city);
  const sanitizedAddress = sanitizeString(h.address);
  const sanitizedZip = sanitizeString(h.zip);
  const sanitizedPhoto = sanitizeString(h.main_photo);
  const sanitizedThumb = sanitizeString(h.thumbnail);
  ...
  await query(
    ...
    [
      ...
      h.rating ?? null,
      h.reviewCount ?? 0,
      h.accessibilityAttributes
        ? JSON.stringify(h.accessibilityAttributes)
        : null,
      h.rohId ?? null,
      !!h.deletedAt,
...
async function mapHotelFacilities(
  hotelId: string,
  facilityIds: number[],
): Promise<void> {
...
async function upsertHotelList(
  ...
  while (hasMore) {
    try {
      ...
      let hotelsData: LiteHotelListItem[] | null = cache.get<
        LiteHotelListItem[]
      >("/data/hotels", cacheKey);
      ...
      const newHotels = hotelsData.filter(
        (h) => !seenIds.has(h.id) && validateHotel(h),
      );
      if (newHotels.length === 0) {
        log.info(
          `${countryCode}: page ${pageNum} returned all duplicates — pagination complete (${hotelIds.length} hotels)`,
        );
        hasMore = false;
        break;
      }
      ...
      if (pageNum % 5 === 0) {
        log.info(
          `${countryCode}: page ${pageNum}, ${hotelIds.length} hotels so far...`,
        );
      }

      if (
        MAX_HOTELS_PER_COUNTRY > 0 &&
        hotelIds.length >= MAX_HOTELS_PER_COUNTRY
      ) {
        hasMore = false;
        ...
        log.warn(
          `${countryCode}: hit max page limit (${MAX_PAGES}), stopping at ${hotelIds.length} hotels`,
        );
        hasMore = false;
      } else {
      ...
      log.warn(
        `Error fetching hotels offset ${offset} for ${countryCode}: ${(err as Error).message}`,
      );
      hasMore = false;
    }
  }
...
async function upsertHotelDetail(
  ...
  try {
    // Check cache first
    let detailData: LiteHotelDetail | null = cache.get<LiteHotelDetail>(
      "/data/hotel",
      { hotelId },
    );
  ...
  try {
    // Sanitize detail fields before update
    const sanitizedInfo = sanitizeString(detail.hotelImportantInformation);
    const sanitizedCheckinStart = sanitizeString(
      detail.checkinCheckoutTimes?.checkin_start,
    );
    const sanitizedCheckinEnd = sanitizeString(
      detail.checkinCheckoutTimes?.checkin_end,
    );
    const sanitizedCheckout = sanitizeString(
      detail.checkinCheckoutTimes?.checkout,
    );
    const sanitizedSpecialInstructions = sanitizeString(
      detail.checkinCheckoutTimes?.special_instructions,
    );
    const sanitizedPhone = sanitizeString(detail.phone);
    const sanitizedFax = sanitizeString(detail.fax);
    const sanitizedEmail = sanitizeString(detail.email);
    const sanitizedVideoUrl = sanitizeString(detail.videoUrl);
    const sanitizedAirportCode = sanitizeString(detail.airportCode);
    ...
    await query(
      ...
      [
        detail.id,
        sanitizedInfo,
        sanitizedCheckinStart,
        sanitizedCheckinEnd,
        sanitizedCheckout,
        detail.checkinCheckoutTimes?.instructions
          ? JSON.stringify(detail.checkinCheckoutTimes.instructions)
          : null,
    ...
    if (detail.hotelImages?.length) {
      ...
      for (const img of detail.hotelImages) {
        try {
          const sanitizedUrl = sanitizeString(img.url);
          const sanitizedUrlHd = sanitizeString(img.urlHd);
          const sanitizedCaption = sanitizeString(img.caption);

          if (!sanitizedUrl) continue; // Skip images without URL
          ...
          log.warn(
            `Failed to insert image for hotel ${detail.id}: ${(err as Error).message}`,
          );
        }
      }
    }
    ...
    if (detail.facilities?.length) {
      for (const f of detail.facilities) {
        try {
          const sanitizedFacilityName =
            sanitizeString(f.name) || "Unknown Facility";

          await query(
            `INSERT INTO hotel.facilities (id, name) VALUES ($1, $2)
             ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, updated_at = NOW()`,
            [f.facilityId, sanitizedFacilityName],
          );
          await query(
            `INSERT INTO hotel.hotel_facility_map (hotel_id, facility_id) VALUES ($1, $2)
             ON CONFLICT DO NOTHING`,
          ...
          log.warn(
            `Failed to insert facility for hotel ${detail.id}: ${(err as Error).message}`,
          );
        }
      }
    }
    ...
    if (detail.policies?.length) {
      for (const p of detail.policies) {
        try {
          const sanitizedPolicyName = sanitizeString(p.name);
          const sanitizedPolicyDesc = sanitizeString(p.description);
          ...
          log.warn(
            `Failed to insert policy for hotel ${detail.id}: ${(err as Error).message}`,
          );
        }
      }
    }
      ...
      try {
        ...
        await query(
          ...
          [
            ...
            detail.sentiment_updated_at
              ? new Date(detail.sentiment_updated_at)
              : new Date(),
          ],
        );
      } catch (err) {
        log.warn(
          `Failed to insert sentiment for hotel ${detail.id}: ${(err as Error).message}`,
        );
      }
    }
    ...
    if (detail.rooms?.length) {
      for (const room of detail.rooms) {
        try {
          const sanitizedRoomName = sanitizeString(room.roomName);
          const sanitizedRoomDesc = sanitizeString(room.description);
          const sanitizedRoomSizeUnit =
          ...
          await query(
            ...
            [
              ...
              sanitizedRoomName,
              sanitizedRoomDesc,
              room.roomSizeSquare ?? null,
              sanitizedRoomSizeUnit,
              room.maxAdults ?? null,
          ...
          if (room.bedTypes?.length) {
            await query(`DELETE FROM hotel.room_bed_types WHERE room_id = $1`, [
              room.id,
            ]);
            for (const bt of room.bedTypes) {
              try {
                const sanitizedBedType = sanitizeString(bt.bedType);
                ...
                log.warn(
                  `Failed to insert bed type for room ${room.id}: ${(err as Error).message}`,
                );
              }
            }
          }
          ...
          if (room.roomAmenities?.length) {
            for (const a of room.roomAmenities) {
              try {
                ...
                log.warn(
                  `Failed to insert room amenity for room ${room.id}: ${(err as Error).message}`,
                );
              }
            }
          }
          ...
          if (room.photos?.length) {
            await query(`DELETE FROM hotel.room_photos WHERE room_id = $1`, [
              room.id,
            ]);
            for (const p of room.photos) {
              try {
                const sanitizedUrl = sanitizeString(p.url);
                const sanitizedHdUrl = sanitizeString(p.hd_url ?? p.urlHd);
                const sanitizedImageDesc = sanitizeString(p.imageDescription);
                const sanitizedImageClass1 = sanitizeString(p.imageClass1);
                const sanitizedImageClass2 = sanitizeString(p.imageClass2);
                const sanitizedFailoverUrl = sanitizeString(p.failoverPhoto);

                if (!sanitizedUrl) continue; // Skip photos without URL
                ...
                await query(
                  ...
                  [
                    ...
                    sanitizedHdUrl,
                    sanitizedImageDesc,
                    sanitizedImageClass1,
                    sanitizedImageClass2,
                    sanitizedFailoverUrl,
                    p.mainPhoto ?? false,
                    p.score ?? null,
                    p.classId ?? null,
                    p.classOrder ?? null,
                ...
                log.warn(
                  `Failed to insert room photo for room ${room.id}: ${(err as Error).message}`,
                );
              }
            }
          }
          ...
          log.warn(
            `Failed to insert room ${room.id}: ${(err as Error).message}`,
          );
        }
      }
    ...
    log.error(
      `Error processing detail for hotel ${hotelId}: ${(err as Error).message}`,
    );
...
async function validateAndReportData(): Promise<void> {
  ...
  const stats: Array<[string, Promise<CountRow[]>]> = [
    [
      "Countries",
      query<CountRow>("SELECT COUNT(*) as count FROM shared.countries"),
    ],
    [
      "Currencies",
      query<CountRow>("SELECT COUNT(*) as count FROM shared.currencies"),
    ],
    [
      "Languages",
      query<CountRow>("SELECT COUNT(*) as count FROM shared.languages"),
    ],
    [
      "Hotel Chains",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.chains"),
    ],
    [
      "Hotel Types",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.types"),
    ],
    [
      "Facilities",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.facilities"),
    ],
    [
      "IATA Airports",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.iata_airports"),
    ],
    ["Cities", query<CountRow>("SELECT COUNT(*) as count FROM hotel.cities")],
    [
      "Hotels (basic)",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.hotels"),
    ],
    [
      "Hotel Images",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.images"),
    ],
    [
      "Hotel Rooms",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.rooms"),
    ],
    [
      "Room Bed Types",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.room_bed_types"),
    ],
    [
      "Room Amenities",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.room_amenities"),
    ],
    [
      "Room Photos",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.room_photos"),
    ],
    [
      "Hotel Policies",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.policies"),
    ],
    [
      "Sentiment Analysis",
      query<CountRow>("SELECT COUNT(*) as count FROM hotel.sentiment_analysis"),
    ],
  ];
  ...
  const hotelSample = await query<HotelRow>(
    "SELECT id, name FROM hotel.hotels LIMIT 1",
  );
  ...
  log.info(
    `Cache: ${cacheStats.memorySize} entries in memory (TTL: ${cacheStats.ttlSeconds}s)`,
  );
  log.info("");
}
...
export async function syncLiteAPI(): Promise<void> {
  ...
  log.info(
    `Configuration: HOTELS_DETAIL_LIMIT=${HOTELS_DETAIL_LIMIT}, BATCH_SIZE=${BATCH_SIZE}, API_CALL_DELAY=${API_CALL_DELAY_MS}ms`,
  );
  ...
  const countryCodesToSync = FETCH_ALL_COUNTRIES
    ? allCountries
    : allCountries.slice(0, 15);

  // Get completed countries for resumable import
  const completedCountries = await getCompletedCountries();
  const countriesToProcess = countryCodesToSync.filter(
    (cc) => !completedCountries.has(cc),
  );
  ...
  log.info(
    `Syncing hotels for ${countriesToProcess.length} countries: ${countriesToProcess.join(", ")}`,
  );
  const limit = pLimit(CONCURRENCY);
  let totalHotels = 0;
  let totalDetailed = 0;
  ...
  await Promise.all(
    countriesToProcess.map((cc) =>
      limit(async () => {
        log.info(
          `[${countriesCompleted + 1}/${countriesToProcess.length}] Fetching hotels in ${cc}...`,
        );
        ...
        try {
          ...
          countriesCompleted++;
          const detailCount =
            HOTELS_DETAIL_LIMIT > 0
              ? Math.min(HOTELS_DETAIL_LIMIT, hotelIds.length)
              : hotelIds.length;
          log.info(
            `${cc}: ${hotelIds.length} hotels listed — fetching detail for ${detailCount}... [${countriesCompleted}/${countriesToProcess.length} countries, ${totalHotels} total hotels]`,
          );
          ...
          const slice =
            HOTELS_DETAIL_LIMIT > 0
              ? hotelIds.slice(0, HOTELS_DETAIL_LIMIT)
              : hotelIds;
          ...
          const successCount = detailResults.filter(
          ...
          if (failedCount > 0) {
            log.warn(
              `${cc}: ${successCount} details fetched, ${failedCount} failed`,
            );
          }
          ...
          if (countriesCompleted % CHECKPOINT_INTERVAL === 0) {
            log.info(
              `✓ Checkpoint: ${countriesCompleted}/${countriesToProcess.length} countries completed, ${totalDetailed}/${totalHotels} details fetched`,
            );
          }
  ...
  log.success(
    `Hotels synced: ${totalHotels} listed, ${totalDetailed} detailed (${totalDetailedFailed} failed detail fetches)`,
  );
