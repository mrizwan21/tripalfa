...
export async function get<T>(
  ...
  while (true) {
    try {
      ...
      if (attempt < MAX_RETRIES && (status === 429 || (status !== undefined && status >= 500))) {
        const delay = RETRY_DELAY_MS * attempt;
        console.warn(`[HTTP] ${status} on ${path} — retry ${attempt}/${MAX_RETRIES} in ${delay}ms`);
        await sleep(delay);
        continue;
      }
      throw err;
...
export function createOERClient(appId: string): AxiosInstance {
  return createClient('https://openexchangerates.org/api', {
    'Accept': 'application/json',
  }, 10_000);
}
