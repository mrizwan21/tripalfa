...
const DECIMAL_PRECISION: Record<string, number> = {
  ...
  MGA: 0, PYG: 0, RWF: 0, UGX: 0, VND: 0, VUV: 0, XAF: 0, XOF: 0,
  XPF: 0,
  // 3 decimal places
  BHD: 3, IQD: 3, JOD: 3, KWD: 3, LYD: 3, MRU: 3, OMR: 3, TND: 3,
...
export async function syncExchangeRates(): Promise<void> {
  ...
  const data   = await get<OERResponse>(client, '/latest.json', { app_id: appId });

  const effectiveAt = new Date(data.timestamp * 1000);
  const rates       = data.rates; // { USD: 1, EUR: 0.848512, ... }
  const codes       = Object.keys(rates);

  log.info(`Received ${codes.length} currency rates (base=${data.base}, effective=${effectiveAt.toISOString()})`);
  ...
  await withTransaction(async (client) => {
    ...
    log.success(`Upserted ${upserted} currency rates, logged ${logged} history rows`);
  });
}
