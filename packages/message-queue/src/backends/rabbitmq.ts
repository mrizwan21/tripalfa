...
export class RabbitMQBackend extends EventEmitter implements QueueBackend {
  ...
  async publish<T = any>(
    ...
    if (options?.delay) {
      ...
      this.channel.publish(this.defaultExchange, `queue.${queue}`, content, publishOptions)
    }
    ...
    await this.channel.consume(
      ...
      async (msg) => {
        ...
        try {
          ...
          message.attempts = (msg.properties.headers?.['x-attempts'] as number || 0) + 1
          ...
          const result: ConsumeResult = {
            ack: async () => {
              ...
              stats.averageProcessingTimeMs = 
                ((stats.averageProcessingTimeMs || 0) * (stats.consumedCount - 1) + (Date.now() - startTime)) / stats.consumedCount
              this.stats.set(queue, stats)
            },
            ...
            extendLock: async (durationMs: number) => {
              ...
              this.log(`Lock extension requested for ${durationMs}ms (not supported in RabbitMQ)`)
            },
          }
