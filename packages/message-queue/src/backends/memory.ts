...
export class MemoryBackend extends EventEmitter implements QueueBackend {
  ...
  private consumers: Map<string, { queue: string; handler: MessageHandler }> =
    new Map();
  ...
  private processQueue(queue: string): void {
    ...
    const process = () => {
      ...
      const consumer =
        queueConsumers[Math.floor(Math.random() * queueConsumers.length)];
      ...
      const result: ConsumeResult = {
        ack: async () => {
          ...
          stats.averageProcessingTimeMs =
            ((stats.averageProcessingTimeMs || 0) * (stats.consumedCount - 1) +
              processingTime) /
            stats.consumedCount;
          this.stats.set(queue, stats);
        ...
        extendLock: async (durationMs: number) => {
          ...
          this.log(
            `Lock extended for message ${message.id} by ${durationMs}ms`,
          );
        },
      };
