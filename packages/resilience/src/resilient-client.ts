...
import { CircuitBreaker, CircuitBreakerOptions, createCircuitBreaker } from './circuit-breaker'
...
const DEFAULT_OPTIONS: Omit<Required<Pick<ResilientClientOptions, 'timeout' | 'enableLogging'>>, 'baseUrl' | 'serviceName'> = {
  timeout: 30000,
  enableLogging: false,
}
...
export class ResilientClient {
   ...
   * Make a GET request
   */
  async get<T = any>(path: string, options?: Omit<RequestOptions, 'method' | 'path' | 'body'>): Promise<Response<T>> {
   ...
   * Make a POST request
   */
  async post<T = any>(path: string, body?: any, options?: Omit<RequestOptions, 'method' | 'path' | 'body'>): Promise<Response<T>> {
   ...
   * Make a PUT request
   */
  async put<T = any>(path: string, body?: any, options?: Omit<RequestOptions, 'method' | 'path' | 'body'>): Promise<Response<T>> {
   ...
   */
  async patch<T = any>(path: string, body?: any, options?: Omit<RequestOptions, 'method' | 'path' | 'body'>): Promise<Response<T>> {
   ...
   */
  async delete<T = any>(path: string, options?: Omit<RequestOptions, 'method' | 'path' | 'body'>): Promise<Response<T>> {
    ...
    const executeWithRetry = async (): Promise<Response<T>> => {
      ...
      return retry(
        ...
        },
        {
          ...
          retryableStatusCodes: this.options.retryOptions?.retryableStatusCodes ?? [
 ...
 */
export function createResilientClient(options: ResilientClientOptions): ResilientClient {
