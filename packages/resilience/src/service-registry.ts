...
const DEFAULT_OPTIONS: Omit<Required<ServiceRegistryOptions>, 'circuitBreakerOptions'> = {
...
export class ServiceRegistry extends EventEmitter {
  ...
  private readonly options: ServiceRegistryOptions & { circuitBreakerOptions?: Partial<CircuitBreakerOptions> }
   ...
   */
  register(instance: Omit<ServiceInstance, 'status' | 'lastHealthCheck' | 'consecutiveFailures' | 'consecutiveSuccesses'>): void {
  ...
  deregister(instanceId: string): void {
    for (const [serviceName, instances] of this.services.entries()) {
      ...
      if (index !== -1) {
        ...
        this.log(`Deregistered service instance: ${serviceName} (${instanceId})`)
        return
      }
    }
  }
  ...
  getInstance(serviceName: string): ServiceInstance | null {
    ...
    const healthyInstances = instances.filter(i => 
      i.status === 'healthy' && !this.isCircuitOpen(i.id)
    )

    if (healthyInstances.length === 0) {
      // Fallback to degraded instances if no healthy ones
      const degradedInstances = instances.filter(i => 
        i.status === 'degraded' && !this.isCircuitOpen(i.id)
      )
  ...
  async checkHealth(instance: ServiceInstance): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const result: HealthCheckResult = {
        ...
        responseTimeMs,
        details: response.ok 
          ? await response.json().then(d => d as Record<string, any>).catch(() => undefined) 
          : undefined,
      }
  ...
  getStats(): ServiceRegistryStats {
    const serviceStats = Array.from(this.services.entries()).map(([name, instances]) => ({
      name,
    ...
    const totalInstances = serviceStats.reduce((sum, s) => sum + s.instances, 0)
    const healthyInstances = serviceStats.reduce((sum, s) => sum + s.healthyInstances, 0)
  ...
  private createCircuitBreaker(instanceId: string, serviceName: string): void {
    const cb = new CircuitBreaker({
      ...
      failureThreshold: this.options.circuitBreakerOptions?.failureThreshold ?? 5,
      successThreshold: this.options.circuitBreakerOptions?.successThreshold ?? 3,
