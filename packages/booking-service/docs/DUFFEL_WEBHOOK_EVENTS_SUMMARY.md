...
The existing TripAlfa booking‑service implements signature verification in `packages/booking-service/src/routes/webhooks.ts` (see `verifySignature` function). The secret is stored in the `WEBHOOK_SECRET` environment variable.
...
The TripAlfa booking‑service already includes a production‑ready webhook delivery system (`packages/booking-service/src/routes/webhooks.ts`) that handles Duffel webhooks with signature verification, database logging, and asynchronous processing. The service supports the following Duffel event types:
