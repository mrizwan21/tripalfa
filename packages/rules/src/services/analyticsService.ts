...
export class AnalyticsService {
  ...
  async getDashboardOverview(
    ...
    try {
      ...
      for (const event of events) {
        if (event.eventType === "booking") {
          ...
          if (event.dealPrice && event.originalPrice) {
            ...
            const discount =
              Number(event.originalPrice) - Number(event.dealPrice);
      ...
      // Get trend data
      const bookingsTrend = await this.getTrendData(
        startDate,
        endDate,
        "booking",
        "count",
      );
      const revenueTrend = await this.getTrendData(
        startDate,
        endDate,
        "booking",
        "revenue",
      ...
      return {
        ...
        conversionRate:
          events.length > 0
            ? (totalBookings /
                events.filter((e: any) => e.eventType === "search").length) *
              100
            : 0,
      ...
      for (const [date, dayEvents] of grouped) {
        ...
        if (metric === "count") {
          ...
          value = dayEvents.reduce(
            (sum: number, e: any) => sum + (Number((e as any).dealPrice) || 0),
            0,
          );
        }
    ...
    try {
      ...
      for (const deal of deals) {
        ...
        const bookings = events.filter(
          (e: any) => e.eventType === "booking",
        ).length;
        const searches = events.filter(
          (e: any) => e.eventType === "search",
        ).length;
        let revenue = 0;
        let discountGiven = 0;
  ...
  async getDealPerformance(
    ...
    try {
      ...
      const metrics = {
        totalBookings: events.filter((e: any) => e.eventType === "booking")
          .length,
        totalSearches: events.filter((e: any) => e.eventType === "search")
          .length,
        totalViews: events.filter((e: any) => e.eventType === "view").length,
        totalApplications: events.filter((e: any) => e.eventType === "apply")
          .length,
        totalCancellations: events.filter(
          (e: any) => e.eventType === "cancellation",
        ).length,
        conversionRate: 0,
        totalRevenue: 0,
      ...
      for (const event of events) {
        if (event.customerId) uniqueCustomers.add(event.customerId);
        if (event.eventType === "booking") {
          if ((event as any).dealPrice)
            totalRevenue += Number((event as any).dealPrice);
          if ((event as any).originalPrice && (event as any).dealPrice) {
            const discount =
              Number((event as any).originalPrice) -
              Number((event as any).dealPrice);
      ...
      metrics.conversionRate =
        metrics.totalSearches > 0
          ? (metrics.totalBookings / metrics.totalSearches) * 100
          : 0;
      metrics.totalRevenue = totalRevenue;
      metrics.totalDiscountGiven = totalDiscountGiven;
      metrics.averageBookingValue =
        metrics.totalBookings > 0 ? totalRevenue / metrics.totalBookings : 0;
      metrics.averageDiscountAmount =
        metrics.totalBookings > 0
          ? totalDiscountGiven / metrics.totalBookings
          : 0;
      ...
      throw new Error(
        `Failed to get deal performance: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   */
  private async getChannelMetricsForDeal(
    dealId: string,
    period: Period,
  ): Promise<Record<string, any>> {
    try {
   ...
   */
  private async getTopRoutesForDeal(
    dealId: string,
    period: Period,
    limit: number = 5,
  ): Promise<any[]> {
    try {
      ...
      for (const event of events) {
        ...
        if (event.eventType === "booking") {
          metrics.bookings++;
          if ((event as any).dealPrice)
            metrics.revenue += Number((event as any).dealPrice);
      ...
      for (const metrics of routeMetrics.values()) {
        metrics.conversionRate =
          metrics.searches > 0
            ? (metrics.bookings / metrics.searches) * 100
            : 0;
      }
  ...
  async generateDailySnapshot(
    ...
    try {
      ...
      throw new Error(
        `Failed to generate daily snapshot: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async getActiveDeals(): Promise<any[]> {
    try {
      ...
      throw new Error(
        `Failed to get active deals: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
