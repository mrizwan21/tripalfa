/**
 * CSS custom properties (CSS variables) for Apple-inspired design system
 */
...
export function generateCSSVariables(): string {
  const lines = Object.entries(cssVariables).map(([key, value]) => {
    return `  ${key}: ${value};`;
  });
...
/**
 * CSS variable names as TypeScript type
 */
