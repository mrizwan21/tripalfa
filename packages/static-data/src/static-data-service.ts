...
const DB_URL = process.env.STATIC_DATABASE_URL || "postgresql://postgres@localhost:5432/staticdatabase";
...
export class StaticDataService {
     ...
     */
    async getHotelsNearCoordinates(lat: number, lon: number, _radiusKm: number = 50, limit: number = 20): Promise<Hotel[]> {
        const client = await this.pool.connect();
        try {
            const sql = `
                SELECT id, name, star_rating as stars, address, city, country_code,
