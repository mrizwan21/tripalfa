    ...
    while (hasMore) {
        try {
            ...
            if (uaeHotels.length > 0) {
                ...
                console.log(`✅ Page ${page}: Found ${uaeHotels.length} UAE hotels (Total: ${allHotels.length})`);
            } else {
...
async function processHotels(pool, hotels) {
    ...
    for (let i = 0; i < hotels.length; i += CONFIG.batchSize) {
        ...
        try {
            ...
            // Log progress
            if (stats.processedHotels % CONFIG.logProgressEvery === 0 || stats.processedHotels === stats.totalHotels) {
                const progress = ((stats.processedHotels / stats.totalHotels) * 100).toFixed(1);
                const elapsed = Math.floor((Date.now() - stats.startTime) / 1000);
                console.log(`📊 Progress: ${stats.processedHotels}/${stats.totalHotels} (${progress}%) - ${elapsed}s elapsed`);
            }
            
        } catch (error) {
            console.error(`❌ Error processing batch ${Math.floor(i/CONFIG.batchSize) + 1}:`, error.message);
...
async function processHotelAmenities(client, hotelId, amenities) {
    ...
    for (const amenity of amenities) {
        ...
        try {
            const amenityResult = await client.query(amenityQuery, [amenityName, amenityCategory, isPopular]);
            const amenityId = amenityResult.rows[0].id;

            await client.query(mappingQuery, [hotelId, amenityId]);
        } catch (error) {
            console.warn(`⚠️  Could not process amenity "${amenityName}" for hotel ${hotelId}:`, error.message);
        }
    }
}
...
function generateReport() {
    ...
    console.log(`Success Rate: ${((stats.processedHotels - stats.errors) / stats.processedHotels * 100).toFixed(1)}%`);
