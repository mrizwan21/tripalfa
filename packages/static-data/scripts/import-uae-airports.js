...
function validateAirportData(airport) {
    ...
    if (airport.latitude && (isNaN(airport.latitude) || airport.latitude < -90 || airport.latitude > 90)) {
        errors.push("Invalid latitude");
    }
    
    if (airport.longitude && (isNaN(airport.longitude) || airport.longitude < -180 || airport.longitude > 180)) {
...
function generateReport() {
    ...
    console.log(`Success Rate: ${((stats.processedAirports - stats.errors) / stats.processedAirports * 100).toFixed(1)}%`);
    ...
    if (stats.errors === 0) {
        ...
        console.log(`⚠️  Airports import completed with ${stats.errors} errors. Check logs above for details.`);
    }
}
