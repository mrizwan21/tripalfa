    ...
    try {
      ...
      const rule = await prisma.markupRule.create({
        data: {
          ...
          value: String(inputData.markupValue),
          minValue: inputData.minMarkup ? String(inputData.minMarkup) : null,
          maxValue: inputData.maxMarkup ? String(inputData.maxMarkup) : null,
          conditions: {
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (inputData.markupValue !== undefined)
        updateData.value = String(inputData.markupValue);
      if (inputData.minMarkup !== undefined)
        updateData.minValue = inputData.minMarkup ? String(inputData.minMarkup) : null;
      if (inputData.maxMarkup !== undefined)
        updateData.maxValue = inputData.maxMarkup ? String(inputData.maxMarkup) : null;
      if (
