 ...
 */
export const requireWorkflowOwnership = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        if (targetWorkflowId) {
            ...
            const isOwner = workflowState.customer?.id === req.user.id ||
