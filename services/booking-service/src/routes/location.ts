...
/**
 * GET /api/location/me
 * Detects user location and timezone based on their IP address using ipapi.co
 * Caches results in local PostgreSQL hotel.ip_locations table.
 */
    ...
    try {
        ...
        try {
            const cacheResult = await pool.query(
                "SELECT * FROM hotel.ip_locations WHERE ip = $1",
                [clientIp]
            );
        ...
        const useProvidedIp = clientIp && (isValidIpAddress(clientIp) || isLocalhost(clientIp))
            && !isLocalhost(clientIp);
        const url = useProvidedIp
        ...
        if (response.data && !response.data.error) {
            ...
            pool.query(
                ...
                [
                    locationData.ip, locationData.city, locationData.region, locationData.country,
                    locationData.countryCode, locationData.timezone, locationData.utcOffset,
                    locationData.latitude, locationData.longitude, locationData.currency, locationData.languages
                ]
