...
<head>
  ...
  <style>
    ...
    .header { background: #1e1b4b; color: white; padding: 20px; border-radius: 8px; }
    ...
    .passenger { background: #f9fafb; padding: 10px; margin: 5px 0; border-radius: 4px; }
  </style>
</head>
<body>
  <div class="header">
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; margin: 40px; color: #333; }
    .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #1e1b4b; padding-bottom: 20px; }
    .invoice-title { font-size: 32px; color: #1e1b4b; }
    .invoice-number { font-size: 18px; color: #6b7280; }
    .section { margin: 20px 0; }
    table { width: 100%; border-collapse: collapse; }
    th, td { padding: 12px; text-align: left; border-bottom: 1px solid #e5e7eb; }
    th { background: #f9fafb; }
    .total-row { font-weight: bold; font-size: 18px; background: #1e1b4b; color: white; }
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; margin: 40px; color: #333; }
    .header { background: #1e1b4b; color: white; padding: 20px; border-radius: 8px; text-align: center; }
    .ticket-number { font-size: 24px; font-weight: bold; }
    .section { margin: 20px 0; padding: 15px; border: 1px solid #e5e7eb; border-radius: 8px; }
    .flight-route { font-size: 28px; text-align: center; margin: 20px 0; }
    .passenger-info { background: #f9fafb; padding: 15px; border-radius: 8px; }
  </style>
</head>
<body>
  <div class="header">
  ...
  try {
    ...
    if (!offerId || !passengers || !customerId || !customerEmail || !totalAmount || !currency) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: offerId, passengers, customerId, customerEmail, totalAmount, currency'
      });
    }
    ...
    if (!isRefundable) {
      return res.status(400).json({
        success: false,
        error: 'Hold booking is not available for non-refundable rates. Please select a refundable fare or pay now.',
        holdAvailable: false,
 ...
 * Requires authentication
 */
router.post('/payment', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/:orderId', optionalAuth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * Requires authentication
 */
router.post('/ticket', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * Requires authentication
 */
router.post('/cancel', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * Requires authentication
 */
router.post('/refund', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * Requires authentication
 */
router.get('/workflow/:workflowId', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/workflows', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const key of keys.slice(0, 100)) { // Limit to 100 workflows
 ...
 * Requires authentication
 */
router.post('/full-flow', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const {
      offerId,
      passengers,
    ...
    if (paymentMethod) {
      ...
      if (cancelAfterTicketing) {
        const cancellationReason = cancelAfterTicketing.reason || 'Customer requested cancellation';
