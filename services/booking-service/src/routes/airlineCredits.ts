...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const duffelCredit = await duffelRequest<any>(`/air/airline_credits/${id}`, 'PATCH', updateData);
...
router.get('/customer/:customerId', async (req: Request, res: Response) => {
  try {
    ...
    const airlines = [...new Set(credits.map(c => (c.metadata as any)?.airlineCode).filter(Boolean))];
...
router.post('/:id/redeem', async (req: Request, res: Response) => {
  try {
    ...
    // Log the redemption
    console.log(`[AirlineCredits] Redeemed ${points} points from credit ${id} for booking ${bookingId}`);
