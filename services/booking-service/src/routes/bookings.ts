...
const BOOKING_SERVICE_URL = process.env.BOOKING_SERVICE_URL || 'http://booking-service:3001/api/bookings';
...
const mockQueues: MockQueueRow[] = [
  { id: 1, bookingRef: 'ET1234567', supplierRef: 'S-124567', product: 'Hotel', details: 'Hilton Jumeirah', customerName: 'Mohamed Rizwan', issuedDate: '01 Dec 24', queueStatus: 'Req. Refund', updatedAt: new Date().toISOString() },
  { id: 2, bookingRef: 'ET1234568', supplierRef: 'S-124568', product: 'Flight', details: 'DXB-LON', customerName: 'Mohamed Rizwan', issuedDate: '01 Dec 24', queueStatus: 'Req. Cancel', updatedAt: new Date().toISOString() },
];

// Simple in-memory audit log for admin actions (fallback)
const mockAuditLogs: Array<{ id: number; action: string; performedBy?: string; ts: string; details?: unknown }> = [];
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const newBooking = {
      ...
      issuedDate: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' }),
    ...
    console.error('Error creating manual booking:', error instanceof Error ? error.message : String(error));
    res.status(500).json({
      success: false,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error fetching booking:', error instanceof Error ? error.message : String(error));
    const axiosError = error as { response?: { status?: number } };
    res.status(axiosError.response?.status || 500).json({
      success: false,
...
router.put('/:id/cancel', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.put(`${BOOKING_SERVICE_URL}/${id}/cancel`, {
      reason,
      refundAmount,
      processedBy,
      adminOverride: true
    ...
    console.error('Error cancelling booking:', error instanceof Error ? error.message : String(error));
    const axiosError = error as { response?: { status?: number } };
    res.status(axiosError.response?.status || 500).json({
      success: false,
...
router.put('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.put(`${BOOKING_SERVICE_URL}/${id}`, {
      status,
      reason,
      processedBy,
      adminOverride: true
    ...
    const status = error && typeof error === 'object' && 'response' in error && error.response && typeof error.response === 'object' && 'status' in error.response ? (error.response as { status: number }).status : 500;
...
router.get('/stats/summary', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.get(`${BOOKING_SERVICE_URL}/stats/summary?period=${period}${companyId ? `&companyId=${companyId}` : ''}`, {
      headers: {
        'Authorization': req.headers.authorization || '',
        'X-Admin-Request': 'true'
      }
  ...
  try {
    ...
    try {
      const response = await axios.get(pdfUrl, {
        headers: { 'Authorization': req.headers.authorization || '', 'X-Admin-Request': 'true' },
    ...
    try {
      ...
      else if (invResp.data && typeof invResp.data.html === 'string') html = invResp.data.html;
    } catch (err) {
    ...
    if (!html) return res.status(501).json({ success: false, error: 'No invoice HTML available to generate PDF' });
    ...
    try {
      ...
      console.error('Puppeteer not available or failed to generate PDF:', message);
      return res.status(501).json({ success: false, error: 'PDF generation not available on server (puppeteer missing)' });
    }
  } catch (error) {
    const message = error && typeof error === 'object' && 'message' in error ? (error as Error).message : String(error);
...
router.get('/queues', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.get(`${BOOKING_SERVICE_URL}/queues?${queryParams}`, {
      headers: {
        'Authorization': req.headers.authorization || '',
        'X-Admin-Request': 'true'
      }
    ...
    res.status(error.response?.status || 500).json({ success: false, error: 'Failed to fetch booking queues' });
    // Fallback to in-memory queues so admin UI remains functional
    return res.json({ success: true, queues: mockQueues, total: mockQueues.length });
  }
});
...
router.post('/:id/queue-action', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.post(`${BOOKING_SERVICE_URL}/${id}/queue-action`, { action, reason, processedBy }, {
      headers: {
        'Authorization': req.headers.authorization || '',
        'X-Admin-Request': 'true'
      }
    ...
    res.status(error.response?.status || 500).json({ success: false, error: 'Failed to perform queue action' });
    ...
    mockAuditLogs.push({ id: mockAuditLogs.length + 1, action: `queue-action:${action}`, performedBy: processedBy || 'system', ts: new Date().toISOString(), details: { queueId: id } });
...
router.get('/:id/invoice', async (req: Request, res: Response) => {
  try {
    ...
    res.status(error.response?.status || 500).json({ success: false, error: 'Failed to fetch invoice' });
    ...
    if (q) {
      const invoice = {
        booking: {
          ...
          currency: 'USD',
          date: q.issuedDate,
          passengers: [{ name: 'Sample Passenger', ticket: '000 0000 000 000', baseFare: 100, taxes: 10, discount: 0, netTotal: 110 }]
        }
      };
    ...
    try {
      ...
      try {
        ...
        if (q) {
          ...
          return res.status(501).json({ success: false, error: 'Invoice HTML unavailable to generate PDF' });
        }
      }
      ...
      return res.status(501).json({ success: false, error: 'PDF generation not available on server' });
    }
  }
});
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      mockAuditLogs.push({
        ...
        performedBy: 'admin',
        ts: new Date().toISOString(),
        details: { invoiceId: newInvoice.id, bookingId: id, amount: invoiceAmount }
      });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error('Error processing wallet payment:', error instanceof Error ? error.message : String(error));
      res.status(500).json({
        success: false,
