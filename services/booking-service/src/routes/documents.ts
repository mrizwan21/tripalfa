...
function generateFlightItineraryHtml(booking: FlightBooking, customer: DocumentCustomerInfo): string {
  const formatDate = (date: string) => new Date(date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const formatTime = (time: string) => new Date(time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
  ...
  const segmentsHtml = booking.segments.map((segment, index) => `
    <div style="margin-bottom: 30px; padding: 20px; background-color: #f9f9f9; border-radius: 8px;">
      <h3 style="margin: 0 0 15px 0; color: #333;">Flight ${index + 1}: ${segment.airline} ${segment.flightNumber}</h3>
      <table style="width: 100%;">
        <tr>
          <td style="padding: 10px;">
            <div style="font-size: 24px; font-weight: bold; color: #0066cc;">${formatTime(segment.departureTime)}</div>
            <div>${segment.departureCity} (${segment.departureAirportCode})</div>
            <div style="font-size: 12px; color: #666;">${formatDate(segment.departureDate)}${segment.departureTerminal ? ` - Terminal ${segment.departureTerminal}` : ''}</div>
          </td>
          <td style="padding: 10px; text-align: center;">✈ ${segment.duration || ''}</td>
          <td style="padding: 10px; text-align: right;">
            <div style="font-size: 24px; font-weight: bold; color: #0066cc;">${formatTime(segment.arrivalTime)}</div>
...
<body style="font-family: Arial; padding: 20px;">
  <div style="max-width: 800px; margin: 0 auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #0066cc, #0052a3); padding: 30px; color: white; text-align: center;">
...
function generateFlightInvoiceHtml(booking: FlightBooking, customer: DocumentCustomerInfo, payment: PaymentBreakdown): string {
...
<body style="font-family: Arial; padding: 20px;">
  <div style="max-width: 800px; margin: 0 auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #0066cc, #0052a3); padding: 30px; color: white;">
...
function generateHotelInvoiceHtml(booking: HotelBooking, customer: DocumentCustomerInfo, payment: PaymentBreakdown): string {
...
function generateFlightReceiptHtml(booking: FlightBooking, customer: DocumentCustomerInfo, payment: PaymentBreakdown): string {
...
<body style="font-family: Arial; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #28a745, #20c997); padding: 30px; color: white; text-align: center;">
    ...
    <div style="padding: 30px;">
      ...
      <hr>
      <p><strong>Amount Paid:</strong> <span style="font-size: 24px; color: #28a745;">${payment.currency} ${payment.paidAmount.toFixed(2)}</span></p>
...
function generateHotelReceiptHtml(booking: HotelBooking, customer: DocumentCustomerInfo, payment: PaymentBreakdown): string {
...
function generateRefundNoteHtml(refundAmount: number, currency: string, bookingRef: string, customer: DocumentCustomerInfo): string {
...
<body style="font-family: Arial; padding: 20px;">
  <div style="max-width: 700px; margin: 0 auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #dc3545, #c82333); padding: 30px; color: white; text-align: center;">
      <div style="font-size: 48px;">↩</div>
    ...
    <div style="padding: 30px;">
      ...
      <p><strong>Customer:</strong> ${customer.name}</p>
      <hr>
      <p><strong>Refund Amount:</strong> <span style="font-size: 24px; color: #28a745;">${currency} ${refundAmount.toFixed(2)}</span></p>
...
function generateDebitNoteHtml(amount: number, currency: string, bookingRef: string, customer: DocumentCustomerInfo): string {
...
<body style="font-family: Arial; padding: 20px;">
  <div style="max-width: 700px; margin: 0 auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <div style="background: linear-gradient(135deg, #fd7e14, #e67e22); padding: 30px; color: white; text-align: center;">
      <div style="font-size: 48px;">💳</div>
    ...
    <div style="padding: 30px;">
      ...
      <p><strong>Customer:</strong> ${customer.name}</p>
      <hr>
      <p><strong>Amount Due:</strong> <span style="font-size: 24px; color: #fd7e14;">${currency} ${amount.toFixed(2)}</span></p>
...
function getSampleFlightData(bookingId: string) {
  ...
  const booking: FlightBooking = {
    ...
    passengers: [{ id: 'p1', firstName: 'John', lastName: 'Doe', type: 'adult' }],
    segments: [{
      id: 's1', flightNumber: 'EK2', airline: 'Emirates', airlineIata: 'EK',
      departureAirport: 'Dubai International', departureAirportCode: 'DXB', departureCity: 'Dubai',
      departureTerminal: '3', departureTime: '2026-03-15T08:30:00', departureDate: '2026-03-15',
      arrivalAirport: 'London Heathrow', arrivalAirportCode: 'LHR', arrivalCity: 'London',
      arrivalTerminal: '3', arrivalTime: '2026-03-15T12:45:00', duration: '7h 15m', cabinClass: 'Economy'
    }],
    totalAmount: 1250.00, baseFare: 1000.00, taxes: 250.00, currency: 'USD'
  };

  const payment: PaymentBreakdown = {
    baseFare: 1000.00, taxes: 250.00, fees: 0, total: 1250.00, currency: 'USD', paymentMethod: 'credit_card', paidAmount: 1250.00
  };
...
function getSampleHotelData(bookingId: string) {
  ...
  const booking: HotelBooking = {
    ...
    numberOfNights: 3,
    rooms: [{
      id: 'r1', roomType: 'Deluxe King', roomName: 'Deluxe King Room',
      adults: 2, children: 0, checkIn: '2026-03-15', checkOut: '2026-03-18',
      numberOfNights: 3, ratePerNight: 200.00, totalRate: 600.00, inclusions: ['Breakfast', 'WiFi']
    }],
  ...
  const payment: PaymentBreakdown = {
    baseFare: 600.00, taxes: 60.00, fees: 0, total: 660.00, currency: 'USD', paymentMethod: 'credit_card', paidAmount: 660.00
  };
...
router.get('/:bookingId', async (req: Request, res: Response) => {
  try {
    ...
    const documents = isHotel ? [
      { id: `doc-itin-${bookingId}`, type: 'itinerary', name: 'Hotel Itinerary', format: 'PDF', url: `/api/documents/${bookingId}/download/itinerary?bookingType=hotel`, available: true },
      { id: `doc-inv-${bookingId}`, type: 'invoice', name: 'Hotel Invoice', format: 'PDF', url: `/api/documents/${bookingId}/download/invoice?bookingType=hotel`, available: true },
      { id: `doc-vch-${bookingId}`, type: 'voucher', name: 'Hotel Voucher', format: 'PDF', url: `/api/documents/${bookingId}/download/voucher?bookingType=hotel`, available: true },
      { id: `doc-rct-${bookingId}`, type: 'receipt', name: 'Payment Receipt', format: 'PDF', url: `/api/documents/${bookingId}/download/receipt?bookingType=hotel`, available: true },
      { id: `doc-ref-${bookingId}`, type: 'refund', name: 'Refund Note', format: 'PDF', url: `/api/documents/${bookingId}/download/refund?bookingType=hotel`, available: true },
      { id: `doc-dbt-${bookingId}`, type: 'debit', name: 'Debit Note', format: 'PDF', url: `/api/documents/${bookingId}/download/debit?bookingType=hotel`, available: true },
    ] : [
      { id: `doc-itin-${bookingId}`, type: 'itinerary', name: 'Flight Itinerary', format: 'PDF', url: `/api/documents/${bookingId}/download/itinerary`, available: true },
      { id: `doc-inv-${bookingId}`, type: 'invoice', name: 'Commercial Invoice', format: 'PDF', url: `/api/documents/${bookingId}/download/invoice`, available: true },
      { id: `doc-tkt-${bookingId}`, type: 'ticket', name: 'E-Ticket', format: 'PDF', url: `/api/documents/${bookingId}/download/ticket`, available: false },
      { id: `doc-rct-${bookingId}`, type: 'receipt', name: 'Payment Receipt', format: 'PDF', url: `/api/documents/${bookingId}/download/receipt`, available: false },
      { id: `doc-ref-${bookingId}`, type: 'refund', name: 'Refund Note', format: 'PDF', url: `/api/documents/${bookingId}/download/refund`, available: false },
      { id: `doc-dbt-${bookingId}`, type: 'debit', name: 'Debit Note', format: 'PDF', url: `/api/documents/${bookingId}/download/debit`, available: false },
    ];
...
router.get('/:bookingId/download/:documentType', async (req: Request, res: Response) => {
  try {
    const { bookingId, documentType } = req.params as { bookingId: string; documentType: string };
    const { bookingType } = req.query;
    const isHotel = bookingType === 'hotel';

    console.log(`[Documents] Generating ${documentType} for booking ${bookingId}, isHotel: ${isHotel}`);

    // Get sample data (in production, fetch from database)
    const { booking, customer, payment } = isHotel ? getSampleHotelData(bookingId) : getSampleFlightData(bookingId);
    ...
    switch (documentType) {
      ...
      case 'refund':
        ...
        htmlContent = generateRefundNoteHtml(payment.paidAmount, payment.currency, bookingId, customer);
        break;
      case 'debit':
        documentName = 'Debit_Note';
        htmlContent = generateDebitNoteHtml(150.00, payment.currency, bookingId, customer);
        break;
      default:
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${documentName}_${bookingId}.html"`);
...
router.post('/:bookingId/email', async (req: Request, res: Response) => {
  try {
    ...
    if (!documentType || !recipientEmail) {
      return res.status(400).json({ success: false, error: 'documentType and recipientEmail are required' });
    }

    // In production: queue email with generated PDF attachment
    console.log(`[Documents] Emailing ${documentType} for booking ${bookingId} to ${recipientEmail}`);
