 ...
 *               modificationType:
 *                 type: string
 *                 enum: [date_change, cancellation, upgrade]
 *               details:
 *                 type: object
 *     responses:
 *       200:
 *         description: Modification request processed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 * /api/communications:
 *   post:
 *     summary: Send a customer communication
 *     tags: [Order Management]
 *     requestBody:
 *       required: true
 *       content:
...
router.post('/reviews', (req: Request, res: Response) => {
  const { orderId, customerId, productId, productType, rating, title, comment, pros, cons } =
    req.body;
  ...
  const hasPurchased = orders.some(
    order =>
      order.customerId === customerId && order.items.some(item => item.productId === productId)
  );
 ...
 *     responses:
 *       200:
 *         description: Customer profile created or updated successfully
 *         content:
 *           application/json:
 *             schema:
 *               type: object
