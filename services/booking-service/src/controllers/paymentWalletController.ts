 ...
 */
export const finalizePayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await paymentFinalizationService.finalizePayment(holdOrderId, paymentMethod, paymentDetails);
...
export const topUpWallet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await walletService.topUpWallet(userId, amount, paymentMethod, paymentDetails);
...
export const getWalletTransactions = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const transactions = await walletService.getWalletTransactions(userId as string, Number(limit), Number(offset));
