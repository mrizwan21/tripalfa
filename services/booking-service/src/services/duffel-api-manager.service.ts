...
export const OfferRequestManager = {
   ...
   */
  async processResponse(requestId: string, apiResponse: any): Promise<APIManagerResponse> {
    try {
      // Cache the offer request
      await DuffelOfferCache.cacheOfferRequest(requestId, apiResponse, apiResponse.id);

      // If offers included, cache them too
      if (apiResponse.offers && Array.isArray(apiResponse.offers)) {
        await DuffelOffersCache.cacheOffers(apiResponse.offers);
      }
    ...
    try {
      ...
      if (cached) {
        return {
          ...
          expiresAt: new Date(Date.now() + DUFFEL_CACHE_TTL.OFFER_REQUEST * 1000).toISOString(),
        };
      }
      ...
      console.error('[OfferRequestManager] Error getting offer request:', error);
      throw error;
    }
  },
...
export const OfferManager = {
   ...
   */
  async processResponse(offerId: string, apiResponse: any): Promise<APIManagerResponse> {
    try {
      await DuffelOffersCache.cacheOffers([apiResponse]);
    ...
    try {
      ...
      if (cached) {
        return {
          ...
          source: cached.source,
          cachedAt: new Date().toISOString(),
          expiresAt: new Date(Date.now() + DUFFEL_CACHE_TTL.OFFER * 1000).toISOString(),
        };
      }
  ...
  async processResponse(
    ...
    try {
      const orderData = {
        ...
        confirmedAt: apiResponse.confirmed_at ? new Date(apiResponse.confirmed_at) : null,
        createdAt: new Date(),
        ...apiResponse,
      };

      await DuffelOrderCache.cacheOrder(orderId, orderData, userId);
    ...
    try {
      ...
      if (cached) {
        return {
          ...
          source: cached.source,
          cachedAt: new Date().toISOString(),
          expiresAt: new Date(Date.now() + DUFFEL_CACHE_TTL.ORDER * 1000).toISOString(),
        };
      }
...
export const SeatMapManager = {
  ...
  async processResponse(
    ...
    try {
      await DuffelSeatMapCache.cacheSeatMap(seatMapData, offerId, orderId);
   ...
   */
  async getSeatMap(offerId?: string, orderId?: string): Promise<APIManagerResponse> {
    try {
      ...
      if (cached) {
        return {
          ...
          source: 'redis',
          cachedAt: new Date().toISOString(),
          expiresAt: new Date(Date.now() + DUFFEL_CACHE_TTL.SEAT_MAP * 1000).toISOString(),
        };
      }
...
export const AvailableServicesManager = {
  ...
  async processResponse(
    ...
    try {
      await DuffelServicesCache.cacheAvailableServices(
        orderId,
      ...
      console.error('[AvailableServicesManager] Error processing response:', error);
      throw error;
    }
  },
  ...
  async getAvailableServices(orderId: string): Promise<APIManagerResponse> {
    try {
      ...
      console.error('[AvailableServicesManager] Error getting available services:', error);
      throw error;
    }
  },
...
export const CancellationManager = {
   ...
   */
  async processResponse(cancellationId: string, apiResponse: any): Promise<APIManagerResponse> {
    try {
      ...
      await DuffelCancellationCache.cacheCancellation(cancellationId, cancellationData);
  ...
  async getCancellation(cancellationId: string): Promise<APIManagerResponse> {
    try {
      const cached = await DuffelCancellationCache.getCancellation(cancellationId);
      if (cached) {
        return {
          success: true,
          data: cached,
