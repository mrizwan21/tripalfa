...
before(async () => {
  const serverPath = path.resolve(
    path.join(process.cwd(), "services/user-service/dist/index.js"),
  );
...
test("GET /user/preferences returns 200 with fallback when not in DB", async () => {
  ...
  const res = await new Promise((resolve, reject) => {
    http
      .get({ port: PORT, path: "/user/preferences?userId=test_user" }, (r) => {
        const chunks = [];
...
test("POST /user/preferences handles bad JSON gracefully", async () => {
  const res = await new Promise((resolve) => {
    const req = http.request(
      { method: "POST", port: PORT, path: "/user/preferences" },
      (r) => {
