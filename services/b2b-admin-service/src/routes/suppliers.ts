...
import { validateZod, paginationSchema, createSupplierSchema, idParamSchema } from '../middleware/validate.js';
...
router.post('/:id/credentials',
    ...
    async (req: AuthRequest, res: Response) => {
        try {
            ...
            const existing = await prisma.supplierCredential.findUnique({
                where: {
                    supplierId_name: {
                        supplierId: id,
                        name,
...
router.put('/:id/credentials/:credId',
    requirePermission('suppliers:update'),
    async (req: AuthRequest, res: Response) => {
        try {
...
router.delete('/:id/credentials/:credId',
    requirePermission('suppliers:update'),
    async (req: AuthRequest, res: Response) => {
        try {
            const { id, credId } = req.params;
...
router.get('/:id/hotel-mappings',
    ...
    async (req: AuthRequest, res: Response) => {
        try {
            ...
            if (search) {
                where.OR = [
                    { supplierHotelId: { contains: search as string, mode: Prisma.QueryMode.insensitive } },
                    { canonicalHotel: { name: { contains: search as string, mode: Prisma.QueryMode.insensitive } } },
                ];
            }
...
router.post('/:id/hotel-mappings',
    requirePermission('suppliers:update'),
    validateZod(idParamSchema),
    async (req: AuthRequest, res: Response) => {
        try {
            const { id } = req.params;
            const { canonicalHotelId, supplierHotelId, supplierHotelCode, matchType, matchConfidence } = req.body;
            ...
            const existing = await prisma.supplierHotelMapping.findUnique({
                where: {
            ...
            const mapping = await prisma.supplierHotelMapping.create({
                data: {
                    ...
                    canonicalHotelId,
                    supplierHotelId,
                    supplierHotelCode: supplierHotelCode || null,
                    matchType: matchType || 'manual',
                    matchConfidence: matchConfidence ? parseFloat(matchConfidence) : null,
                },
                include: {
                    canonicalHotel: {
                        select: {
...
router.delete('/:id/hotel-mappings/:mappingId',
    requirePermission('suppliers:update'),
    async (req: AuthRequest, res: Response) => {
        try {
