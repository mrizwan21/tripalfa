  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const transaction = await prisma.$transaction(async tx => {
        ...
        const currentBalance = currentWallet?.balance ?? new Prisma.Decimal(0);
        const newBalance = currentBalance.add(new Prisma.Decimal(amount));
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const transaction = await prisma.$transaction(async tx => {
        ...
        // Calculate new balance
        const currentBalance = currentWallet?.balance ?? new Prisma.Decimal(0);
        const newBalance = currentBalance.sub(new Prisma.Decimal(amount));
