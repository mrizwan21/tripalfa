...
router.get('/wallets/:userId/transactions',
    requirePermission('finance:read'),
    async (req: AuthRequest, res: Response) => {
        try {
            const { userId } = req.params;
...
router.post('/wallets/:userId/credit',
...
router.post('/wallets/:userId/debit',
    requirePermission('finance:update'),
    async (req: AuthRequest, res: Response) => {
        try {
            const { userId } = req.params;
        ...
        try {
            ...
            res.json({
                ...
                data: currencies.map(c => ({
                    ...c,
                    exchangeRate: c.exchangeRate?.toNumber?.() ?? c.exchangeRate,
                    bufferPercentage: c.bufferPercentage?.toNumber?.() ?? c.bufferPercentage,
                })),
            });
        } catch (error) {
...
router.put('/currencies/:code',
    ...
    async (req: AuthRequest, res: Response) => {
        try {
            ...
            const { exchangeRate, bufferPercentage, decimalPrecision, roundingMode, isBaseCurrency, isActive } = req.body;
            ...
            const updateData: any = {};
            if (exchangeRate !== undefined) updateData.exchangeRate = new Prisma.Decimal(exchangeRate);
            if (bufferPercentage !== undefined) updateData.bufferPercentage = new Prisma.Decimal(bufferPercentage);
            if (decimalPrecision !== undefined) updateData.decimalPrecision = decimalPrecision;
            if (roundingMode !== undefined) updateData.roundingMode = roundingMode;
            ...
            res.json({
                ...
                data: {
                    ...updatedCurrency,
                    exchangeRate: updatedCurrency.exchangeRate?.toNumber?.() ?? updatedCurrency.exchangeRate,
                    bufferPercentage: updatedCurrency.bufferPercentage?.toNumber?.() ?? updatedCurrency.bufferPercentage,
                },
