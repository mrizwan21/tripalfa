        ...
        try {
            ...
            res.json({
                ...
                data: modifications.map(m => ({
                    ...m,
                    modificationFee: m.modificationFee?.toNumber?.() ?? m.modificationFee,
                    penaltyFee: m.penaltyFee?.toNumber?.() ?? m.penaltyFee,
                })),
            });
        } catch (error) {
...
router.get('/queues',
    ...
    async (req: AuthRequest, res: Response) => {
        try {
            ...
            if (search) {
                where.OR = [
                    { bookingRef: { contains: search as string, mode: Prisma.QueryMode.insensitive } },
                    { customerEmail: { contains: search as string, mode: Prisma.QueryMode.insensitive } },
                ];
            }
            ...
            const queues = bookings.map(booking => ({
                ...
                details: booking.bookingSegments[0]?.flightNumber ||
