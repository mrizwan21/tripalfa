...
export const monitoringMiddleware = (req: AuthRequest, res: Response, next: NextFunction): void => {
  ...
  res.on("finish", () => {
    ...
    if (responseTime > 1000) {
      console.warn(`[Monitoring] Slow request: ${req.method} ${req.path} - ${responseTime.toFixed(2)}ms`);
    }
  });
    ...
    if (webhookUrl) {
      try {
        ...
        if (error instanceof Error && error.name === "AbortError") {
          console.error(`[ALERT] Webhook request timed out after ${process.env.ALERT_WEBHOOK_TIMEOUT_MS || "10000"}ms`);
        } else {
 ...
 */
export function stopAlertSystem(): void {
  if (alertInterval) {
