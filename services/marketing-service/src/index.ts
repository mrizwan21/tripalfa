 ...
 */
async function verifyToken(token: string): Promise<{ id: string; email: string; role: string; companyId: string } | null> {
  try {
    ...
    if (tokenValue === process.env.INTERNAL_API_KEY && process.env.INTERNAL_API_KEY) {
      return { id: 'system', email: 'system@internal', role: 'admin', companyId: 'system' };
    }
...
const createMarketingCampaignSchema = z.object({
  ...
  campaignType: z.enum(['email', 'sms', 'push', 'social', 'display', 'affiliate', 'referral']).optional(),
...
app.post('/api/marketing/campaigns', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
...
app.patch('/api/marketing/campaigns/:id', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
...
app.post('/api/marketing/campaigns/:id/activate', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
...
app.post('/api/marketing/campaigns/:id/deactivate', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
...
app.delete('/api/marketing/campaigns/:id', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
...
app.post('/api/marketing/promo-codes', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
...
app.post('/api/marketing/promo-codes/validate', async (req: Request, res: Response) => {
  try {
    ...
    if (promoCode.minOrderAmount && purchaseAmount && purchaseAmount < promoCode.minOrderAmount) {
      return res.status(400).json({
        valid: false,
...
app.post('/api/marketing/promo-codes/:id/apply', requireAuth, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
    ...
    // Log usage
    console.log(`[MarketingService] Promo code ${promoCode.code} applied by user ${userId} for booking ${bookingId}`)
...
app.delete('/api/marketing/promo-codes/:id', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string
...
app.get('/api/marketing/analytics/overview', requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const [totalCampaigns, activeCampaigns, totalPromoCodes, activePromoCodes, totalUsage] = await Promise.all([
