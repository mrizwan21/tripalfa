...
const brandingConfigs: BrandingConfig[] = [
  {
    ...
    metaDescription:
      "Book your perfect vacation with our white-label travel portal",
...
router.get("/", (req: Request, res: Response) => {
  ...
  if (!companyId || typeof companyId !== "string") {
    return res
      .status(400)
      .json({ message: "companyId query parameter required" });
  }
  ...
  if (!config) {
    return res
      .status(404)
      .json({ message: "Branding configuration not found" });
  }
...
router.post("/", (req: Request, res: Response) => {
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "companyId, primaryColor, secondaryColor, accentColor, and fontFamily are required",
    });
  }
...
router.put("/:id", (req: Request, res: Response) => {
  ...
  if (emailTemplateHeader !== undefined)
    config.emailTemplateHeader = emailTemplateHeader;
  if (emailTemplateFooter !== undefined)
    config.emailTemplateFooter = emailTemplateFooter;
  if (socialLinks)
    config.socialLinks =
      typeof socialLinks === "string" ? JSON.parse(socialLinks) : socialLinks;
