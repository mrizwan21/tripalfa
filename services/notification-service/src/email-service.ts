...
function replaceTemplateVariables(
  ...
  result = result.replace(
    /\{\{currentFlight\.airline\}\}/g,
    data.currentFlight.airline || "TBD",
  );
  result = result.replace(
    /\{\{currentFlight\.departure\}\}/g,
    data.currentFlight.departure,
  );
  result = result.replace(
    /\{\{currentFlight\.arrival\}\}/g,
    data.currentFlight.arrival,
  );
  result = result.replace(
  ...
  result = result.replace(
    /\{\{currentFlight\.duration\}\}/g,
    data.currentFlight.duration,
  );
  result = result.replace(
    /\{\{currentFlight\.stops\}\}/g,
    data.currentFlight.stops.toString(),
  );

  // Proposed flight
  result = result.replace(
    /\{\{proposedFlight\.airline\}\}/g,
    data.proposedFlight.airline,
  );
  result = result.replace(
    /\{\{proposedFlight\.departure\}\}/g,
    data.proposedFlight.departure,
  );
  result = result.replace(
    /\{\{proposedFlight\.arrival\}\}/g,
    data.proposedFlight.arrival,
  );
  result = result.replace(
  ...
  result = result.replace(
    /\{\{proposedFlight\.duration\}\}/g,
    data.proposedFlight.duration,
  );
  result = result.replace(
    /\{\{proposedFlight\.stops\}\}/g,
    data.proposedFlight.stops.toString(),
  );
  ...
  result = result.replace(
    /\{\{financialImpact\.currency\}\}/g,
    data.financialImpact.currency,
  );
 ...
 */
export function generateAmendmentApprovalEmail(
  data: AmendmentNotificationData,
): string {
  try {
...
async function sendEmailViaResend(
  ...
  console.error(
    `[EMAIL_SERVICE] Failed to send email after ${maxRetries} attempts:`,
    lastError,
  );
  return {
    success: false,
...
export async function sendAmendmentApprovalEmail(
  ...
  try {
    console.log(
      `[EMAIL_SERVICE] Sending amendment approval email to ${data.travelerEmail}`,
    );
    ...
    console.error(
      `[EMAIL_SERVICE] Failed to send amendment approval email`,
      error,
    );
    return {
      success: false,
...
export async function sendAmendmentReminderEmail(
  ...
  try {
    console.log(
      `[EMAIL_SERVICE] Sending amendment reminder email to ${data.travelerEmail}`,
    );
    console.log(`  Booking: ${data.bookingReference}`);
    ...
    if (result.success) {
      ...
      console.error(
        `[EMAIL_SERVICE] Failed to send reminder email: ${result.error}`,
      );
    }
...
export async function sendAmendmentConfirmationEmail(
  ...
  try {
    console.log(
      `[EMAIL_SERVICE] Sending amendment confirmation email to ${travelerEmail}`,
    );
    ...
    if (result.success) {
      ...
      console.error(
        `[EMAIL_SERVICE] Failed to send confirmation email: ${result.error}`,
      );
    }
...
export const emailTemplates = {
  "flight-amendment-approval": {
    ...
    description:
      "Email sent to traveler with amendment details and approval link",
    requiredVariables: [
      "travelerName",
      "bookingReference",
      "currentFlight",
 ...
 */
export function generateWalletDepositReceiptEmail(
 ...
 */
function replaceWalletDepositVariables(
  html: string,
  data: WalletDepositReceiptData,
): string {
  let result = html;
...
export async function sendWalletDepositReceiptEmail(
  ...
  try {
    console.log(
      `[EMAIL_SERVICE] Sending wallet deposit receipt email to ${data.customerEmail}`,
    );
    ...
    if (result.success) {
      ...
      console.error(
        `[EMAIL_SERVICE] Failed to send wallet deposit receipt: ${result.error}`,
      );
    }
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error";
    console.error(
      `[EMAIL_SERVICE] Failed to send wallet deposit receipt email`,
      error,
    );
    return {
      success: false,
