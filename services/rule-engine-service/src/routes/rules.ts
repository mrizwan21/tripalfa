...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { name, description, category, trigger, triggerEvent, condition, actions, priority, metadata } =
      req.body
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const { name, description, condition, actions, priority, enabled, metadata } = req.body
...
router.post('/:id/debug', async (req: Request, res: Response) => {
  try {
    ...
    if (!sampleData) {
      return res.status(400).json({ error: 'Missing required field: sampleData' })
    }
...
router.get('/:id/stats', async (req: Request, res: Response) => {
  try {
    ...
    const successRate = rule.totalExecutions > 0 ? (rule.successCount / rule.totalExecutions) * 100 : 0
