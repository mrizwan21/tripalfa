...
describe('fxService', () => {
  ...
  describe('saveSnapshot', () => {
    it('should save a valid FX snapshot', async () => {
      ...
      const snapshot = await fxService.saveSnapshot('openexchangerates', 'USD', rates, new Date());
    ...
    it('should be idempotent (not create duplicates)', async () => {
      ...
      const snapshot1 = await fxService.saveSnapshot('openexchangerates', 'USD', rates, now);
      const snapshot2 = await fxService.saveSnapshot('openexchangerates', 'USD', rates, now);

      expect(snapshot1.id).toBe(snapshot2.id);

      // Verify only one snapshot exists
      const allSnapshots = await pool.query('SELECT * FROM exchange_rate_snapshots');
  ...
  describe('getLatestSnapshot', () => {
    it('should return the latest active snapshot', async () => {
      ...
      await fxService.saveSnapshot('openexchangerates', 'USD', rates, new Date());
