...
class KiwiDepositService {
  ...
  async ensureKiwiWallet(): Promise<string> {
    ...
    logger.info(
      `[KiwiDepositService] Created Kiwi supplier wallet: ${wallet.id}`,
    );
    return wallet.id;
  }
  ...
  async getBalance(): Promise<{
    ...
    const wallet = (await prisma.wallet.findUnique({
      ...
      | (Awaited<ReturnType<typeof prisma.wallet.findUnique>> & {
          reservedBalance: DecimalLike;
        })
      | null;
  ...
  async holdForBooking(
    ...
    if (existing) {
      logger.info(
        `[KiwiDepositService] Returning existing hold for booking: ${bookingId}`,
      );
      return {
    ...
    if (available < amount) {
      throw new Error(
        `Insufficient Kiwi deposit balance. Available: ${available}, Required: ${amount}`,
      );
    }
    ...
    const expiresAt = new Date(
      now.getTime() + HOLD_DURATION_HOURS * 60 * 60 * 1000,
    );
    ...
    const hold = await prisma.$transaction(async (tx) => {
      ...
      const wallet = (await tx.wallet.findUnique({
        ...
        | (typeof wallet & { reservedBalance: ReturnType<typeof toNumber> })
        | null;
    ...
    logger.info(
      `[KiwiDepositService] Funds held for booking: ${bookingId}, amount: ${amount} ${currency}`,
    );
  ...
  async settleBooking(
    ...
    logger.info(
      `[KiwiDepositService] Booking settled: ${kiwiBookingId}, gross: ${grossAmount}, commission: ${commission}, net: ${netAmount}`,
    );
  ...
  async processRefund(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          `[KiwiDepositService] Refund API error: ${response.status} - ${error}`,
        );
      ...
      logger.info(
        `[KiwiDepositService] Refund processed: ${request.kiwiBookingId} -> ${result.refund_id}`,
      );
      ...
      logger.error(
        `[KiwiDepositService] Refund processing failed: ${errorMessage}`,
      );
      throw error;
    }
  }
  ...
  async handleWebhook(payload: KiwiWebhookPayload): Promise<void> {
    logger.info(
      `[KiwiDepositService] Webhook received: ${payload.event} for booking ${payload.booking_id}`,
    );
    ...
    switch (payload.event) {
      ...
      default:
        logger.warn(
          `[KiwiDepositService] Unhandled webhook event: ${payload.event}`,
        );
    }
  }

  private async handleBookingConfirmed(
    payload: KiwiWebhookPayload,
  ): Promise<void> {
  ...
  private async handlePriceChange(payload: KiwiWebhookPayload): Promise<void> {
    ...
    logger.warn(
      `[KiwiDepositService] Price change notification for booking ${payload.booking_id} - manual review required`,
    );
    ...
    try {
      ...
      await prisma.kiwiPriceChange.create({
        data: {
          ...
          difference:
            (payload.data?.new_price || 0) - (payload.data?.old_price || 0),
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      logger.error(
        `[KiwiDepositService] Failed to create price change record for booking ${payload.booking_id}: ${errorMessage}`,
      );
    }
  }
  ...
  private async handleScheduleChange(
    ...
    logger.info(
      `[KiwiDepositService] Schedule change for booking ${payload.booking_id} - notifying customer`,
    );
  ...
  async addBaggage(
    kiwiBookingId: string,
    baggage: { weight: number; count: number },
  ): Promise<any> {
    // TODO: Implement baggage API call
    logger.info(
      `[KiwiDepositService] Adding baggage to booking ${kiwiBookingId}: ${baggage.weight}kg x ${baggage.count}`,
    );
    return {
      success: true,
      message: "Baggage addition requires Kiwi API integration",
    };
  }
  ...
  async addSeating(
    kiwiBookingId: string,
    seats: Array<{ segment: number; seat: string }>,
  ): Promise<any> {
    // TODO: Implement seating API call
    logger.info(
      `[KiwiDepositService] Adding seating to booking ${kiwiBookingId}: ${seats.length} seats`,
    );
    return {
      success: true,
      message: "Seating addition requires Kiwi API integration",
    };
  }
  ...
  async addDisruptionProtection(kiwiBookingId: string): Promise<any> {
    ...
    logger.info(
      `[KiwiDepositService] Adding disruption protection to booking ${kiwiBookingId}`,
    );
    return {
      success: true,
      message: "Disruption protection requires Kiwi API integration",
    };
  }
   ...
   */
  async checkBalanceAlert(): Promise<{
    needsTopup: boolean;
    ...
    if (needsTopup) {
      logger.warn(
        `[KiwiDepositService] Low balance alert: ${total} (min: ${this.config.minBalance})`,
      );
    }
  ...
  async releaseExpiredHolds(): Promise<number> {
    ...
    for (const hold of expired) {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        logger.error(
          `[KiwiDepositService] Failed to release expired hold ${hold.bookingId}: ${errorMessage}`,
        );
      }
    }
    ...
    if (expired.length > 0) {
      logger.info(
        `[KiwiDepositService] Released ${expired.length} expired holds`,
      );
    }
