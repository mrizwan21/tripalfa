 ...
 * /api/wallet/transfer:
 *   post:
 *     summary: Transfer funds between wallets with FX conversion
 *     tags: [Transfers]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *           schema:
 *             type: object
 *             required: [userId, fromCurrency, toCurrency, amount, idempotencyKey]
 *             properties:
 *               userId:
 *                 type: string
 *               fromCurrency:
...
router.post('/transfer', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  const { userId, fromCurrency, toCurrency, amount, idempotencyKey } = req.body as TransferRequest;

  // Input validation
  if (!userId || !fromCurrency || !toCurrency || !amount || !idempotencyKey) {
    res.status(400).json({
      error: 'Missing required fields: userId, fromCurrency, toCurrency, amount, idempotencyKey',
    });
    return;
  }
  ...
  if (fromCurrency === toCurrency) {
    res.status(400).json({ error: 'Currencies must be different for transfer' });
    return;
  }
  ...
  try {
    ...
    // 1. IDEMPOTENCY CHECK
    // ========================================================================
    console.log(`${SERVICE_NAME}: Checking idempotency for key ${idempotencyKey}`);
    ...
    // ========================================================================
    console.log(`${SERVICE_NAME}: Converting ${amount} ${fromCurrency} -> ${toCurrency}`);
    const fxData = await convertAmount(parseFloat(String(amount)), fromCurrency, toCurrency);
    ...
    if (!toWalletResult.rows.length) {
      throw new Error(`Destination wallet not found: ${userId} ${toCurrency}`);
    }
    ...
    if (fromBalance < amountNum) {
      throw new Error(`Insufficient funds: have ${fromBalance} ${fromCurrency}, need ${amountNum}`);
    }
    ...
    try {
      await pool.query(
        ...
        [idempotencyKey, 'hash_placeholder', JSON.stringify(errorResponse), 400]
      );
    } catch (cacheErr) {
      console.log(`${SERVICE_NAME}: Failed to cache error response`, cacheErr as Error);
    }
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.log(`${SERVICE_NAME}: Failed to fetch transfer history`, err as Error);
