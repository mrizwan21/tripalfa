...
- Initialize Neon CLI: `brew install neonctl && neonctl init`
- Select/create your Neon project/container with CLI
...
- **Deployment Platform** (Render, Railway, Heroku, etc.):
  - Build: `npm run build`
  - Start: `npm start`
...
## Migration: Docker PostgreSQL to Neon
...
### 2. Create/Select a Neon Project and Database

1. Install and initialize CLI: `brew install neonctl && neonctl init`
2. Select or create a Neon project/container (PostgreSQL 15+).
3. Copy pooled/direct connection strings for later use.
...
1. Manually check balances and transactions via API endpoints or a Postgres client (e.g., TablePlus, DBeaver).
2. Confirm all scheduled jobs (FX fetcher, reconciliation) run successfully.
