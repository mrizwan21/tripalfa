...
if (!JWT_SECRET) {
  console.error(
    "[KYCService] FATAL: JWT_SECRET environment variable is not set!",
  );
...
async function verifyToken(
  ...
  try {
    ...
    if (!JWT_SECRET) {
      console.error(
        "[KYCService] Cannot verify JWT: JWT_SECRET is not configured",
      );
      return null;
    }
...
// GET /api/kyc - List all KYC verifications with pagination (Admin only)
...
app.post("/api/kyc/submit", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[KYCService] KYC verification submitted for user ${data.userId}`,
    );
...
// Users can only view their own status, admins can view any
...
// GET /api/kyc/:id - Get KYC verification by ID (Admin only for full details)
app.get("/api/kyc/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (
      authenticatedUser?.role !== "admin" &&
      authenticatedUser?.id !== verification.userId
    ) {
...
// PATCH /api/kyc/:id/status - Update KYC verification status (Admin only)
app.patch(
  "/api/kyc/:id/status",
  requireAdmin,
  async (req: Request, res: Response) => {
    try {
      ...
      const { status, rejectionReason, reviewedBy, notes } =
...
// POST /api/kyc/:id/resubmit - Resubmit KYC after rejection
// Users can only resubmit their own rejected verifications
...
// DELETE /api/kyc/:id - Delete KYC verification (soft delete by expiring) (Admin only)
app.delete(
  "/api/kyc/:id",
  requireAdmin,
  async (req: Request, res: Response) => {
...
// GET /api/kyc/stats/summary - Get KYC statistics summary (Admin only)
app.get(
