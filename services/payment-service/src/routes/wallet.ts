...
export default (walletManager: WalletManager): Router => {
    ...
    router.post('/credit', async (req: Request, res: Response) => {
        try {
            ...
            if (!currency || !amount || !key) {
                return res.status(400).json({ error: 'Currency, amount, and idempotencyKey are required' });
            }

            const transaction = await walletManager.creditWallet(userId, currency, amount, reason, key as string);
    ...
    router.post('/debit', async (req: Request, res: Response) => {
        try {
            ...
            const transaction = await walletManager.debitWallet(userId, currency, amount, reason, key as string);
            res.status(201).json(transaction);
        } catch (error: any) {
    ...
    router.post('/transfer', async (req: Request, res: Response) => {
        try {
            ...
            if (!fromCurrency || !toCurrency || !amount || !key) {
                return res.status(400).json({ error: 'fromCurrency, toCurrency, amount, and idempotencyKey are required' });
            }

            const transfer = await walletManager.transferBetweenCurrencies(userId, fromCurrency, toCurrency, amount, key as string);
