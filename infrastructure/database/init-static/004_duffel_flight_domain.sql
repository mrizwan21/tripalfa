...
-- NOTE: Disabled trigram indexes due to character type incompatibility
-- These can be recreated with CAST to text if needed:
-- CREATE INDEX idx_places_name_trgm ON flight.places USING GIN (name::text gin_trgm_ops);
-- CREATE INDEX idx_places_iata_trgm ON flight.places USING GIN (iata_code::text gin_trgm_ops);
