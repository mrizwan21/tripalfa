...
function fail(message: string, details?: any): never {
...
function assert(condition: any, message: string, details?: any): asserts condition {
    if (!condition) {
...
async function requestJson(url: string, options: any = {}): Promise<ApiResponse> {
    const response = await fetch(url, options);
    const json = await response.json().catch(() => ({}));
    return { ok: response.ok, status: response.status, json: json as any };
}

async function postJson(url: string, token: string, body: any): Promise<ApiResponse> {
...
async function getJson(url: string, token: string): Promise<ApiResponse> {
    return requestJson(url, {
        headers: {
            authorization: `Bearer ${token}`,
        },
...
function hasText(source: string, text: string): boolean {
