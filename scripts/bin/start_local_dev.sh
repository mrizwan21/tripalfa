...
start_service() {
  ...
  cd "$WORKSPACE_DIR"
  PORT=$port API_GATEWAY_PORT=$port BOOKING_SERVICE_PORT=$port COMPANY_SERVICE_PORT=$port \
...
declare -a SERVICES=(
  ...
  "static-data-service:packages/static-data:3002"
  "booking-service:services/booking-service:3001"
  "user-service:services/user-service:3004"
  "company-service:services/company-service:3006"
  ...
  "booking-engine-app:apps/booking-engine:5174"
)
...
echo "  Static Data Service:   http://localhost:3002"
...
echo "  B2B Admin App:         http://localhost:5173"
echo "  Booking Engine App:    http://localhost:5174"
echo ""
